...
pompath = os.path.join(os.path.dirname(os.path.dirname(__file__)), "pom.xml")
...
# The most recently release X.Y.Z. version,
...
# The release series, 34, 32, etc...
series =  re.sub('\\..*', '', re.sub('-.*', '', version))
print("Docs generated for series", series)

if snapshot:
  # Adjust version to 34.x format for readability
  branch = version.replace('-SNAPSHOT', '.x')
  print("Building snapshot docs from branch", branch)
else:
  # Docs being generated during the release process, so use the tag
  branch = release
  print("Building release docs from branch", branch)
