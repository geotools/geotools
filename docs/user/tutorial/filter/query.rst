...
Note the same code can be used to connect to quite different types of data stores as specified by the
...
9. Spatial queries are also supported:
   ...
   .. note::
     The geometry of a Shapefile is always called ``the_geom``, for other
     data stores we would need to look up the name of the geometry attribute.
