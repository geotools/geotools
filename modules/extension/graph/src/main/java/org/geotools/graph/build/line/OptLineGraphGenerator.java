 ...
 * <br>
 * Since building optimized graphs requires knowing the degree of nodes before creating them, the physical construction
 * of the graph is delayed until a call to generate() is made. No component is created with a call to add(Object), only
 * information about the object is recorded.
 *
...
public class OptLineGraphGenerator implements LineGraphGenerator {
    ...
    /**
     * Returns the edge which represents a line. This method must be called after the call to generate(). Note that if
     * the exact same line has been added to the graph multiple times, then only one of the edges that represents it
     * will be returned. It is undefined which edge will be returned.
     *
