...
/**
 * Optimized implementation of DirectedNode. The following optimizations reduce
 * space and increase performance. <br>
 *
 * <UL>
 * <LI>In and Out edge adjacency list stored as arrays of exact size.
 * <LI>Support from removing edges is removed
 * <LI>The related component iterators iterates over the underlying edge arrays
 * of the node instread of newly created collections.
 * </UL>
 *
...
public class OptDirectedNode extends OptGraphable implements DirectedNode {
  ...
  /**
   * Constructs a new OptDirectedNode. This constructor does not create the edge
   * adjacency arrays for the node.
   */
   ...
   *
   * @param indegree
   *          Number of in adjacenct edges to the node.
   * @param outdegree
  ...
  public Edge getEdge(Node other) {
    ...
    if (e == null)
      e = getOutEdge((DirectedNode) other);
    return (e);
  }
  ...
  public Edge getInEdge(DirectedNode other) {
    for (int i = 0; i < m_in.length; i++) {
      if (m_in[i].getInNode().equals(other))
        return (m_in[i]);
    }
    return (null);
  }
  ...
  public Edge getOutEdge(DirectedNode other) {
    for (int i = 0; i < m_out.length; i++) {
      if (m_out[i].getOutNode().equals(other))
        return (m_out[i]);
    }
  ...
  /**
   * Overrides the default deserialization operation. Since edge adjacency lists
   * of Nodes are not written out upon serialization, they must be recreated
   * upon deserialization.
   *
   * @param in
   *          Object input stream containing serialized objects.
   * @throws IOException
  ...
  /**
   * Overrides the default serialization operation. Since edge adjacency lists
   * of Nodes are not written out upon serialization, all the information needed
   * to recreate them must be written to the object stream as well. Since the
   * edge list is not written out, and the node does not store its degree
   * explicitly, it must be written to the output stream.
   *
   * @param in
   *          Object output stream containing serialized objects.
   * @throws IOException
   * @throws ClassNotFoundException
   */
...
>>>>>>> Stashed changes
    ...
    public Node next() {
      ...
      case BOTH:
        return (m_index < m_in.length ? m_in[m_index++].getInNode() : m_out[m_index++ - m_in.length].getOutNode());
      }
      return (null);
    }
  }
