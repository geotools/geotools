...
/**
 * A path iterator that uses a function (usually denoted f(x)) to determine the
 * order in which the algorithm visits nodes, f(x) is a sum of two functions:
 *
 * <ul>
 * <li>The path-cost function (usually denoted g(x), which may or may not be a
 * heuristic)
 ...
 *
 * @author Germán E. Trouillet, Francisco G. Malbrán. Universidad Nacional de
 *         Córdoba (UNC)
 */
public class AStarIterator extends SourceGraphIterator {
  ...
  /**
   * Returns the next node in the priority queue. if the queue is empty then
   * there is no path from the source to the destiny in this graph.
   *
  ...
  /**
   * Makes a step of the A* algorithm. Takes the current node, looks for its
   * neighbours. The ones which are closed are discarted. The ones "in" the
   * opened queue are checked to see if its necessary to update them. The rest
   * of the nodes are initialized as AStarNodes and inserted in the opened
   * queue.
   *
   ...
   *
   * @author German E. Trouillet, Francisco G. Malbrán. Universidad Nacional de
   *         Córdoba (UNC)
   */
