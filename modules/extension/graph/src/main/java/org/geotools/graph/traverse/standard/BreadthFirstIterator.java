 ...
 * <br>
 * The iteration operates by maintaining a node queue of <B>active</B> nodes. An
 * <B>active</B> node is a node that will returned at a later stage of the
 * iteration. The node queue for a Breadth First iteration is implemented as a
 * <B>First In First Out</B> queue. A node is placed in the the node queue if it
 * has not been visited, and it is adjacent to a a node that has been visited.
...
public class BreadthFirstIterator extends SourceGraphIterator {
   ...
   * Subsequent calls to the method clear the node queue and reset the
   * iteration.
   *
   ...
   * is possible for the node queue to contain duplicate entries. To prevent the
   * iteration returning the same node multiple times, the visited flag is
   * checked on nodes coming out of the queue. If the flag is set, the node is
   * ignored, not returned, and the next node in the queue is returned. This is
   ...
   * An adjacent node is only placed into the node queue if its visited flag is
   * unset.
   *
