 ...
 * <br>
 * Initially all nodes of degree less than two are <B>active</B> (ready to be
 * visited). As nodes are visited, a node can become active when all but one of
 * its related nodes have been visited ( degree = counter + 1). When a node
 * becomes active it is placed into the <B>active node queue</B> (queue of nodes
 * to be visited). The Breadth First Topological iterator places nodes into the
 * queue in <B>First In First Out</B> order.<br>
 * <br>
 * To determine when a node is to become active the iterator uses the counter
 * associated with each node. If these counters are modified by an entity other
 * then the iterator, the iteration may be affected in undefined ways.
 *
...
public class BreadthFirstTopologicalIterator extends AbstractGraphIterator {
  ...
  /**
   * Continues the iteration by incrementing the counters of any unvisited nodes
   * related to the current node. If any related nodes have counters equal to
   * degree of that node - 1, it is placed into the active queue.
   *
  ...
  public void cont(Graphable current, GraphTraversal traversal) {
    ...
    for (Iterator<? extends Graphable> itr = current.getRelated(); itr.hasNext();) {
      ...
      if (!traversal.isVisited(related)) {
        ...
        if (related.getDegree() - 1 == related.getCount())
          m_queue.add(related);
      }
    }
  }
  ...
  /**
   * Kills the current branch of the traversal by not incrementing the counters
