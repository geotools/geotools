...
/**
 * Removes all nodes of degree 2 from a graph. The following are examples of
 * graphs being fused.<br>
 * <br>
 ...
 * <br>
 * When a node of degree 2 is removed from tan unfused graph, the two edges is
 * is adjacent to must be merged into a single edge. More generally if n
...
public class GraphFuser {
   ...
   *          Graph to fuse.
   * @param builder
   *          GraphBuilder used to fuse graph.
   * @param merger
   *          Used to merge edges.
   */
    ...
    if (m_ndegree2 == 0) {
      ...
      for (Iterator<ArrayList<Graphable>> sitr = m_sets.iterator(); sitr.hasNext();) {
        ...
        if (nodes.size() == 1) {
          ...
          rel = node.getRelated();
          last = (Node) rel.next();
          if (last.equals(nodes.get(nodes.size() - 2)))
            last = (Node) rel.next();
