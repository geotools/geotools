...
public class GraphTestUtil {
   ...
   *
   * @param builder
   *          Builder to use to construct graph.
   * @param nnodes
  ...
  public static Node[] buildSingleBifurcation(final GraphBuilder builder, int nnodes, final int bifurcation) {
    ...
    builder.getGraph().visitNodes(new GraphVisitor() {
      @Override
   ...
   * Each node created contains a string representing the nodes location in the
   * tree.<br>
   * <br>
   * locstring(root) = "0"; locstring(node) = locstring(parent) + ".0" (if left
   * child); locstring(node) = locstring(parent) + ".1" (if right child);
   *
   * @param builder
   *          Builder to construct graph with
   * @param levels
   *          Number of levels in the tree.
   * @return A 2 element object array. 0 = root node 1 = map of locstring to
   *         node
   */
