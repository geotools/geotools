...
public class DisjointIntegrity extends RelationIntegrity {
     ...
     *
     * <p>This validation tests for a geometry that is disjoint with itself. Uses JTS'
     * Geometry.disjoint(Geometry) method. <b>Description:</b><br>
     *
     * <p>The function filters the SimpleFeatureSource using the given bounding box. It creates
     * iterators over the filtered FeatureSource. It calls disjoint() using the geometries in the
     ...
     *
     * @param featureSourceA - the SimpleFeatureSource to pull the original geometries from. This
     *     geometry is the one that is tested for being disjoint with itself
     * @param expected - boolean value representing the user's expected outcome of the test
     * @param results - ValidationResults
     * @param bBox - Envelope - the bounding box within which to perform the disjoint()
     * @return boolean result of the test
