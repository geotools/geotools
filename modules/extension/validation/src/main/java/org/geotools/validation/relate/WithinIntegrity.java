 ...
 *     <p>Tests to see if a Geometry lies within another Geometry.
...
public class WithinIntegrity extends RelationIntegrity {
     ...
     *
     * <p>This validation tests for a geometry lies within another geometry. Uses JTS'
     * Geometry.within(Geometry) method. Returns true if the DE-9IM intersection matrix for the two
     * Geometrys is T*F**F***. <b>Description:</b><br>
     *
     * <p>The function filters the FeatureSources using the given bounding box. It creates iterators
     * over both filtered FeatureSources. It calls within() using the geometries in the
     ...
     *
     * <p>This validation tests for a geometry that lies within itself. Uses JTS'
     * Geometry.within(Geometry) method. Returns true if the DE-9IM intersection matrix for the two
     * Geometrys is T*F**F***. <b>Description:</b><br>
     *
     * <p>The function filters the SimpleFeatureSource using the given bounding box. It creates
     * iterators over the filtered FeatureSource. It calls within() using the geometries in the
