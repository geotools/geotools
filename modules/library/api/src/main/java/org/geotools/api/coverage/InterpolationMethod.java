...
/**
 * A list of codes that identify interpolation methods that may be used for evaluating {@linkplain ContinuousCoverage
 * continuous coverages}. Evaluation of a continuous coverage involves interpolation between known feature attribute
 * values associated with geometric objects in the domain of the {@linkplain DiscreteCoverage discrete coverage} that is
 * provided as control for the continuous coverage. This code list includes 9 interpolation methods. Each is used in the
 * context of specified geometric configurations (table below). Since {@code InterpolationMethod} is a {@code CodeList},
 * it may be extended in an application schema that specifies additional interpolation methods.
 *
 * <p>
 *
 * <table border="1">
...
public class InterpolationMethod extends CodeList<InterpolationMethod> {
     ...
     * with the nearest domain object in the domain of the coverage. Nearest neighbour interpolation extends a discrete
     * coverage to a step function defined on the convex hull of the domain objects in the domain of the coverage.
     * Nearest neighbour interpolation is the only interpolation method described in ISO 19123 that can be used to
     * interpolate attributes that have nominal or ordinal values.
     *
     * <p><B>NOTE:</B> In the case of a discrete point coverage, the "steps" of the step function are the Thiessen
     * polygons generated by the set of points in the domain of the coverage.
     */
     ...
     * </blockquote>
     *
     * <p>Linear interpolation may be used to interpolate feature attribute values along a line segment connecting any
     ...
     * (<var>p</var><sub>t</sub>, <var>v</var><sub>t</sub>), where <var>p</var><sub>s</sub> is the start point and
     * <var>p</var><sub>t</sub> is the end point of a value segment, and <var>v</var><sub>s</sub> and
     * <var>v</var><sub>t</sub> are the feature attribute values associated with those points, the feature attribute
     * value <var>v</var><sub>i</sub> associated with the direct position <var>p</var><sub>i</sub> is:
     *
     * <p>
     *
     * <blockquote>
     ...
     * </blockquote>
     *
     * <p>where <var>a</var> is the value of a feature attribute at the start of a value segment and <var>v</var> is the
     * value of a feature attribute at distance <var>x</var> along the curve from the start. Three point value pairs are
     * needed to provide control values for calculating the coefficients of the function.
     */
     ...
     *
     * <p>where <var>a</var> is the value of a feature attribute at the start of a value segment and <var>v</var> is the
     * value of a feature attribute at distance <var>x</var> along the curve from the start. Four point value pairs are
     * needed to provide control values for calculating the coefficients of the function.
     */
