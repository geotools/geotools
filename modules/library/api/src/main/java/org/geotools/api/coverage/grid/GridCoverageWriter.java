...
/**
 * Support for writing grid coverages into a persistent store. Instance of {@code GridCoverageWriter} are obtained
 * through a call to {@code AbstractGridFormat.getWriter}. Grid coverages are usually added to the output stream in a
 * sequential order.
 *
...
public interface GridCoverageWriter {
    ...
    /**
     * Returns the output destination. This is the object passed to the {@code AbstractGridFormat.getWriter} method. It
     * can be a {@link java.lang.String}, an {@link java.io.OutputStream}, a {@link java.nio.channels.FileChannel}, etc.
     */
    Object getDestination();

    /**
     * Returns the list of metadata keywords associated with the {@linkplain #getDestination output destination} as a
    ...
    /**
     * Set the name for the next grid coverage to {@linkplain #write write} within the {@linkplain #getDestination
     * output destination}. The subname can been fetch later at reading time.
     *
     * @throws IOException if an error occurs during writing.
     ...
     * @throws FileFormatNotCompatibleWithGridCoverageException if the grid coverage can't be exported in the
     ...
     * no longer be using this {@code GridCoverageWriter}. Otherwise, the writer may continue to hold on to resources
     * indefinitely.
     *
     * @throws IOException if an error occured while disposing resources (for example while flushing data and closing a
     *     file).
     */
