 ...
 * org.geotools.api.coverage.grid.GridPoint grid points}, and the interstices between the grid lines
 * are {@linkplain org.geotools.api.coverage.grid.GridCell grid cells}.
 *
 ...
 * org.geotools.api.coverage.grid.GridCell grid cells} are parallelograms or parallelepipeds. In its
 * own coordinate system, such a grid is a network composed of two or more sets of equally spaced
 ...
 * org.geotools.api.coverage.grid.Grid#getDimension dimension of the grid}. It has one set of grid
 * lines parallel to each axis. The size of the grid is described by a sequence of integers, in
 * which each integer is a count of the number of lines parallel to one of the axes. There are
 * {@linkplain org.geotools.api.coverage.grid.GridPoint grid points} at all grid line intersections.
 * The axes of the grid provide a basis for defining {@linkplain
 * org.geotools.api.coverage.grid.GridCoordinates grid coordinates}, which are measured along the
 * axes away from their origin, which is distinguished by having coordinate values of 0. Grid
 * coordinates of grid points are integer numbers. The axes need to be identified to support
 * sequencing rules for associating feature attribute value records to the grid points.
 *
 * <BLOCKQUOTE>
 *
 ...
 * org.geotools.api.referencing.crs.CoordinateReferenceSystem coordinate reference system}. This
 * requires additional information about the location of the grid's origin within the external
 * coordinate reference system, the orientation of the grid axes, and a measure of the spacing
 * between the grid lines. If the spacing is uniform, then there is an affine relationship between
 * the grid and external coordinate system, and the grid is called a rectified grid. If, in
 * addition, the external coordinate reference system is related to the earth by a datum, the grid
 * is a georectified grid. The grid lines of a rectified grid need not meet at right angles; the
 * spacing between the grid lines is constant along each axis, but need not be the same on every
 * axis. The essential point is that the transformation of grid coordinates to coordinates of the
 * external coordinate reference system is an affine transformation.
 *
 * <BLOCKQUOTE>
 *
 ...
 *
 * <P ALIGN="justify">When the relationship between a {@linkplain
 * org.geotools.api.coverage.grid.Grid grid} and an external {@linkplain
 * org.geotools.api.referencing.crs.CoordinateReferenceSystem coordinate reference system} is not
 * adequate to specify it in terms of an origin, an orientation, and spacing in that coordinate
 * reference system, it may still be possible to transform the grid coordinates into coordinates in
 * the coordinate reference system. This transformation need not be in analytic form; it may be a
 * table, relating the grid points to coordinates in the external coordinate reference system. Such
 * a grid is classified as a referenceable grid. If the external coordinate reference system is
 * related to the earth by a datum, the grid is a georeferenceable grid. A referenceable grid is
 * associated with information that allows the location of all points in the grid to be determined
 * in the coordinate reference system, but the location of the points is not directly available from
 * the grid coordinates, as opposed to a rectified grid where the location of the points in the
 * coordinate reference system is derivable from the properties of the grid itself. The
 * transformation produced by the information associated with a referenceable grid will produce a
 * grid as seen in the coordinate reference system, but the grid lines of that grid need not be
 * straight or orthogonal, and the grid cells may be of different shapes and sizes.
 *
 ...
 * org.geotools.api.coverage.grid.GridPoint grid point} represent characteristics of the real world
 ...
 * org.geotools.api.coverage.grid.GridCell grid cells}. A common simplifying assumption is that the
