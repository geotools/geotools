...
/**
 * A parameter value used by an operation method. Most parameter values are numeric, but other types of parameter values
 * are possible. The parameter type can be fetch with the <code>
...
public interface ParameterValue<T> extends GeneralParameterValue {
     ...
     * convenience method applies unit conversion on the fly as needed.
     *
     ...
     * @throws InvalidParameterTypeException if the value is not a numeric type.
     * @unitof Measure
     * @rename Renamed {@code value} to {@code doubleValue} for consistency with {@link Number#doubleValue} and the
     *     other {@code fooValue} in this interface. Also because {@link #getValue} is already used for an
     *     {@link Object} type, for consistency with {@link #setValue(Object)}.
     * @see #getUnit
     * @see #setValue(double)
     * @see #doubleValueList
     */
     ...
     * @rename Renamed {@code integerValue} to {@code intValue} for consistency with {@link Number#intValue} and the
    ...
    /**
     * Returns an ordered sequence of numeric values in the specified unit of measure. This convenience method applies
     * unit conversions on the fly as needed.
     *
     * @param unit The unit of measure for the value to be returned.
     ...
     * @see #setValue(Object)
     * @see #doubleValue()
     * @rename Renamed {@code valueList} as {@code doubleValueList} for consistency with {@link #doubleValue()}. Also
     *     because, like {@code doubleValue()}, this method returns a {@code double} value rather than a {@code Measure}
     *     object.
     */
     ...
     * {@link Boolean}, {@link String}, {@link URI}, {@code double[]} or {@code int[]}.
     *
     * <p>The argument is not restricted to the parameterized type {@code T} because the type is typically unknown (as
     * in <code>group.{@linkplain ParameterValueGroup#parameter
     * parameter}("<var>name</var>").setValue(<var>value</var>)</code>) and because some implementations may choose to
     * convert a wider range of types.
     *
     * @param value The parameter value.
