 ...
 * {@linkplain org.geotools.api.referencing.operation.Operation operation} or higher level {@code ParameterValueGroup},
...
public interface ParameterValueGroup extends GeneralParameterValue {
     ...
     *
     * @departure The ISO name was {@code "valuesOfGroup"}. GeoAPI uses {@code "descriptor"} instead in order to
    ...
    /**
     * Returns the values in this group. The returned list may or may not be unmodifiable; this is
     * implementation-dependent. However, if some aspects of this list are modifiable, then any modification shall be
     * reflected back into this {@code ParameterValueGroup}. More specifically:
     *
     * <UL>
     *   <LI>
     *       <p>If the list supports the {@link List#add(Object) add} operation, then it should ensure that the added
     *       {@linkplain GeneralParameterValue general parameter value} is valid and can be added to this group. An
     *       {@link InvalidParameterCardinalityException} (or any other appropriate exception) shall be thrown if it is
     *       not the case.
     *   <LI>
     *       <p>The list may also supports the {@link List#remove(Object) remove} operation as a way to remove parameter
     *       created by the {@link #parameter} method.
     * </UL>
     *
     ...
     * {@linkplain ParameterDescriptor#getMinimumOccurs
     * minimumOccurs} == 0</code>), then a {@linkplain ParameterValue parameter value} is automatically created and
     ...
     * </blockquote>
     *
     * This method do not search recursively in subgroups. This is because more than one subgroup may exist for the same
     * {@linkplain ParameterDescriptorGroup descriptor}. The user must {@linkplain #groups query all subgroups} and
     ...
     * optional (i.e. <code>
     * {@linkplain ParameterDescriptor#getMinimumOccurs minimumOccurs} == 0</code>) and no value were defined
     * previously, then this method returns an empty set.
     *
