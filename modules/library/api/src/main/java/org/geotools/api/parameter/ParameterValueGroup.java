 ...
 * {@linkplain org.geotools.api.referencing.operation.Operation operation} or higher level {@code
...
public interface ParameterValueGroup extends GeneralParameterValue {
     ...
     *
     * @departure The ISO name was {@code "valuesOfGroup"}. GeoAPI uses {@code "descriptor"} instead
     *     in order to override the {@linkplain GeneralParameterValue#getDescriptor generic method
     *     provided in the parent interface}. The "descriptor" name make more apparent that this
     *     method returns an abstract definition of parameters - not their actual values - and is
     *     consistent with usage in other Java libraries like {@link
     ...
     * modification shall be reflected back into this {@code ParameterValueGroup}. More
     * specifically:
     *
     * <UL>
     *   <LI>
     *       <p>If the list supports the {@link List#add(Object) add} operation, then it should
     *       ensure that the added {@linkplain GeneralParameterValue general parameter value} is
     *       valid and can be added to this group. An {@link InvalidParameterCardinalityException}
     *       (or any other appropriate exception) shall be thrown if it is not the case.
     *   <LI>
     *       <p>The list may also supports the {@link List#remove(Object) remove} operation as a way
     *       to remove parameter created by the {@link #parameter} method.
     * </UL>
     *
     ...
     * @throws ParameterNotFoundException if no {@linkplain ParameterDescriptorGroup descriptor} was
     *     found for the given name.
     * @throws IllegalStateException if this parameter group already contains the {@linkplain
