...
/**
 * Builds up complex {@linkplain CoordinateReferenceSystem coordinate reference systems} from simpler objects or values.
 * {@code CRSFactory} allows applications to make {@linkplain CoordinateReferenceSystem coordinate reference systems}
 * that cannot be created by a {@link CRSAuthorityFactory}. This factory is very flexible, whereas the authority factory
 * is easier to use.
 *
 * <p>So {@link CRSAuthorityFactory} can be used to make "standard" coordinate reference systems, and {@code CRSFactory}
 * can be used to make "special" coordinate reference systems.
 *
 * <p>For example, the EPSG authority has codes for USA state plane coordinate systems using the NAD83 datum, but these
...
public interface CRSFactory extends ObjectFactory {
    ...
    /**
     * Creates a derived coordinate reference system. If the transform is an affine map performing a rotation, then any
     * mixed axes must have identical units. For example, a (<var>lat_deg</var>, <var>lon_deg</var>,
     * <var>height_feet</var>) system can be rotated in the (<var>lat</var>, <var>lon</var>) plane, since both affected
     * axes are in degrees. But the transform should not rotate this coordinate system in any other plane.
     *
     * <p>The {@code conversionFromBase} shall contains the {@linkplain Conversion#getParameterValues parameter values}
     * required for the conversion. It may or may not contain the corresponding "{@linkplain Conversion#getMathTransform
     * base to derived}" transform, at user's choice. If a transform is provided, this method may or may not use it at
     * implementation choice. Otherwise it shall creates the transform from the parameters.
     *
     * <p>It is the user's responsability to ensure that the conversion performs all required steps, including unit
     * conversions and change of axis order, if needed. Note that this behavior is different than
     * {@link #createProjectedCRS createProjectedCRS} because transforms other than <cite>cartographic
     * projections</cite> are not standardized.
     *
     * @param properties Name and other properties to give to the new object. Available properties are
     *     {@linkplain ObjectFactory listed there}.
     * @param baseCRS Coordinate reference system to base the projection on. The number of axes must matches the
     *     {@linkplain OperationMethod#getSourceDimensions source dimensions} of the conversion from base.
     * @param conversionFromBase The {@linkplain CoordinateOperationFactory#createDefiningConversion defining
     *     conversion}.
    ...
    /**
     * Creates a projected coordinate reference system from a defining conversion. The {@code conversionFromBase} shall
     * contains the {@linkplain Conversion#getParameterValues parameter values} required for the projection. It may or
     * may not contain the corresponding "{@linkplain Conversion#getMathTransform base to derived}" transform, at user's
     * choice. If a transform is provided, this method may or may not use it at implementation choice. Otherwise it
     * shall creates the transform from the parameters.
     *
     * <p>The supplied conversion should <strong>not</strong> includes the operation steps for performing
     * {@linkplain CoordinateSystemAxis#getUnit unit} conversions and change of {@linkplain CoordinateSystem#getAxis
     * axis} order; those operations shall be inferred by this constructor by some code equivalent to:
     *
     * <blockquote>
     *
     * <code>
     ...
     * </blockquote>
     *
     * This behavior is different than {@link #createDerivedCRS createDerivedCRS} because parameterized transforms are
     * standardized for projections. See the {@linkplain MathTransformFactory#createParameterizedTransform note on
     * cartographic projections}.
     *
     * @param properties Name and other properties to give to the new object. Available properties are
     *     {@linkplain ObjectFactory listed there}.
     * @param baseCRS Geographic coordinate reference system to base the projection on. The number of axes must matches
     *     the {@linkplain OperationMethod#getSourceDimensions source dimensions} of the conversion from base.
     * @param conversionFromBase The {@linkplain CoordinateOperationFactory#createDefiningConversion defining
     *     conversion}.
    ...
    /**
     * Creates a coordinate reference system object from a string. The <A HREF="../doc-files/WKT.html">definition for
     * WKT</A> is shown using Extended Backus Naur Form (EBNF).
     *
