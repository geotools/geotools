 ...
 * </ul>
 *
 * <p>There is not just one ellipsoid. An ellipsoid is a matter of choice, and therefore many choices are possible. The
 * size and shape of an ellipsoid was traditionally chosen such that the surface of the geoid is matched as closely as
 * possible locally, e.g. in a country. A number of global best-fit ellipsoids are now available. An association of an
 * ellipsoid with the earth is made through the definition of the size and shape of the ellipsoid and the position and
 * orientation of this ellipsoid with respect to the earth. Collectively this choice is captured by the concept of
 * "{@linkplain GeodeticDatum geodetic datum}". A change of size, shape, position or orientation of an ellipsoid will
 * result in a change of geographic coordinates of a point and be described as a different geodetic datum. Conversely
 * geographic coordinates are unambiguous only when associated with a geodetic datum.
 *
 * @departure ISO 19111 defines {@link #getSemiMinorAxis semiMinorAxis}, {@link #getInverseFlattening inverseFlattening}
 *     and {@link #isIvfDefinitive isSphere} in a separated structure named {@code secondDefiningParameter} of type
 *     {@code union}. The C/C++ concept of {@code union} doesn't exist in Java, but implementors can achieve the same
 *     functionality by providing different {@code Ellipsoid} subclasses computing one parameter on-the-fly from the
 *     other one. The {@link #isIvfDefinitive isIvfDefinitive} attribute imported from OGC 01-004 can be used for
...
public interface Ellipsoid extends IdentifiedObject {
     ...
     * equatorial/polar radius by the formula
     *
     ...
     * POSITIVE_INFINITY} value is used.
     *
