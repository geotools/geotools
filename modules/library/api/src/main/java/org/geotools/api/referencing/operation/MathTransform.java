 ...
 * {@linkplain CoordinateOperation#getSourceCRS source coordinate reference system} to coordinate value for the same
 * point in the {@linkplain CoordinateOperation#getTargetCRS target coordinate reference system}.
 *
 * <p>In a {@linkplain Conversion conversion}, the transformation is accurate to within the limitations of the computer
 * making the calculations. In a {@linkplain Transformation transformation}, where some of the operational parameters
 * are derived from observations, the transformation is accurate to within the limitations of those observations.
 *
 * <p>If a client application wishes to query the source and target
 * {@linkplain org.geotools.api.referencing.crs.CoordinateReferenceSystem coordinate reference systems} of an operation,
 * then it should keep hold of the {@link CoordinateOperation} interface, and use the contained math transform object
 * whenever it wishes to perform a transform.
 *
...
public interface MathTransform {
    ...
    /**
     * Transforms the specified {@code ptSrc} and stores the result in {@code ptDst}. If {@code ptDst} is {@code null},
     * a new {@link Position} object is allocated and then the result of the transformation is stored in this object. In
     * either case, {@code ptDst}, which contains the transformed point, is returned for convenience. If {@code ptSrc}
     * and {@code ptDst} are the same object, the input point is correctly overwritten with the transformed point.
     *
     * @param ptSrc the specified coordinate point to be transformed.
     * @param ptDst the specified coordinate point that stores the result of transforming {@code ptSrc}, or
     *     {@code null}.
     * @return the coordinate point after transforming {@code ptSrc} and storing the result in {@code ptDst}, or a newly
     ...
     *     wile some other implementations will fill the untransformable points with {@linkplain Double#NaN NaN} values,
     ...
     * matrix will have size <code>N&times;M</code>. The elements of the matrix <code>{e<sub>n,m</sub> : n=0..(N-1)}
     * </code> form a vector in the output space which is parallel to the displacement caused by a small change in the
     * <var>m</var>'th ordinate in the input space.
     *
     * <p>For example, if the input dimension is 4 and the output dimension is 3, then a small displacement <code>
     * (x<sub>0</sub>,&nbsp;x<sub>1</sub>,&nbsp;x<sub>2</sub>,&nbsp;x<sub>3</sub>)</code> in the input space will result
     * in a displacement <code>
     * (y<sub>0</sub>,&nbsp;y<sub>1</sub>,&nbsp;y<sub>2</sub>)</code> in the output space computed as below (<code>
     * e<sub>n,m</sub></code> are the matrix's elements):
     *
     * <pre>
     ...
     * </pre>
     *
     * @param point The coordinate point where to evaluate the derivative. Null value is accepted only if the derivative
     *     is the same everywhere. For example affine transform accept null value since they produces identical
     *     derivative no matter the coordinate value. But most map projection will requires a non-null value.
     * @return The derivative at the specified point (never {@code null}). This method never returns an internal object:
     ...
     * HREF="../doc-files/WKT.html">defined in extended Backus Naur form</A>. This operation may fails if an object is
     * too complex for the WKT format capability.
     *
