 ...
 * never need to use this factory directly; they can use a {@linkplain CoordinateOperationFactory coordinate operation
 * factory} instead. However, the {@code MathTransformFactory} interface can be used directly by applications that wish
...
public interface MathTransformFactory extends Factory {
     ...
     * createParameterizedTransform}, or {@code null} if not applicable.
     *
     * <p>Implementors should document how their implementation behave in a multi-threads environment. For example some
     * implementations use {@linkplain java.lang.ThreadLocal thread local variables}, while other can choose to returns
     * {@code null} in all cases since this method is optional.
     *
    ...
    /**
     * Returns the default parameter values for a math transform using the given method. The {@code method} argument is
     * the name of any operation method returned by <code>
     * {@link #getAvailableMethods getAvailableMethods}({@linkplain Operation}.class)</code>. A typical example is
     * <code>
     * "<A HREF="http://www.remotesensing.org/geotiff/proj_list/transverse_mercator.html">Transverse_Mercator</A>"
     * </code>).
     *
     * <p>The {@linkplain ParameterDescriptorGroup#getName parameter group name} shall be the method name, or an alias
     * to be understood by <code>{@linkplain #createParameterizedTransform
     * createParameterizedTransform}(parameters)</code>. This method creates new parameter instances at every call.
     * Parameters are intented to be modified by the user before to be given to the above-cited
     * {@code createParameterizedTransform} method.
     *
    ...
    /**
     * Creates a {@linkplain #createParameterizedTransform parameterized transform} from a base CRS to a derived CS.
     * This convenience method {@linkplain #createConcatenatedTransform concatenates} the parameterized transform with
     * any other transform required for performing units changes and ordinates swapping, as described in the
     * {@linkplain #createParameterizedTransform note on cartographic projections}.
     *
     * <p>In addition, implementations are encouraged to infer the {@code "semi_major"} and {@code "semi_minor"}
     * parameter values from the {@linkplain Ellipsoid ellipsoid}, if they are not explicitly given.
     *
     ...
     *
     * <p>Cartographic projection transforms are used by {@linkplain ProjectedCRS projected coordinate reference
     * systems} to map geographic coordinates (e.g. <var>longitude</var> and <var>latitude</var>) into
     * (<var>x</var>,<var>y</var>) coordinates. These (<var>x</var>,<var>y</var>) coordinates can be imagined to lie on
     * a plane, such as a paper map or a screen. All cartographic projection transforms created through this method will
     * have the following properties:
     *
     * <UL>
     *   <LI>Converts from (<var>longitude</var>,<var>latitude</var>) coordinates to (<var>x</var>,<var>y</var>).
     ...
     * </UL>
     *
     * <p>Although all cartographic projection transforms must have the properties listed above, many projected
     * coordinate reference systems have different properties. For example, in Europe some projected CRSs use grads
     * instead of degrees, and often the {@linkplain ProjectedCRS#getBaseCRS base geographic CRS} is
     * (<var>latitude</var>, <var>longitude</var>) instead of (<var>longitude</var>, <var>latitude</var>). This means
     * that the cartographic projected transform is often used as a single step in a series of transforms, where the
     * other steps change units and swap ordinates.
     *
     ...
     * operate on a subset of ordinates. For example giving (<var>latitude</var>, <var>longitude</var>,
     * <var>height</var>) coordinates, a pass through transform can convert the height values from meters to feet
     * without affecting the (<var>latitude</var>, <var>longitude</var>) values.
     *
    ...
    /**
     * Creates a math transform object from a string. The <A HREF="../doc-files/WKT.html">definition for WKT</A> is
