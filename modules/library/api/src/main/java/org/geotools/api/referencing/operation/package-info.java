...
/**
 * {@linkplain org.geotools.api.referencing.operation.CoordinateOperation Coordinate operations}
 * (relationship between any two {@linkplain
 * org.geotools.api.referencing.crs.CoordinateReferenceSystem coordinate reference systems}). The
 ...
 * the concept of {@linkplain org.geotools.api.referencing.crs.DerivedCRS Derived CRS}.
 *
 * <P ALIGN="justify">The best-known example of this source-derived relationship is a {@linkplain
 * org.geotools.api.referencing.crs.ProjectedCRS projected coordinate reference system}, which is
 * always based on a source {@linkplain org.geotools.api.referencing.crs.GeographicCRS geographic
 * coordinate reference system}. The associated map projection effectively defines the projected
 * coordinate reference system from the geographic coordinate system.
 *
 ...
 * org.geotools.api.referencing.operation.CoordinateOperation} and {@link
 * org.geotools.api.referencing.crs.CoordinateReferenceSystem}. This would exclude coordinate
 ...
 * are defined in the {@link org.geotools.api.referencing.crs.GeneralDerivedCRS}, by specifying the
 * base (i.e., source) CRS and the defining conversion. The derived coordinate reference system
 * itself is the target CRS in this situation. When used in a concatenated operation, the
 * conversion's source and target coordinate reference system are equally subject to the above
 * constraint as the source and target of a transformation although they are specified in a
 * different manner.
 *
 ...
 * {@linkplain org.geotools.api.referencing.operation.OperationMethod operation method}.
 * Concatenated operations and pass-through operations do not require a coordinate operation to be
 * specified. Each {@linkplain org.geotools.api.referencing.operation.OperationMethod operation
 * method} uses a number of {@linkplain org.geotools.api.parameter.ParameterValue parameters}
 * (although some coordinate conversions use none), and each {@linkplain
 * org.geotools.api.referencing.operation.CoordinateOperation coordinate operation} assigns value to
 * these parameters.
 *
