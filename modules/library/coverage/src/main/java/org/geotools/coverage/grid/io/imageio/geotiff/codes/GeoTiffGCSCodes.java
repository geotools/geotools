...
/**
 * 6.3.2.1 Geographic CS Type Codes Note: A Geographic coordinate system consists of both a datum
 * and a Prime Meridian. Some of the names are very similar, and differ only in the Prime Meridian,
 * so be sure to use the correct one. The codes beginning with GCSE_xxx are unspecified GCS which
 * use ellipsoid (xxx); it is recommended that only the codes beginning with GCS_ be used if
 * possible. Ranges: 0 = undefined [ 1, 1000] = Obsolete EPSG/POSC Geographic Codes [ 1001, 3999] =
 * Reserved by GeoTIFFWritingUtilities [ 4000, 4199] = EPSG GCS Based on Ellipsoid only [ 4200,
 * 4999] = EPSG GCS Based on EPSG Datum [ 5000, 32766] = Reserved by GeoTIFFWritingUtilities 32767 =
