 ...
 * default value is nearest neighbor. The new interpolation type operates on all sample dimensions. Possible values for
 * type are: {@code "NearestNeighbor"}, {@code "Bilinear"} and {@code "Bicubic"} (the {@code "Optimal"} interpolation
 ...
 * This behavior is convenient when processing remote sensing images of geophysics data, for example <cite>Sea Surface
 * Temperature</cite> (SST), in which clouds may mask many pixels (i.e. set them to some {@code NaN} values). Because
 * {@code "Bicubic"} interpolation needs 4&times;4 pixels while {@code "Bilinear"} interpolation needs only 2x2 pixels,
 * the {@code "Bilinear"} interpolation is less likely to fails because of clouds ({@code NaN} values) than the
 * {@code "Bicubic"} one (note: only one {@code NaN} value is enough to make an interpolation fails). One can workaround
 * the problem by trying a bicubic interpolation first, then a linear interpolation if {@code "Bicubic"} failed at a
 * particular location, <cite>etc.</cite> This behavior can be specified with the following {@code "Type"} argument:
 * <code>new String[]{"Bicubic", "Bilinear", "NearestNeighbor"}
 * </code>.
 *
...
public class Interpolate extends Operation2D {
     ...
     * {@link org.geotools.coverage.processing.DefaultProcessor} for the {@code "Interpolate"} operation.
     */
    @Override
