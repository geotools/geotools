 ...
 * also an {@link javax.media.jai.Interpolation} argument type, for interoperability with <A
 * HREF="http://java.sun.com/products/java-media/jai/">Java Advanced Imaging</A>. Second, it accepts
 * also an array of {@link java.lang.String} or {@link javax.media.jai.Interpolation} objects. When
 * an array is specified, the first interpolation in the array is applied. If this interpolation
 * returns a {@code NaN} value, then the second interpolation is tried as a fallback. If the second
 * interpolation returns also a {@code NaN} value, then the third one is tried and so on until an
 ...
 * less likely to fails because of clouds ({@code NaN} values) than the {@code "Bicubic"} one (note:
 * only one {@code NaN} value is enough to make an interpolation fails). One can workaround the
 * problem by trying a bicubic interpolation first, then a linear interpolation if {@code "Bicubic"}
 * failed at a particular location, <cite>etc.</cite> This behavior can be specified with the
 * following {@code "Type"} argument: <code>new String[]{"Bicubic", "Bilinear", "NearestNeighbor"}
 * </code>.
 *
