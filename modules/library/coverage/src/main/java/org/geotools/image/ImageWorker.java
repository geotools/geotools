...
public class ImageWorker {
    ...
    public final static String USE_JAI_SCALE2_KEY = "it.geosolutions.jaiext.scale2";

    public final static boolean JAIEXT_ENABLED;

    public final static boolean USE_JAI_SCALE2;
    ...
    /** 
     * Update the ROI by extracting it from the current image
     * using  the underlying property generator (when specified)
     * 
     * @param forceUpdate update the ROI no matter if 
     *          the original roi was is null.
     * @param opName if not null, get the property from the underlying
     *          property generator
     */
    private void updateROI(boolean forceUpdate, String opName) {
        ...
        if (forceUpdate || roi != null) {
            ...
            if (opName != null) {
                ...
                PropertyGenerator gen = getOperationDescriptor(opName)
    ...
    private double[] getScalingParams(ParameterBlock paramBlock, boolean isScale2) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            scalingParams[i] = isScale2 ? paramBlock.getDoubleParameter(i)
                    : paramBlock.getFloatParameter(i); 
        }
...
//                invalidateStatistics();
     ...
     */
    public ImageWorker scale(double xScale, double yScale,
            double xTrans, double yTrans, Interpolation interp) {
        ParameterBlock pb = new ParameterBlock();
        pb.setSource(image, 0); // The source image.
