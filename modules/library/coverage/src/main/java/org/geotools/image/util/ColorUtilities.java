 ...
 * but not really rigorous. This is why they do not appear in any "official" package, but instead in
 * this private one.
 *
...
public final class ColorUtilities {
     ...
     * @throws IllegalArgumentException if {@coder}, {@code g}, {@code b} or {@code a} are outside
    ...
    /**
     * Returns an index color model for specified ARGB codes. If the specified array has not
     * transparent color (i.e. all alpha values are 255), then the returned color model will be
     * opaque. Otherwise, if the specified array has one and only one color with alpha value of 0,
     * the returned color model will have only this transparent color. Otherwise, the returned color
     * model will be translucent.
     *
    ...
    /**
     * Returns a suggered type for an {@link IndexColorModel} of {@code mapSize} colors. This method
     * returns {@link DataBuffer#TYPE_BYTE} or {@link DataBuffer#TYPE_USHORT}.
     */
    ...
    /**
     * Transforms a color from XYZ color space to LAB. The color are transformed in place. This
     * method returns {@code color} for convenience. Reference:
    ...
    /**
     * Tells us if a specific {@link IndexColorModel} contains only gray color or not, ignoring
     * alpha information.
     *
     * @param icm {@link IndexColorModel} to be inspected.
     * @param ignoreTransparents {@code true} if the RGB values of fully transparent pixels (the
     *     ones with an {@linkplain IndexColorModel#getAlpha(int) alpha} value of 0) should not be
     *     taken in account during the check for gray color.
            ...
            default:
                throw new IllegalArgumentException(
                        MessageFormat.format(
                                "Illegal argument: \"{0}={1}\".", "DataType unknown:", dataType));
        }
    }
    ...
    public static int findColorIndex(Color bgColor, IndexColorModel icm) {
        if (bgColor == null)
            throw new NullPointerException(
                    (MessageFormat.format("Argument \"{0}\" should not be null.", "bgColor")));
        if (icm == null)
            throw new NullPointerException(
                    (MessageFormat.format("Argument \"{0}\" should not be null.", "icm")));
