...
public class DefaultFeatureResultsTest extends TestCase {
    ...
    public void testMaxFeatureOptimized() throws Exception {
        ...
        SimpleFeatureSource fs = EasyMock.createMock(SimpleFeatureSource.class);
        EasyMock.expect(fs.getSchema()).andReturn(type).anyTimes();
        EasyMock.expect(fs.getCount(q)).andReturn(20);
        replay(fs);
    ...
    public void testMaxfeaturesHandCount() throws Exception {
        ...
        EasyMock.expect(ds.getFeatureReader(q, Transaction.AUTO_COMMIT)).andReturn(fr);
        replay(ds);
        ...
        SimpleFeatureSource fs = EasyMock.createMock(SimpleFeatureSource.class);
        EasyMock.expect(fs.getSchema()).andReturn(type).anyTimes();
        EasyMock.expect(fs.getCount(q)).andReturn(-1);
        EasyMock.expect(fs.getDataStore()).andReturn(ds);
        replay(fs);
