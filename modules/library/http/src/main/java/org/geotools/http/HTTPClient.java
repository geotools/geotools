...
public interface HTTPClient {
    ...
    /**
     * Executes an HTTP POST request against the provided URL, sending the contents of {@code postContent} as the POST
     * method body and setting the Content-Type request header to {@code postContentType} if given, and returns the
     * server response.
     *
     * <p>If an HTTP authentication {@link #getUser() user} and {@link #getPassword() password} is set, the appropriate
     * authentication HTTP header will be sent with the request.
     *
     * <p>If a {@link #getConnectTimeout() connection timeout} is set, the http connection will be set to respect that
     * timeout.
     *
     * <p>If a {@link #getReadTimeout() read timeout} is set, the http connection will be set to respect it.
     *
