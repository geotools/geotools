...
public class AbstractHttpClientTest {
    ...
    @Test
    public void testNoAuthority() throws MalformedURLException {
        // a URL with no authority must not gain an empty "//" one: with a jar URL that would turn the nested
        // "file:/tmp" into a host
        assertEquals(
                "jar:file:/tmp/citt%C3%A0.jar!/x",
                AbstractHttpClient.encodeURL(new URL("jar:file:/tmp/città.jar!/x"))
                        .toExternalForm());
    }
