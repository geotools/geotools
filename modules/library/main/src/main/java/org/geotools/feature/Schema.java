...
/**
 * This class contains utility methods focused on the schema represented by the FeatureType data
 * model.
 *
 * <p>These methods are often used for implementation the convience methods such as
 * FeatureType.getAttributeCount(), although they may be used directly with any FeatureType.
 *
 * <p>These schema methods are based on the *complete* picture indicated by a FeatureType and its
 * ancestors. Many of these methods are focused on the derivation of AttribtueTypes during an
 * override.
 *
 * @see FeatureTypes
 * @see FeatureType
...
public class Schema {
     ...
     *
     * <p>used to detect duplicate attributes names (ie override)
     *
     * @param featureType
     */
     ...
     *
     * <p>This method does not produce the complete schema (ie derrived restrictions based on
     * attribute facets). It is only used to get a list of the unique attribtues in the resulting
     * schema.
     *
     * @param featureType
    ...
    public Filter getRestrictions(SimpleFeatureType featureType, String name) {
        ...
        List restrictions =
                restriction(featureType, name, Collections.singletonList(Filter.INCLUDE));
     ...
     * Look up based on name in the provided position.
     *
     * <p>AttributeType needs a xpath based access
     *
     * @param type
     * @param xpath
     */
    ...
    private AttributeDescriptor override(AttributeDescriptor type, AttributeDescriptor override) {
        ...
        List restrictions =
                override(type.getType().getRestrictions(), override.getType().getRestrictions());
        ...
        return new AttributeDescriptorImpl(
                new AttributeTypeImpl(
                        new NameImpl(name), javaType, false, false, restrictions, null, null),
                new NameImpl(name),
                min,
                max,
                isNilable,
