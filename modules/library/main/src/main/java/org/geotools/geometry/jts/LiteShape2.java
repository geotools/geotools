 ...
 * coordinate cloning.
 *
 * <p>This implementation supports the use of MathTransform and can be constructed with a Decimation stratagy object
 * (allowing you to fine tune the process by which a Geometry is simplified into a Shape).
 *
 * <p>This implementation is very careful about cloning; and has the ability to go faster when you are using a
 * LiteCoordinateSequenceFactory behind your geometry classes.
 *
 * @author Jesse Eichar
 * @version $Id$
 */
public final class LiteShape2 implements Shape, Cloneable {
     ...
     * @param mathTransform - the transformation applied to the geometry in order to get to the shape points
     * @param decimator -
     ...
     * @param decimator -
     * @param generalize - set to true if the geometry need to be generalized during rendering
     * @param clone - if clone is false the original geometry may be modified directly, if true it will be cloned to
     ...
     * </code> contains the <code>Rectangle2D</code>. The <code>Area</code> class can be used to perform more accurate
     ...
     * that lie inside the rectangular area must lie within the <code>Shape
     ...
     *
     * This means that this method might return <code>false</code> even though the <code>Shape
     * </code> contains the rectangular area. The <code>Area</code> class can be used to perform more accurate
     * computations of geometric intersection for any <code>Shape</code> object if a more precise answer is required.
     *
     ...
     *     intersects</code> method returns <code>true
     ...
     * getBounds
     * </code> method and never fails due to overflow problems since the return value can be an instance of the <code>
     * Rectangle2D</code> that uses double precision values to store the dimensions.
     *
     ...
     * <code>Shape</code> object independently from any other <code>PathIterator
     ...
     * </code> parameter, which specifies the maximum distance that any point on the unflattened transformed curve can
     * deviate from the returned flattened path segments. Note that a limit on the accuracy of the flattened path might
     * be silently imposed, causing very small flattening parameters to be treated as larger values. This limit, if
     ...
     * computations of geometric intersection for any <code>Shape
