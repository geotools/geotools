...
/**
 * Provide an abstract base class for ContrastMethodStrategies which hold the actual implementations of the optional
 * methods to carry out contrast enhancements of rasters when rendering.
 *
 * <p>Subclasses must provide a {@link ContrastMethod} in method that represents the type of operation and a NAME that
 * matches that method.
 *
 * <p>This class provides all the required methods and functionality. Subclasses should override methods only if they
 * wish to check for specific algorithms or sets of parameter names. They may also provide suitable default values if
 * necessary.
 *
 * @author Ian Turton
 */
...
public abstract class AbstractContrastMethodStrategy implements ContrastMethodStrategy {
    ...
    /**
     * A parameter to be used by this method. Subclasses can implement checks for valid parameter names by overriding
     * this method.
     *
    ...
    /**
     * The algorithm to be used by this method. Subclasses can implement checks for valid algorithms by overriding this
     * method.
     *
