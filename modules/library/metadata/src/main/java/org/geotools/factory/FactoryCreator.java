...
public class FactoryCreator extends FactoryRegistry {
    ...
    /**
     * Returns a provider for the specified category, using the specified map of hints (if any). If
     * a provider matching the requirements is found in the registry, it is returned. Otherwise, a
     * new provider is created and returned. This creation step is the only difference between this
     * method and the {@linkplain FactoryRegistry#getServiceProvider super-class method}.
     *
        ...
        for (final Iterator<T> it = getUnfilteredProviders(category); it.hasNext(); ) {
            ...
            try {
                ...
                Logging.recoverableException(
                        LOGGER, FactoryCreator.class, "getServiceProvider", exception);
                continue;
     ...
     * #getServiceProvider} that it need to search for an other implementation. This is needed for
     * avoiding infinite recursivity when a factory is a wrapper around an ther factory of the same
     * category. For example this is the case of {@link
