 ...
 * {@linkplain MetadataStandard metadata standard}. This class uses {@linkplain java.lang.reflect
 * Java reflection} in order to provide default implementation of {@linkplain
...
public abstract class AbstractMetadata {
     ...
     * {@code source} metadata must implements the same metadata interface (defined by the
     * {@linkplain #getStandard standard}) than this class, but don't need to be the same
    ...
    /**
     * Returns {@code true} if this metadata is modifiable. The default implementation uses
     * heuristic rules which return {@code false} if and only if:
     *
     * <p>
     *
     * <ul>
     *   <li>this class do not contains any {@code set*(...)} method
     *   <li>All {@code get*()} methods return a presumed immutable object. The maining of
     *       "<cite>presumed immutable</cite>" may vary in different Geotools versions.
     * </ul>
     *
     * <p>Otherwise, this method conservatively returns {@code true}. Subclasses should override
     * this method if they can provide a more rigorous analysis.
     */
    ...
    /**
     * Returns a view of this metadata as a tree. Note that while {@link TreeModel} is defined in
     * the {@link javax.swing.tree} package, it can be seen as a data structure independent of
    ...
    /**
     * Computes a hash code value for this metadata using Java reflection. The hash code is defined
     * as the sum of hash code values of all non-null properties. This is the same contract than
     * {@link java.util.Set#hashCode} and ensure that the hash code value is insensitive to the
