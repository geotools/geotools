 ...
 * frequently added elements are first, and most frequently added ones are last. If some elements
 * were added the same amount of time, then the iterator will traverse them in their insertion
 * order.
 *
 * <p>An optional boolean argument in the constructor allows the construction of set in reversed
 * order (most frequently added elements first, less frequently added last). This is similar but not
 * identical to creating a defaut {@code FrequencySortedSet} and iterating through it in reverse
 ...
 */
public class FrequencySortedSet<E> extends AbstractSet<E>
     ...
     * ordinary hash map because we want to preserve insertion order for elements that occur at the
     * same frequency.
     */
    private final Map<E, Integer> count;

    /**
     * {@code +1} if the element should be sorted in the usual order, or {@code -1} if the elements
     * should be sorted in reverse order (most frequent element first).
     */
    ...
    /**
     * The frequency for each {@linkplain #sorted} element. This array is invalid if {@link #sorted}
     * is null.
     */
     ...
     *
     * @param reversed {@code true} if the elements should be sorted in reverse order (most frequent
        ...
        if (occurence != 0) {
            if (occurence < 0) {
                throw new IllegalArgumentException(
                        Errors.format(ErrorKeys.NOT_GREATER_THAN_ZERO_$1, occurence));
            }
            sorted = null;
            occurence *= order;
    ...
    /**
     * The comparator used for sorting map entries. Most be consistent with {@link #compare}
     * implementation.
     */
     ...
     * FrequencySortedSet}, the comparator is always {@code this}.
     *
    ...
    /**
     * Compares the specified elements for {@linkplain #frequency frequency}. For {@code
     * FrequencySortedSet} with default ordering, this method returns a positive number if {@code
     * o1} has been added more frequently to this set than {@code o2}, a negative number if {@code
     * o1} has been added less frequently than {@code o2}, and 0 otherwise. For {@code
     * FrequencySortedSet} with reverse ordering, this is the converse.
     *
     * <p>This method is final because the {@code FrequencySortedSet} implementation makes
     * assumptions on the comparator that would not hold if this method were overrided.
     */
    ...
    /**
     * Returns the frequency of the specified element in this set. Returns a negative number if this
     * set has been created for reversed order.
     */
