...
public final class LRULinkedHashMap<K, V> extends LinkedHashMap<K, V> {
    ...
    /**
     * Constructs a {@code LRULinkedHashMap} with default initial capacity, maximum size and load
     * factor.
     */
     ...
     * @param initialCapacity The initial capacity.
     * @param loadFactor The load factor.
     * @param accessOrder The ordering mode: {@code true} for access-order, {@code false} for
     *     insertion-order.
     */
    public LRULinkedHashMap(
    ...
    /**
     * For private usage by static convenience constructors only. The {@code maximumSize} must be
     * checked before this construcor is invoked, because it is used in the calculation of the value
     * given to {@code super} constructor call.
     */
    private LRULinkedHashMap(final boolean accessOrder, final int maximumSize) {
        super(
                (int) Math.ceil(maximumSize / DEFAULT_LOAD_FACTOR),
    ...
    /**
     * Constructs a {@code LRULinkedHashMap} with all entries from the specified map. The
     * {@linkplain #getMaximumSize maximum size} is set to the given {@linkplain Map#size map size}.
     *
    ...
    private static void checkMaximumSize(final int maximumSize) throws IllegalArgumentException {
        if (maximumSize <= 0) {
            throw new IllegalArgumentException(
                    Errors.format(ErrorKeys.NOT_GREATER_THAN_ZERO_$1, maximumSize));
        }
    }
    ...
    /**
     * Creates a map for the most recently accessed entries. This convenience constructor uses an
     * initial capacity big enough for holding the given maximum number of entries, so it is not
     * appropriate if {@code maximumSize} is very large.
     *
     * @param <K> The type of keys in the map.
     * @param <V> The type of values in the map.
     * @param maximumSize Maximum number of entries for this LRU map.
    ...
    /**
     * Creates a map for the most recently inserted entries. This convenience constructor uses an
     * initial capacity big enough for holding the given maximum number of entries, so it is not
     * appropriate if {@code maximumSize} is very large.
     *
     * @param <K> The type of keys in the map.
    ...
    /**
     * Returns the maximal {@linkplain #size size} allowed for this map. This is the value given to
     * constructors.
     *
