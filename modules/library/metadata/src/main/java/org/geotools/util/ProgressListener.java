...
/**
 * Monitor the progress of some lengthly operation. This interface makes no assumption about the
 * output device. It may be the standard output stream (see {@link
 * org.geotools.gui.headless.ProgressPrinter} implementation), a window ({@link
 * org.geotools.gui.swing.ProgressWindow}) or mails automatically sent to some address ({@link
 * org.geotools.gui.headless.ProgressMailer}). Additionnaly, this interface provides support for
 * non-fatal warning and exception reports.
 *
 * <p>All {@code ProgressListener} implementations are multi-thread safe, even the
 * <cite>Swing</cite> implemention. {@code ProgressListener} can be invoked from any thread, which
 * never need to be the <cite>Swing</cite>'s thread. This is usefull for performing lenghtly
 ...
 * </blockquote>
 *
 * <strong>Note:</strong> The line <code>if ((j&nbsp;&amp;&nbsp;255)&nbsp;==&nbsp;0)</code> is used
 * for reducing the amount of calls to {@link #progress} (only once every 256 steps). This is not
 * mandatory, but may speed up the process.
 *
 * <p>Here is another example showing how to cancel:
 *
 * <pre><code>
...
public interface ProgressListener extends org.opengis.util.ProgressListener {
    ...
    /**
     * Set the description for the lenghtly operation to be reported. This method is usually invoked
     ...
     * 100% or disaspears, at implementor choice. If warning messages were pending, they will be
     * displayed now.
     */
    void complete();

    /**
     * Release any resources used by this listener. If the progress were reported in a window, this
