 ...
 * {@linkplain #SoftValueHashMap(int) construction time}.
 * <p>
...
public class SoftValueHashMap<K, V> extends AbstractMap<K, V> {
    ...
    public V get(final Object key) {
        ...
        if (value instanceof Reference) {
            /*
             * The value is a soft reference only if it was not used for a while and the map contains more than 'hardReferenceCount' entries.
             * Otherwise, it is an ordinary reference and is returned directly. See the 'retainStrongly' method.
             *
             * If the value is a soft reference, get the referent and clear it immediately for avoiding the reference to be enqueued. We abandon the
             * soft reference and reinject the referent as a strong reference in the hash map, since we try to keep the last entries by strong
             * references.
             */
            value = ((Reference) value).getAndClear();
            if (value != null) {
                /*
                 * Transforms the soft reference into a hard one. The cast should be safe because hash.get(key) should not have returned a non-null
                 * value if the key wasn't valid.
                 */
        ...
        /*
         * The safety of this cast depends only on this implementation, not on users. It should be safe if there is no bug in the way this class
         * manages 'hash'.
         */
        @SuppressWarnings("unchecked")
        final V v = (V) value;
    ...
    /**
     * Declares that the value for the specified key must be retained by hard reference. If there is already {@link #hardReferencesCount} hard
     * references, then this method replaces the oldest hard reference by a soft one.
     */
    ...
    public V put(final K key, final V value) {
        ...
        if (oldValue instanceof Reference) {
            ...
            /*
             * The value was retained by hard reference, which implies that the key must be in the hard-cache list. Removes the key from the list,
             * since we want to reinsert it at the beginning in order to mark the value as the most recently used. This method performs a linear
             * search, which may be quite inefficient. But it still efficient enough if the key was recently used, in which case it appears near the
             * beginning of the list. We assume that this is a common case. We may revisit later if profiling show that this is a performance issue.
             */
    ...
    /**
     * A soft reference to a map entry. Soft references are created only when the map contains more than {@link #hardReferencesCount}, in order to
     * avoid to put more pressure on the garbage collector.
     */
    private static final class Reference<K, V> extends SoftReference<V> {
        /**
         * A reference to the {@link SoftValueHashMap#hash} entries. We keep this reference instead than a reference to {@link SoftValueHashMap}
         * itself in order to avoid indirect retention of {@link SoftValueHashMap#hardCache}, which is not needed for this reference.
         */
        private final Map<K, Object> hash;
         ...
         * </ul>
         * This is because this method is invoked when the entry should have already be removed, or is about to be removed.</li>
         */
        ...
        /**
         * Removes the entries from the backing hash map. This method need to override the {@link SoftReference#clear} method because it is invoked by
         * {@link WeakCollectionCleaner}.
         */
        @Override
        public void clear() {
            if (cleaner != null) {
            ...
            synchronized (hash) {
                ...
                /*
                 * If the entry was used for an other value, then put back the old value. This case may occurs if a new value was set in the hash map
                 * before the old value was garbage collected.
                 */
