...
/**
 * Simple operations on arrays. This class provides a central place for inserting and deleting elements in an array, as
 * well as resizing the array. This class may be removed if JavaSoft provide some language construct functionally
 * equivalent to C/C++'s {@code realloc}.
 *
 * @since 2.0
 * @version $Id$
 * @author Martin Desruisseaux (IRD)
 * @todo Replace all {@code resize} methods by {@code Arrays.copyOf} when we will be allowed to compile for Java 6.
 */
public final class XArray {
    ...
    /**
     * Returns an array containing the same elements as the given {@code array} but with the specified {@code length},
     * truncating or padding with {@code null} if necessary.
     *
     * <ul>
     *   <li>
     *       <p>If the given {@code length} is longer than the length of the given {@code array}, then the returned
     *       array will contain all the elements of {@code array} at index <var>i</var> &lt; {@code array.length}.
     *       Elements at index <var>i</var> &gt;= {@code array.length} are initialized to {@code null}.
     *   <li>
     *       <p>If the given {@code length} is shorter than the length of the given {@code array}, then the returned
     *       array will contain only the elements of {@code array} at index <var>i</var> &lt; {@code length}. Remainding
     *       elements are not copied.
     *   <li>
     *       <p>If the given {@code length} is equals to the length of the given {@code array}, then {@code array} is
     *       returned unchanged. <strong>No copy</strong> is performed. This behavior is what make this method different
     *       than {@link Arrays#copyOf}.
     *
     * @param  <T> The array elements.
     * @param array Array to copy.
     * @param length Length of the desired array.
     * @return A new array of the requested length, or {@code array} if the original array already have the requested
     *     length.
    ...
    /**
     * Returns an array containing the same elements as the given {@code array} but specified {@code length}, truncating
     * or padding with zeros if necessary.
     *
     * @param array Array to copy.
     * @param length Length of the desired array.
     * @return A new array of the requested length, or {@code array} if the original array already have the requested
     ...
     * @return Array with the same elements than the given {@code array} except for the removed elements, or
    ...
    /**
     * Inserts spaces into the middle of an array. These "spaces" will be made up of elements initialized to
     * {@code null}.
     *
     * @param array Array in which to insert spaces.
     * @param index Index where the first space should be inserted. All {@code array} elements having an index equal to
     *     or higher than {@code index} will be moved forward.
     * @param length Number of spaces to insert.
     * @return Array containing the {@code array} elements with the additional space inserted, or {@code array} if
     *     {@code length} is 0.
     */
    ...
    /**
     * Inserts a table slice into another table. The {@code src} table will be entirely or partially inserted into the
     * {@code dst} table.
     *
     ...
     * @param dst_pos {@code dst} index in which to insert {@code src} data. All elements of {@code dst} whose index is
     *     equal to or greater than {@code dst_pos} will be moved forward.
     * @param length Number of {@code src} data items to insert.
     * @return Table which contains the combination of {@code src} and {@code dst}. This method can directly return
     *     {@code dst}, but never {@code src}. It most often returns a newly created table.
     */
    ...
    /**
     * Returns {@code true} if all elements in the specified array are in increasing order. This method is usefull in
     * assertions.
     */
    ...
    /**
     * Returns {@code true} if all elements in the specified array are in increasing order. Since {@code NaN} values are
     * unordered, they may appears anywhere in the array; they will be ignored. This method is usefull in assertions.
     */
    ...
    /**
     * Returns {@code true} if all elements in the specified array are in strictly increasing order. This method is
     * usefull in assertions.
     */
    ...
    /**
     * Returns {@code true} if all values in the specified array are equals to the specified value, which may be
     * {@link Float#NaN}.
     */
    ...
    /**
     * Returns {@code true} if the specified array contains the specified value, ignoring case. This method should be
     * used only for very small arrays.
     *
    ...
    /**
     * Returns {@code true} if the specified array contains the specified value. This method should be used only for
     * very small arrays, or for search to be performed only once, because it performs a linear search. If more than one
     * search need to be done on the same array, consider using {@link java.util.HashSet} instead.
     *
    ...
    /**
     * Returns {@code true} if at least one element in the first array is {@linkplain Object#equals equals} to an
     * element in the second array. The element doesn't need to be at the same index in both array.
     *
     * <p>This method should be used only for very small arrays since it may be very slow. If the arrays are large or if
     * an array will be involved in more than one search, consider using {@link java.util.HashSet} instead.
     */
    ...
    /**
     * Returns a string representation of an array of numbers. Current implementation supports only primitive or
