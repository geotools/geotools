...
/**
 * Interface for classes which provide the ability to retrieve the delegate instance when the instance in question is in
 * fact a proxy/decorator class.
 *
 * <p>Developers may wish to gain access to the resources that are wrapped (the delegates) as proxy class instances
 * representing the the actual resources.
 *
 * <p>This interface extends JDK Wrapper class, overriding the thrown exception with a more meaningful named one.
 */
public interface Wrapper extends java.sql.Wrapper {
     ...
     * low-cost operation compared to <code>unwrap</code> so that callers can use this method to avoid expensive <code>
