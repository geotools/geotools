 ...
 *       constructors. They are the hints to be returned by {@link #getImplementationHints}.
 *   <li>An automatic {@linkplain ServiceRegistry#setOrdering ordering} applied on the basis of
 *       subclasses-provided {@linkplain #priority} rank.
 * </ul>
 *
 * <p>When more than one factory implementation is {@linkplain
 * ServiceRegistry#registerServiceProvider registered} for the same category (i.e. they implement
 * the same {@link Factory} sub-interface), the actual instance to be used is selected according
 * their {@linkplain ServiceRegistry#setOrdering ordering} and user-supplied {@linkplain Hints
 ...
 * This implies that the ordering is unspecified between all factories created with the {@linkplain
 * #AbstractFactory() default constructor}, since they all have the same {@linkplain
 * #NORMAL_PRIORITY default priority} level.
 *
 ...
 * pass them to any dependencies. Do <strong>not</strong> use the {@link #hints} field for that; use
 * the hints provided by the user in the constructor. If all dependencies are created at
 * construction time (<cite>constructor injection</cite>), there is no need to keep user's hints
 * once the construction is finished.
 *
 * <p>The {@link #hints} field is for the second case only. Implementations shall copy in this field
 * only the user's hints that are know to be relevant to this factory. If a hint is relevant but the
 * user didn't specified any value, the hint key should be added to the {@link #hints} map anyway
 * with a {@code null} value. Only direct dependencies shall be put in the {@link #hints} map.
 * Indirect dependencies (i.e. hints used by other factories used by this factory) will be inspected
 * automatically by {@link FactoryRegistry} in a recursive way.
 *
 * <p><strong>Note:</strong> The lack of constructor expecting a {@link Map} argument is
 * intentional. This is in order to discourage blind-copy of all user-supplied hints to the {@link
 * #hints} map.
 *
 * <p><strong>Example:</strong> Lets two factories, A and B. Factory A need an instance of Factory
...
public class AbstractFactory implements Factory, RegistrableFactory {
    /**
     * The minimum priority for a factory, which is {@value}. Factories with lowest priority will be
     * used only if there is no other factory in the same {@linkplain ServiceRegistry#getCategories
     * category}.
     *
     * @see #priority
     * @see #onRegistration
    ...
    /**
     * The maximum priority for a factory, which is {@value}. Factories with highest priority will
     * be preferred to any other factory in the same {@linkplain ServiceRegistry#getCategories
     * category}.
     *
     * @see #priority
     * @see #onRegistration
    ...
    /**
     * The priority for this factory, as a number between {@link #MINIMUM_PRIORITY} and {@link
     * #MAXIMUM_PRIORITY} inclusive. Priorities are used by {@link FactoryRegistry} for selecting a
     * preferred factory when many are found for the same service.
     *
     * @see #getPriority
     * @todo Consider deprecating this field. See <A
     *     HREF="http://jira.codehaus.org/browse/GEOT-1100">GEOT-1100</A> for details.
     */
     ...
     *
     * <ul>
     *   <li>The primary use of this map is to check if this factory can be reused. It is not for
     *       creating new factories.
     *   <li>This map needs to allow {@code null} values, as of {@linkplain
     ...
     *     and {@link #MAXIMUM_PRIORITY} inclusive.
     */
     ...
     * implementation returns {@link #priority} with no change. Subclasses should override this
     * method if they want to return a higher or lower priority.
     *
     * @since 2.3
     */
    ...
    /**
     * Called when this factory is removed from the given {@code category} of the given {@code
     * registry}. The object may still be registered under another category or categories.
     *
     * <p>This method is invoked automatically when this factory is no longer registered as a
     * plugin, and should not be invoked directly by the user.
     *
    ...
    /**
     * Returns a hash value for this factory. The default implementation computes the hash value
     * using only immutable properties. This computation do <strong>not</strong> relies on
     * {@linkplain #getImplementationHints implementation hints}, since there is no garantee that
    ...
    /**
     * Returns a string representation of the specified hints. This is used by {@link
