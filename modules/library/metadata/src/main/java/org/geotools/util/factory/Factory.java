 ...
 *
 * <p>This interfaces forms the core of the Geotools plug-in system, by which capabilities can be
 * added to the library at runtime. Each sub-interface defines a <cite>service</cite>. Most services
 * are set up with concrete implementation being registered for use in a <cite>service
 * registry</cite>, which acts as a container for service implementations.
 *
 * <p>Service registries don't need to be a Geotools implementation. They can be (but are not
 * limited to) any {@link javax.imageio.spi.ServiceRegistry} subclass. If the standard {@code
 * ServiceRegistry} (or its Geotools extension {@link FactoryRegistry}) is selected as a container
 ...
 *         <li>Add the fully qualified name of the <u>implementation</u> class in the {@code
 *             META-INF/services/}<var>classname</var> file where <var>classname</var> is the fully
 *             qualified name of the service <u>interface</u>.
 *         <li>The factory implementations will be discovered when {@link
 ...
 * </ul>
 *
 * <p>In addition, it is recommended that implementations provide a constructor expecting a single
 * {@link Hints} argument. This optional argument gives to the user some control of the factory's
 * low-level details. The amount of control is factory specific. The geotools library defines a
 * global class called {@link Hints} that is ment as API (i.e. you can assume these hints are
 * supported). Factories may also provide information on their own custom hints as part of their
 * javadoc class description. <strong>Examples:</strong>
 *
 * <ul>
 *   <li>
 ...
 * </ul>
 *
 * <p>As seen in those examples this concept of a hint becomes more interesting when the operation
 * being controlled is discovery of other services used by the Factory. By supplying appropriate
 * hints one can chain together several factories and retarget them to an application specific task.
 *
