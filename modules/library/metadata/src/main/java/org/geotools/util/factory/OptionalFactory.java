 ...
 *
 * <p>Such factories often need some external resources. For example the default {@linkplain
 * org.geotools.referencing.factory.epsg.AccessDialectEpsgFactory EPSG factory} need a MS-Access
 * database installed on the client machine. This database is not bundle in Geotools distribution;
 * if the user have not installed it, the factory can't work.
 *
 * <p>This interface is <strong>not</strong> a manager for automatic download of external resources.
 * It is just a way to tell to {@link FactoryFinder} that this factory exists, but can't do its job
 * for whatever reasons (usually a missing resource that the user shall download and install
 * himself), so {@code FactoryFinder} has to choose an other factory. In other words, the {@code
 ...
 * </ul>
 *
 * <p><strong>NOTE:</strong> {@code OptionalFactory} is not designed for factories with intermittent
...
public interface OptionalFactory extends Factory {
    /**
     * Returns {@code true} if this factory is ready for use. An optional factory may returns {@code
     * false} for now but returns {@code true} later. However, the converse is not recommended.
     */
    boolean isAvailable();
}
