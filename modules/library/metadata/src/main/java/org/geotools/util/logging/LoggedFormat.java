 ...
 * class provides a {@link #parse} method which performs the following tasks:
 *
 * <p>
 *
 * <ul>
 *   <li>Checks if the string was fully parsed and log a warning if it was not. This is different
 *       than the default {@link #parseObject(String)} behavior which check only if the
 *       <em>begining</em> of the string was parsed and ignore any remaining characters.
 *   <li>Ensures that the parsed object is of some specific class specified at construction time.
 *   <li>If the string can't be fully parsed or is not of the expected class, logs a warning.
 * </ul>
 *
 * @since 2.4
 * @version $Id$
...
public class LoggedFormat<T> extends Format {
    ...
    /**
     * Sets the logger level for the warnings eventually emitted by the {@link #parse} method. The
     * default value is {@link Level#WARNING}.
     *
    ...
    /**
     * Sets the {@linkplain LogRecord#setSourceClassName source class name} and {@linkplain
     * LogRecord#setSourceMethodName source method name} for the warnings eventually emitted by the
     * {@link #parse} method.
     *
     ...
     * {@linkplain Format format} specified at construction time. This method to not perform any
     * logging.
     *
     ...
     * except that the result is returned as a {@link String} rather than a {@link LogRecord}. This
     * is provided as a convenience method for creating the message to give to an {@linkplain
     * Exception#Exception(String) exception constructor}.
     *
     ...
     * @since 2.5
     */
    public static String formatUnparsable(
            final String text, final int index, final int errorIndex, final Locale locale) {
    ...
    /**
     * Implementation of {@code formatUnparsable} methods. Returns a {@link LogRecord} if {@code
     * level} is non-null, or a {@link String} otherwise.
     */
    private static Object doFormatUnparsable(
            String text, final int index, int errorIndex, final Locale locale, final Level level) {
    ...
    public String toString() {
        final StringBuilder buffer =
                new StringBuilder(Classes.getShortClassName(this))
                        .append('[')
                        .append(Classes.getShortClassName(format));
        if (logger != null) {
