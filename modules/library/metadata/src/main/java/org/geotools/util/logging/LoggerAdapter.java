...
/**
 * An adapter that redirect all Java logging events to an other logging framework. This class
 * redefines the {@link #severe(String) severe}, {@link #warning(String) warning}, {@link
 * #info(String) info}, {@link #config(String) config}, {@link #fine(String) fine}, {@link
 * #finer(String) finer} and {@link #finest(String) finest} methods as <em>abstract</em> ones.
 * Subclasses should implement those methods in order to map Java logging levels to the backend
 * logging framework.
 *
 * <p>All {@link #log(Level,String) log} methods are overriden in order to redirect to one of the
 * above-cited methods. Note that this is the opposite approach than the Java logging framework one,
 * which implemented everything on top of {@link Logger#log(LogRecord)}. This adapter is defined in
 * terms of {@link #severe(String) severe} &hellip; {@link #finest(String) finest} methods instead
 * because external frameworks like <a href="http://commons.apache.org/logging/">Commons-logging</a>
 ...
 * every configuration methods inherited from {@link Logger} are disabled:
 *
 * <p>
 *
 * <ul>
 *   <li>{@link #addHandler} since the handling is performed by the external framework.
 *   <li>{@link #setUseParentHandlers} since this adapter never delegates to the parent handlers.
 *       This is consistent with the previous item and avoid mixing loggings from the external
 *       framework with Java loggings.
 *   <li>{@link #setParent} since this adapter should not inherits any configuration from a parent
 *       logger using the Java logging framework.
 *   <li>{@link #setFilter} for keeping this {@code LoggerAdapter} simple.
 * </ul>
 *
 * <p>Since {@code LoggerAdapter}s do not hold any configuration by themself, it is not strictly
 * necessary to {@linkplain java.util.logging.LogManager#addLogger add them to the log manager}. The
 ...
 * If a log record {@linkplain Level level} is not one of the predefined ones, then this class maps
...
public abstract class LoggerAdapter extends Logger {
    ...
    /**
     * Sets the level for this logger. Subclasses must redirect the call to the external logging
     * framework, or do nothing if the level can not be changed programmatically.
     */
    @Override
    public abstract void setLevel(Level level);

    /**
     * Returns the level for this logger. Subclasses shall get this level from the external logging
     * framework.
     */
    @Override
    ...
    /**
     * Returns the level for {@link #entering}, {@link #exiting} and {@link #throwing} methods. The
     * default implementation returns {@link Level#FINER}, which is consistent with the value used
    ...
    /**
     * Logs a method entry to the {@linkplain #getDebugLevel debug level}. Compared to the default
     * {@link Logger}, this implementation bypass the level check in order to let the backing
     * logging framework do its own check.
     */
    @Override
    ...
    /**
     * Logs a method return to the {@linkplain #getDebugLevel debug level}. Compared to the default
     * {@link Logger}, this implementation bypass the level check in order to let the backing
     * logging framework do its own check.
     */
    @Override
    ...
    /**
     * Logs a method failure to the {@linkplain #getDebugLevel debug level}. Compared to the default
     * {@link Logger}, this implementation bypass the level check in order to let the backing
     * logging framework do its own check.
     */
    @Override
    ...
    /**
     * Logs a record at the specified level. The default implementation discards the exception and
     * delegates to <code>{@linkplain #log(Level,String) log}(level, message)</code>.
     */
    @Override
     ...
     * {@linkplain #log(Level,String,Object[]) log}(level, message, params)</code> where the {@code
     * params} array is built from the {@code param} object.
     */
    @Override
     ...
     * immediately, then delegates to <code>{@linkplain #log(Level,String) log}(level, message)
     * </code>.
     */
    @Override
    ...
    /**
     * Logs a record at the specified level. The defaut implementation discards the source class and
     * source method, then delegates to <code>{@linkplain #log(Level,String) log}(level, message)
     * </code>.
     */
    @Override
    public void logp(
    ...
    /**
     * Logs a record at the specified level. The defaut implementation discards the source class and
     * source method, then delegates to <code>
     ...
     * sourceMethod, message, params)</code> where the {@code params} array is built from the {@code
     * param} object.
     *
    ...
    /**
     * Do nothing since this logger adapter does not supports handlers. The configuration should be
     * fully controlled by the external logging framework (e.g. <a
     * href="http://commons.apache.org/logging/">Commons-logging</a>) instead, which is not expected
     * to use {@link Handler} objects.
     */
    @Override
    ...
    /**
     * Do nothing since this logger never use parent handlers. This is consistent with {@link
     * #addHandler} not allowing to add any handlers, and avoid mixing loggings from the external
     * framework with Java loggings.
     */
    @Override
    ...
    /**
     * Do nothing since this logger adapter does not support arbitrary parents. More specifically,
     * it should not inherits any configuration from a parent logger using the Java logging
     * framework.
     */
    @Override
     ...
     * efficiently the filter in this {@code LoggerAdapter} architecture (e.g. we would need to make
     * sure that {@link Filter#isLoggable} is invoked only once even if a {@code log} call is
     * cascaded into many other {@code log} calls, and this test must works in multi-threads
     * environment).
     */
    @Override
    ...
    /**
     * Wraps the specified object in an array. This is a helper method for {@code log(..., Object)}
     * methods that delegate their work to {@code log(..., Object[])}
     */
    ...
    /**
     * Localize the specified message. This is a helper method for {@code logrb(...)} methods that
     * delegate their work to {@code logp(...)}
     */
