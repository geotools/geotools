 ...
 * simpler objects or values. {@code CRSFactory} allows applications to make {@linkplain
 * CoordinateReferenceSystem coordinate reference systems} that cannot be created by a {@link
 * CRSAuthorityFactory}. This factory is very flexible, whereas the authority factory is easier to
 * use.
 *
 * <p>So {@link CRSAuthorityFactory} can be used to make "standard" coordinate reference systems,
 * and {@code CRSFactory} can be used to make "special" coordinate reference systems.
 *
 * <p>For example, the EPSG authority has codes for USA state plane coordinate systems using the
 * NAD83 datum, but these coordinate systems always use meters. EPSG does not have codes for NAD83
...
public interface CRSFactory extends ObjectFactory {
    ...
    /**
     * Creates a coordinate reference system object from a string. The <A
     * HREF="../doc-files/WKT.html">definition for WKT</A> is shown using Extended Backus Naur Form
     * (EBNF).
     *
