...
/**
 * Class that implements simple, type safe enumerations in Java. Two enumeration values are equal if
 * {@code e1 == e2} or if {@code e1.getValue() == e2.getValue()}.
 *
...
public abstract class SimpleEnumerationType<E extends SimpleEnumerationType<E>>
    ...
    /**
     * Load an icon for this enumeration. {@code loadIconResource} loads the icon found by {@code
     * a_class.getResource(name)}.
     *
     * @param a_class class to use to find icon resource.
     * @param name name of icon resource, for example, "SOLID.gif"
     * @return a loaded icon for this enumeration. When no icon can be found the null icon is
     *     returned.
     * @see #getNullIcon
     */
     ...
     * Gets the null icon.
     *
     * @return a singleton icon that can be used when no icon was found in {@code loadIconResource}.
     *     The null icon is a fixed size, 16x64, and its paint method simply returns.
     */
    ...
    /**
     * Add a new {@code SimpleEnumerationType} to the given list using the given {@code name} and
     * {@code description}. The icon will be set to the Null Icon.
     *
     ...
     */
    protected SimpleEnumerationType(
            Collection<E> values, String name, String description, Icon icon) {
        super(name, values);
        this.description = description;
        this.icon = icon;
    }
    ...
    /**
     * A icon that can be used when no icon was found in load icon. The null icon is a fix 16x64 and
     * its paint method simply returns.
     */
