 ...
 * ambiguity as two {@linkplain Position direct positions} (coordinate points). To encode an {@code Envelope}, it is
 * sufficient to encode these two points.
 *
 * <p>This particular implementation of {@code Envelope} is said "General" because it uses coordinates of an arbitrary
 * dimension.
 *
 * <p><strong>Tip:</strong> The metadata package provides a
 * {@link org.geotools.api.metadata.extent.GeographicBoundingBox}, which can be used as a kind of envelope with a
 * coordinate reference system fixed to WGS 84 (EPSG:4326).
 *
 * @since 2.0
...
public class GeneralBounds extends AbstractBounds implements Cloneable, Serializable {
     ...
     * maximum ordinates. This layout is convenient for the creation of lower and upper corner direct positions.
     *
    ...
    /**
     * Constructs an empty envelope of the specified dimension. All ordinates are initialized to 0 and the coordinate
     ...
     * @throws IllegalArgumentException if an ordinate value in the minimum point is not less than or equal to the
    ...
    /**
     * Constructs a envelope defined by two positions. The coordinate reference system is inferred from the supplied
     * direct position.
     *
    ...
    /**
     * Converts the envelope to a general envelope, avoiding the construction of a new object in case the input envelope
     ...
     * system is set to {@linkplain DefaultGeographicCRS#WGS84 WGS84}.
     *
    ...
    /**
     * Constructs two-dimensional envelope defined by a {@link Rectangle2D}. The coordinate reference system is
     * initially undefined.
     *
    ...
    /**
     * Creates an envelope for a grid range transformed to an envelope using the specified math transform. The
     * <cite>grid to CRS</cite> transform should map either the {@linkplain PixelInCell#CELL_CENTER cell center} (as in
     * OGC convention) or {@linkplain PixelInCell#CELL_CORNER cell corner} (as in Java2D/JAI convention) depending on
     * the {@code anchor} value. This constructor creates an envelope containing entirely all pixels on a <cite>best
     * effort</cite> basis - usually accurate for affine transforms.
     *
     * <p><b>Note:</b> The convention is specified as a {@link PixelInCell} code instead than the more detailled
     * {@link org.geotools.api.metadata.spatial.PixelOrientation}, because the later is restricted to the
     ...
     * @throws MismatchedDimensionException If one of the supplied object doesn't have a dimension compatible with the
     *     other objects.
     * @throws IllegalArgumentException if an argument is illegal for some other reason, including failure to use the
     *     provided math transform.
     * @since 2.3
     * @see org.geotools.coverage.grid.GeneralGridEnvelope#GeneralGridEnvelope(Bounds,PixelInCell,boolean)
     */
    ...
    /**
     * Checks if ordinate values in the minimum point are less than or equal to the corresponding ordinate value in the
     * maximum point.
     *
    ...
    /**
     * Sets the coordinate reference system in which the coordinate are given. This method <strong>do not</strong>
     * reproject the envelope, and do not check if the envelope is contained in the new domain of validity. The later
     * can be enforced by a call to {@link #normalize}.
     *
    ...
    /**
     * Restricts this envelope to the CS or CRS {@linkplain CoordinateReferenceSystem#getDomainOfValidity domain of
     * validity}. This method performs two steps:
     *
     * <ol>
     *   <li>
     *       <p>First, it ensures that the envelope is contained in the {@linkplain CoordinateSystem coordinate system}
     *       domain. Out of range ordinates are validated in a way that depends on the
     *       {@linkplain CoordinateSystemAxis#getRangeMeaning range meaning}:
     *       <ul>
     *         <li>If {@linkplain RangeMeaning#EXACT EXACT} (typically <em>latitudes</em> ordinates), values greater
     *             than the {@linkplain CoordinateSystemAxis#getMaximumValue maximum value} are replaced by the maximum,
     *             and values smaller than the {@linkplain CoordinateSystemAxis#getMinimumValue minimum value} are
     *             replaced by the minimum.
     *         <li>If {@linkplain RangeMeaning#WRAPAROUND WRAPAROUND} (typically <em>longitudes</em> ordinates), a
     *             multiple of the range (e.g. 360° for longitudes) is added or subtracted. If a value stay out of range
     *             after this correction, then the ordinates are set to the full [
     *             {@linkplain CoordinateSystemAxis#getMinimumValue minimum} ...
     *             {@linkplain CoordinateSystemAxis#getMaximumValue maximum}] range.
     *             <blockquote>
     *             <b>Example:</b> [185° ... 190°] of longitude is equivalent to [-175° ... -170°]. But [175° ... 185°]
     *             would be equivalent to [175° ... -175°], which is likely to mislead most users of {@link Bounds}
     *             since the lower bounds is numerically greater than the upper bounds. Reordering as [-175° ... 175°]
     *             would interchange the meaning of what is "inside" and "outside" the envelope. So this implementation
     *             conservatively expands the range to [-180° ... 180°] in order to ensure that the validated envelope
     *             fully contains the original envelope.
     *             </blockquote>
     *       </ul>
     *   <li>
     *       <p>Second and only if {@code crsDomain} is {@code true}, the envelope normalized in the previous step is
     *       intersected with the CRS {@linkplain CoordinateReferenceSystem#getDomainOfValidity domain of validity}, if
     *       any.
     * </ol>
     *
     * @param crsDomain {@code true} if the envelope should be restricted to the CRS domain in addition of the CS
     *     domain.
     * @return {@code true} if this envelope has been modified, or {@code false} if no change was done.
     * @since 2.5
     */
    ...
    /**
     * A coordinate position consisting of all the {@linkplain #getMinimum minimal ordinates} for each dimension for all
    ...
    /**
     * A coordinate position consisting of all the {@linkplain #getMaximum maximal ordinates} for each dimension for all
     * points within the {@code Envelope}.
     *
    ...
    /**
     * A coordinate position consisting of all the {@linkplain #getCenter(int) middle ordinates} for each dimension for
     * all points within the {@code Envelope}.
     *
     ...
     * @throws IllegalArgumentException if the target units are not compatible with the units of the provided
     *     {@code dimension}
     * @since 2.5
     */
     ...
     * coordinates. The number of arguments provided shall be twice this {@linkplain #getDimension envelope dimension},
     * and minimum shall not be greater than maximum.
     *
     ...
     * @throws MismatchedDimensionException if the specified envelope doesn't have the expected number of dimensions.
     * @since 2.2
     */
    ...
    /**
     * Sets the lower corner to {@linkplain Double#NEGATIVE_INFINITY negative infinity} and the upper corner to
     * {@linkplain Double#POSITIVE_INFINITY positive infinity}. The {@linkplain #getCoordinateReferenceSystem coordinate
     * reference system} (if any) stay unchanged.
     *
     * @since 2.2
     */
    ...
    /**
     * Returns {@code true} if at least one ordinate has an {@linkplain Double#isInfinite infinite} value.
     *
    ...
    /**
     * Sets all ordinate values to {@linkplain Double#NaN NaN}. The {@linkplain #getCoordinateReferenceSystem coordinate
     * reference system} (if any) stay unchanged.
     *
     * @since 2.2
     */
    ...
    /**
     * Returns {@code false} if at least one ordinate value is not {@linkplain Double#NaN NaN}. The {@code isNull()}
     * check is a little bit different than {@link #isEmpty()} since it returns {@code false} for a partially
     * initialized envelope, while {@code isEmpty()} returns {@code false} only after all dimensions have been
     * initialized. More specifically, the following rules apply:
     *
     * <p>
     *
     * <ul>
    ...
    /**
     * Determines whether or not this envelope is empty. An envelope is non-empty only if it has at least one
     * {@linkplain #getDimension dimension}, and the {@linkplain #getLength length} is greater than 0 along all
     * dimensions. Note that a non-empty envelope is always non- {@linkplain #isNull null}, but the converse is not
     * always true.
     *
    ...
    /**
     * Returns {@code true} if at least one of the specified CRS is null, or both CRS are equals. This special
     * processing for {@code null} values is different from the usual contract of an {@code equals} method, but allow to
     * handle the case where the CRS is unknown.
     */
    ...
    /**
     * Adds a position to this bounds. The resulting bounds is the smallest bounds that contains both the original
     * bounds and the specified position. After adding a position, a call to {@link #contains} with the added position
     * as an argument will return {@code true}, except if one of the position's ordinates was {@link Double#NaN} (in
     * which case the corresponding ordinate have been ignored).
     *
     * <p>This method assumes that the specified position uses the same CRS than this envelope. For performance reason,
     * it will not be verified unless J2SE assertions are enabled.
     *
     ...
     * which case the corresponding ordinate have been ignored).
     *
     * <p>This method position that the specified position uses the same CRS than this envelope. For performance reason,
     * it will not be verified unless J2SE assertions are enabled.
     *
    ...
    /**
     * Adds an envelope object to this envelope. The resulting envelope is the union of the two {@code Envelope}
     * objects.
     *
     * <p>This method assumes that the specified envelope uses the same CRS than this envelope. For performance reason,
     * it will no be verified unless J2SE assertions are enabled.
     *
     ...
     *
     * <p>This method assumes that the specified point uses the same CRS than this envelope. For performance reason, it
     * will no be verified unless J2SE assertions are enabled.
     *
     * @param position The point to text.
     * @return {@code true} if the specified coordinates are inside the boundary of this envelope; {@code false}
     *     otherwise.
     * @throws MismatchedDimensionException if the specified point doesn't have the expected dimension.
     */
    ...
    /**
     * Returns {@code true} if this envelope completly encloses the specified envelope. If one or more edges from the
     * specified envelope coincide with an edge from this envelope, then this method returns {@code true} only if
     * {@code edgesInclusive} is {@code true}.
     *
     * <p>This method assumes that the specified envelope uses the same CRS than this envelope. For performance reason,
     * it will no be verified unless J2SE assertions are enabled.
     *
    ...
    /**
     * Returns {@code true} if this envelope intersects the specified envelope. If one or more edges from the specified
     * envelope coincide with an edge from this envelope, then this method returns {@code true} only if
     * {@code edgesInclusive} is {@code true}.
     *
     * <p>This method assumes that the specified envelope uses the same CRS than this envelope. For performance reason,
     * it will no be verified unless J2SE assertions are enabled.
     ...
     * ordinates into a new envelope, beginning at dimension <code>lower</code> and extending to dimension <code>upper-1
     * </code>. Thus the dimension of the subenvelope is <code>
     * upper-lower</code>.
     *
    ...
    /**
     * Returns a {@link Rectangle2D} with the same bounds as this {@code Envelope}. This is a convenience method for
    ...
    /**
     * Compares to the specified envelope for equality up to the specified tolerance value. The tolerance value
     * {@code eps} can be either relative to the {@linkplain #getLength envelope length} along each dimension or can be
     * an absolute value (as for example some ground resolution of a
     * {@linkplain org.geotools.api.coverage.grid.GridCoverage grid coverage}).
     *
     * <p>If {@code relativeToLength} is set to {@code true}, the actual tolerance value for a given dimension
     * <var>i</var> is {@code eps}&times;{@code length} where {@code length} is the maximum of {@linkplain #getLength
     * this envelope length} and the specified envelope length along dimension <var>i</var>.
     *
     * <p>If {@code relativeToLength} is set to {@code false}, the actual tolerance value for a given dimension
     * <var>i</var> is {@code eps}.
     *
     * <p>Relative tolerance value (as opposed to absolute tolerance value) help to workaround the fact that tolerance
     * value are CRS dependent. For example the tolerance value need to be smaller for geographic CRS than for UTM
     * projections, because the former typically has a range of -180 to 180° while the later can have a range of
     * thousands of meters.
     *
     * <p>This method assumes that the specified envelope uses the same CRS than this envelope. For performance reason,
     * it will no be verified unless J2SE assertions are enabled.
     ...
     * @param epsIsRelative {@code true} if the tolerance value should be relative to axis length, or {@code false} if
     *     it is an absolute value.
     * @return {@code true} if the given object is equals to this envelope up to the given tolerance value.
     * @see #contains(Bounds, boolean)
     * @see #intersects(Bounds, boolean)
     * @since 2.4
     */
