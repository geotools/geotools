...
public class DefaultEngineeringCRS extends AbstractSingleCRS implements EngineeringCRS {
    ...
    /**
     * A two-dimensional cartesian coordinate reference system with {@linkplain DefaultCoordinateSystemAxis#X x},
     * {@linkplain DefaultCoordinateSystemAxis#Y y} axis in {@linkplain SI#METER metres}. By default, this CRS has no
     * transformation path to any other CRS (i.e. a map using this CS can't be reprojected to a
    ...
    /**
     * A three-dimensional cartesian coordinate reference system with {@linkplain DefaultCoordinateSystemAxis#X x},
     * {@linkplain DefaultCoordinateSystemAxis#Y y}, {@linkplain DefaultCoordinateSystemAxis#Z z} axis in
     * {@linkplain SI#METER metres}. By default, this CRS has no transformation path to any other CRS (i.e. a map using
     * this CS can't be reprojected to a {@linkplain DefaultGeographicCRS geographic coordinate reference system} for
     * example).
     */
    ...
    /**
     * A two-dimensional wildcard coordinate system with {@linkplain DefaultCoordinateSystemAxis#X x},
     * {@linkplain DefaultCoordinateSystemAxis#Y y} axis in {@linkplain SI#METER metres}. At the difference of
     * {@link #CARTESIAN_2D}, this coordinate system is treated specially by the default
     * {@linkplain org.geotools.referencing.operation.DefaultCoordinateOperationFactory coordinate operation factory}
     * with loose transformation rules: if no transformation path were found (for example through a
     * {@linkplain DefaultDerivedCRS derived CRS}), then the transformation from this CRS to any CRS with a compatible
     * number of dimensions is assumed to be the identity transform. This CRS is usefull as a kind of wildcard when no
    ...
    /**
     * A three-dimensional wildcard coordinate system with {@linkplain DefaultCoordinateSystemAxis#X x},
     * {@linkplain DefaultCoordinateSystemAxis#Y y}, {@linkplain DefaultCoordinateSystemAxis#Z z} axis in
     * {@linkplain SI#METER metres}. At the difference of {@link #CARTESIAN_3D}, this coordinate system is treated
     * specially by the default {@linkplain org.geotools.referencing.operation.DefaultCoordinateOperationFactory
     * coordinate operation factory} with loose transformation rules: if no transformation path were found (for example
     * through a {@linkplain DefaultDerivedCRS derived CRS}), then the transformation from this CRS to any CRS with a
     * compatible number of dimensions is assumed to be the identity transform. This CRS is usefull as a kind of
     * wildcard when no CRS were explicitly specified.
     */
    ...
    /**
     * Constructs a new enginnering CRS with the same values than the specified one. This copy constructor provides a
    ...
    public DefaultEngineeringCRS(final EngineeringCRS crs) {
        super(crs);
        if (crs instanceof DefaultEngineeringCRS rS) {
    ...
    /**
     * Compares the specified object to this CRS for equality. This method is overridden because, otherwise,
     * {@code CARTESIAN_xD} and {@code GENERIC_xD} would be considered equals when metadata are ignored.
     */
    @Override
    ...
    /**
     * Returns true if this is a wildcard CRS, that is, one that will transform from and to any other CRS using the
