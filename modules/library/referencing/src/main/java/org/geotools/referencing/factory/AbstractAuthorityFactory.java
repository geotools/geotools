...
/**
 * Base class for authority factories. An <cite>authority</cite> is an organization that maintains definitions of
 * authority codes. An <cite>authority code</cite> is a compact string defined by an authority to reference a particular
 * spatial reference object. For example the <A HREF="http://www.epsg.org">European Petroleum Survey Group (EPSG)</A>
 * maintains a database of coordinate systems, and other spatial referencing objects, where each object has a code
 * number ID. For example, the EPSG code for a WGS84 Lat/Lon coordinate system is {@code "4326"}.
 *
 * <p>This class defines a default implementation for most methods defined in the {@link DatumAuthorityFactory},
 * {@link CSAuthorityFactory} and {@link CRSAuthorityFactory} interfaces. However, those interfaces do not appear in the
 * {@code implements} clause of this class declaration. This is up to subclasses to decide which interfaces they declare
 * to implement.
 *
 * <p>The default implementation for all {@code createFoo} methods ultimately invokes {@link #createObject}, which may
...
public abstract class AbstractAuthorityFactory extends ReferencingFactory implements AuthorityFactory {
    ...
    /**
     * Returns {@code true} if this factory is available. This method may returns {@code false} for example if a
     * connection to the EPSG database failed. This method is defined here for implementation convenience, but not yet
     * public because not yet applicable. It will be made public in {@link DeferredAuthorityFactory} and
     * {@link AuthorityFactoryAdapter} subclasses, which implement the {@link org.geotools.util.factory.OptionalFactory}
     * interface.
     */
    boolean isAvailable() {
        return true;
     ...
     * {@linkplain #getAuthorityCodes authority codes}, returns {@code true}. Example of such wrappers are
     * {@link BufferedAuthorityFactory} and {@link TransformedAuthorityFactory}.
     *
     * <p>This method should perform a cheap test. It is for {@link FallbackAuthorityFactory} internal use only and
    ...
    /**
     * Returns a description of the underlying backing store, or {@code null} if unknown. This is for example the
     * database software used for storing the data. The default implementation returns always {@code null}.
     *
     ...
     * {@link CoordinateSystem}, {@link CoordinateReferenceSystem} or {@link CoordinateOperation}. The default
     * implementation always throw an exception. Subclasses should override this method if they are capable to
     * automatically detect the object type from its code.
     *
    ...
    /**
     * Returns an arbitrary {@linkplain Datum datum} from a code. The default implementation invokes <code>
    ...
    /**
     * Creates a {@linkplain EngineeringDatum engineering datum} from a code. The default implementation invokes <code>
     * {@linkplain #createDatum createDatum}(code)</code>.
     *
     * @param code Value allocated by authority.
     * @return The datum for the given code.
     * @throws NoSuchAuthorityCodeException if the specified {@code code} was not found.
    ...
    /**
     * Creates a {@linkplain ImageDatum image datum} from a code. The default implementation invokes <code>
     * {@linkplain #createDatum createDatum}(code)</code>.
     *
     * @param code Value allocated by authority.
     * @return The datum for the given code.
    ...
    /**
     * Creates a {@linkplain VerticalDatum vertical datum} from a code. The default implementation invokes <code>
     * {@linkplain #createDatum createDatum}(code)</code>.
     *
     * @param code Value allocated by authority.
     * @return The datum for the given code.
    ...
    /**
     * Creates a {@linkplain TemporalDatum temporal datum} from a code. The default implementation invokes <code>
     * {@linkplain #createDatum createDatum}(code)</code>.
     *
     * @param code Value allocated by authority.
     * @return The datum for the given code.
    ...
    /**
     * Returns a {@linkplain GeodeticDatum geodetic datum} from a code. The default implementation invokes <code>
     * {@linkplain #createDatum createDatum}(code)</code>.
     *
     * @param code Value allocated by authority.
     * @return The datum for the given code.
    ...
    /**
     * Returns an {@linkplain Ellipsoid ellipsoid} from a code. The default implementation invokes <code>
     * {@linkplain #createObject createObject}(code)</code>.
     *
     * @param code Value allocated by authority.
    ...
    /**
     * Returns a {@linkplain PrimeMeridian prime meridian} from a code. The default implementation invokes <code>
     * {@linkplain #createObject createObject}(code)</code>.
     *
     * @param code Value allocated by authority.
    ...
    /**
     * Returns a {@linkplain Extent extent} (usually an area of validity) from a code. The default implementation
    ...
    /**
     * Returns an arbitrary {@linkplain CoordinateSystem coordinate system} from a code. The default implementation
     * invokes <code>{@linkplain #createObject createObject}(code)</code>.
     *
     * @param code Value allocated by authority.
    ...
    /**
     * Returns a {@linkplain CoordinateSystemAxis coordinate system axis} from a code. The default implementation
     * invokes <code>{@linkplain #createObject createObject}(code)</code>.
     *
     * @param code Value allocated by authority.
    ...
    /**
     * Creates an operation from coordinate reference system codes. The default implementation returns an
     * {@linkplain Collections.emptySet() empty set}. We do not delegate to some kind of
     * {@linkplain CoordinateOperationFactory#createOperation(CoordinateReferenceSystem, CoordinateReferenceSystem)
     * coordinate operation factory method} because the usual contract for this method is to extract the information
     * from an authority database like <A HREF="http://www.epsg.org">EPSG</A>, not to compute operations on-the-fly.
     *
     * <p><strong>Rational:</strong> Coordinate operation factory
     * {@linkplain org.geotools.referencing.operation.AuthorityBackedFactory backed by an authority} will invoke this
     * method. If this method invoked the coordinate operation factory in turn, the application could be trapped in
    ...
    /**
     * Returns a finder which can be used for looking up unidentified objects. The finder fetchs a fully
     * {@linkplain IdentifiedObject identified object} from an incomplete one, for example from an object without
     * identifier or "{@code AUTHORITY[...]}" element in <cite>Well Known Text</cite> terminology.
     *
     * @param type The type of objects to look for. Should be a GeoAPI interface like {@code GeographicCRS.class}, but
     *     this method accepts also implementation class. If the type is unknown, use {@code IdentifiedObject.class}. A
     *     more accurate type may help to speed up the search, since it reduces the amount of tables to scan in some
    ...
    /**
     * Releases resources immediately instead of waiting for the garbage collector. Once a factory has been disposed,
     * further {@code create(...)} invocations may throw a {@link FactoryException}. Disposing a previously-disposed
     * factory, however, has no effect.
     *
    ...
    /**
     * Creates an exception for an unknow authority code. This convenience method is provided for implementation of
