...
/**
 * A buffered authority factory which will defer the {@linkplain #createBackingStore creation of a backing store} until
 * when first needed. This approach allow to establish a connection to a database (for example) only when first needed.
 * In addition, the backing store can be automatically disposed after a timeout and recreated when needed again.
 *
 * @since 2.1
 * @version $Id$
 * @author Martin Desruisseaux (IRD)
 * @todo Extends {@link BufferedAuthorityFactory} for now in order to improve the trunk stability during GEOT-1286
 *     development, but we may revisit that after GEOT-1286 completion.
 */
public abstract class DeferredAuthorityFactory extends BufferedAuthorityFactory implements OptionalFactory {
    ...
    /**
     * The task for disposing the backing store, or {@code null} if none. This task will be scheduled for repeated
     * execution by {@link #setTimeout}.
     */
    private TimerTask disposer;

    /**
     * {@code true} if the backing store was used since the last time the timer task was run. A value of {@code true}
     * means that the task must wait again. A value of {@code false} means that it can dispose the backing store.
     */
    private boolean used;

    /**
     * Constructs an instance without initial backing store. Subclasses are responsible for creating an appropriate
     * backing store when the {@link #createBackingStore} method is invoked.
     *
    ...
    /**
     * Returns {@code true} if this factory is available. The default implementation returns {@code false} if
     * {@link #createBackingStore} throws an exception.
     */
    @Override
    ...
    /**
     * Creates the backing store authority factory. This method is invoked the first time a {@code createXXX(...)}
     * method is invoked.
     *
     * @return The backing store to uses in {@code createXXX(...)} methods.
    ...
    /**
     * Returns {@code true} if this deferred factory is connected to its backing store. This method returns
     * {@code false} if no {@code createFoo} method has been invoked, if the backing store has been automatically
     * disposed after the {@linkplain #setTimeout timeout} or if this factoy has been {@linkplain #dispose disposed}.
     */
    ...
    /**
     * Set a timer for disposing the backing store after the specified amount of milliseconds of inactivity. The
     * {@link #createBackingStore} method will be responsible for creating a new backing store when needed. Note that
     * the backing store disposal can be vetoed if {@link #canDisposeBackingStore} returns {@code false}.
     *
     * @param delay The minimal delay before to close the backing store. This delay is very approximative. The backing
     *     store will not be closed before, but may take as much as twice that time before to be closed.
     */
     ...
     * amount of time specified by {@link #setTimeout} if the factory were not used during that time. The default
     * implementation always returns {@code true}. Subclasses should override this method and returns {@code false} if
     * they want to prevent the backing store disposal under some circonstances.
     *
    ...
    /**
     * Releases resources immediately instead of waiting for the garbage collector. This method disposes the backing
     * store regardeless of {@link #canDisposeBackingStore} value.
     */
    @Override
