...
/**
 * Default implementation for a coordinate reference system authority factory backed by a property file. This gives some
 * of the benificts of using the {@linkplain org.geotools.referencing.factory.epsg.DirectEpsgFactory EPSG database
 * backed authority factory} (for example), in a portable property file.
 *
...
public class PropertyAuthorityFactory extends DirectAuthorityFactory
    ...
    /**
     * The properties object for our properties file. Keys are the authority code for a coordinate reference system and
     * the associated value is a WKT string for the CRS.
     *
     * <p>It is technically possible to add or remove elements after they have been loaded by the constructor. However
     * if such modification are made, then we should update {@link Hints#FORCE_LONGITUDE_FIRST_AXIS_ORDER} accordingly.
     * It may be an issue since hints are supposed to be immutable after factory construction. For now, this class do
     * not allow addition of elements.
     */
    ...
    /**
     * Creates a factory for the specified authorities from the specified file. More than one authority may be specified
     * when the CRS to create should have more than one {@linkplain CoordinateReferenceSystem#getIdentifiers
     * identifier}, each with the same code but different namespace. For example a
     * {@linkplain org.geotools.referencing.factory.epsg.EsriExtension factory for CRS defined by ESRI} uses the
     * {@code "ESRI"} namespace, but also the {@code "EPSG"} namespace because those CRS are used as extension of the
     * EPSG database. Concequently, the same CRS can be identified as {@code "ESRI:53001"} and {@code "EPSG:53001"},
     * where {@code "53001"} is a unused code in the official EPSG database.
     *
     * @param factories The underlying factories used for objects creation.
     * @param authorities The organizations or party responsible for definition and maintenance of the database.
     * @param definitions URL to the definition file.
     * @throws IOException if the definitions can't be read.
     * @since 2.4
     */
     ...
     *       {@link #createGeographicCRS createGeographicCRS}, {@link #createProjectedCRS createProjectedCRS},
     *       {@link #createVerticalCRS createVerticalCRS}, {@link #createTemporalCRS createTemporalCRS} and their
     *       friends.
     *   <li>{@code ProjectedCRS.class} asks only for authority codes accepted by {@link #createProjectedCRS
     *       createProjectedCRS}.
     * </ul>
     *
     * The default implementaiton filters the set of codes based on the {@code "PROJCS"} and {@code "GEOGCS"} at the
     * start of the WKT strings.
     *
     ...
     * {@link PropertyAuthorityFactory} in order to keep only the codes for the specified type. Filtering is performed
     * on the fly. Consequently, this set is cheap if the user just want to check for the existence of a particular
     * code.
     */
    private static final class Codes extends DerivedSet<String, String> {
        ...
        /** Returns the code if the associated key is of the expected type, or {@code null} otherwise. */
        @Override
    ...
    /**
     * Returns an arbitrary object from a code. If the object type is know at compile time, it is recommended to invoke
     * the most precise method instead of this one.
     *
     * @param code Value allocated by authority.
     * @throws NoSuchAuthorityCodeException if the specified {@code code} was not found.
    ...
    /**
     * Returns a coordinate reference system from a code. If the object type is know at compile time, it is recommended
     * to invoke the most precise method instead of this one.
     *
     * @param code Value allocated by authority.
     * @throws NoSuchAuthorityCodeException if the specified {@code code} was not found.
     * @throws FactoryException if the object creation failed for some other reason.
     ...
     * {@linkplain #PropertyAuthorityFactory(ReferencingFactoryContainer, Citation[], URL) construction time}, then any
     * of them may appears as the scope in the supplied code.
     *
    ...
    /**
     * The WKT parser for this authority factory. This parser add automatically the authority code if it was not
     * explicitly specified in the WKT.
     */
