...
/**
 * An {@linkplain DefaultOperationMethod operation method} capable to creates a {@linkplain MathTransform math
 * transform} from set of {@linkplain GeneralParameterValue parameter values}. Implementations of this class should be
 ...
 * </blockquote>
 *
 * <p>The {@linkplain DefaultMathTransformFactory math transform factory} will parse this file in order to gets all
 * available providers on a system. If this file is bundle in many JAR files, the
...
public abstract class MathTransformProvider extends DefaultOperationMethod {
     ...
     * {@linkplain DefaultOperationMethod#DefaultOperationMethod(Map,int,int,ParameterDescriptorGroup) super-class
     * constructor}.
     *
     ...
     * {@linkplain org.geotools.api.referencing.operation.Projection}.class</code>, <cite>etc</cite>.
     *
     * <p>The default implementation returns {@code Operation.class}. Subclass should overrides this methods and returns
     * the appropriate OpenGIS interface type (<strong>not</strong> the implementation type).
     *
    ...
    /**
     * Creates a math transform from the specified group of parameter values. Subclasses can implements this method as
     ...
     * @throws FactoryException if the math transform can't be created for some other reason (for example a required
     *     file was not found).
    ...
    /**
     * Constructs a parameter descriptor from a set of alias. The parameter is identified by codes provided by one or
     * more authorities. Common authorities are {@link Citations#OGC OGC} and {@link Citations#EPSG EPSG} for example.
     *
     * <p>The first entry in the {@code identifiers} array is both the {@linkplain ParameterDescriptor#getName main
     * name} and the {@linkplain ParameterDescriptor#getIdentifiers identifiers}. All others are
     * {@linkplain ParameterDescriptor#getAlias aliases}.
     *
    ...
    /**
     * Constructs an optional parameter descriptor from a set of alias. The parameter is identified as with
    ...
    /**
     * Constructs a parameter group from a set of alias. The parameter group is identified by codes provided by one or
     * more authorities. Common authorities are {@link Citations#OGC OGC} and {@link Citations#EPSG EPSG} for example.
     *
     * <p>Special rules:
     *
     * <ul>
     *   <li>The first entry in the {@code identifiers} array is the {@linkplain ParameterDescriptorGroup#getName
     *       primary name}.
     *   <li>If a an entry do not implements the {@link GenericName} interface, it is an
    ...
    /**
     * Ensures that the given set of parameters contains only valid values. This method compares all parameter names
     * against the names declared in the {@linkplain #getParameters operation method parameter descriptor}. If an unknow
     * parameter name is found, then an {@link InvalidParameterNameException} is thrown. This method also ensures that
     * all values are assignable to the {@linkplain ParameterDescriptor#getValueClass expected class}, are between the
     * {@linkplain ParameterDescriptor#getMinimumValue minimum} and {@linkplain ParameterDescriptor#getMaximumValue
     * maximum} values and are one of the {@linkplain ParameterDescriptor#getValidValues set of valid values}. If the
     * value fails any of those tests, then an {@link InvalidParameterValueException} is thrown.
     *
     * @param values The parameters values to check.
     * @return The parameter values to use for {@linkplain MathTransform math transform} construction. May be different
     *     than the supplied {@code values} argument if some missing values needed to be filled with default values.
    ...
    /**
     * Implementation of {@code ensureValidValues}, to be invoked recursively if the specified values contains
     * sub-groups of values. This method copy all values from the user-supplied parameter values into the
     * provider-supplied one. The provider one should understand alias, and performs name conversion as well as argument
     * checking on the fly.
     *
    ...
    /**
     * Returns the parameter value for the specified operation parameter. This convenience method is used by subclasses
     * for initializing {@linkplain MathTransform math transform} from a set of parameters.
     *
     ...
     * @param param The parameter to look for.
     * @param group The parameter value group to search into.
     * @return The requested parameter value, or {@code null} if {@code param} is {@linkplain #createOptionalDescriptor
     *     optional} and the user didn't provided any value.
     * @throws ParameterNotFoundException if the parameter is not found.
     ...
     * @param param The parameter to look for.
     * @param group The parameter value group to search into.
     * @return The requested parameter value, or {@code 0} if {@code param} is {@linkplain #createOptionalDescriptor
     *     optional} and the user didn't provided any value.
     * @throws ParameterNotFoundException if the parameter is not found.
     * @todo Move to the {@link org.geotools.parameter.Parameters} class.
     */
     ...
     * standard units specified by the supplied {@code param} argument. This convenience method is used by subclasses
     * for initializing {@linkplain MathTransform math transform} from a set of parameters.
     *
     * @param param The parameter to look for.
     * @param group The parameter value group to search into.
     * @return The requested parameter value, or {@code NaN} if {@code param} is {@linkplain #createOptionalDescriptor
     *     optional} and the user didn't provided any value.
     * @throws ParameterNotFoundException if the parameter is not found.
     * @todo Move to the {@link org.geotools.parameter.Parameters} class.
     */
    ...
    /**
     * The result of a call to {@link MathTransformProvider#createMathTransform createMathTransform}. This class
     * encapsulates a reference to the {@linkplain #method originating provider} as well as the {@linkplain #transform
     * created math transform}. This information is needed when a provider delegates the work to an other provider
     * according the parameter values. For example a generic instance of
     * {@link org.geotools.referencing.operation.transform.ProjectiveTransform.ProviderAffine ProviderAffine} may
     * delegates the creation of an <cite>affine transform</cite> to an other {@code ProviderAffine} instance with
     * <cite>source</cite> and <cite>target</cite> dimensions matching the supplied parameters, because those dimensions
     * determine the set of legal <code>
     * "elt_<var>j</var>_<var>i</var>"</code> parameters.
     *
     * <p>Most {@linkplain MathTransformProvider math transform provider} do not delegate their work to an other one,
     * and consequently do not need this class.
     *
     * <p>Future Geotools version may extends this class for handling more information than just the
     * {@linkplain #transform transform} creator. This class is more convenient than adding new methods right into
     * {@link MathTransformProvider}, because it is sometime difficult for a provider to infer all the conditions
     * prevaling when {@link MathTransformProvider#createMathTransform createMathTransform} was executed. Furthermore,
     * it avoid to pollute {@code MathTransformProvider} with methods unused for the vast majority of providers.
     *
     * @version $Id$
     * @author Martin Desruisseaux (IRD)
     * @since 2.2
