...
/**
 * Transverse Mercator Projection (EPSG code 9807). This is a cylindrical projection, in which the cylinder has been
 * rotated 90°. Instead of being tangent to the equator (or to an other standard latitude), it is tangent to a central
 * meridian. Deformation are more important as we are going futher from the central meridian. The Transverse Mercator
 * projection is appropriate for region wich have a greater extent north-south than east-west.
 *
 * <p>The elliptical equations used here are series approximations, and their accuracy decreases as points move farther
 * from the central meridian of the projection. The forward equations here are accurate to a less than a mm &plusmn;10
 * degrees from the central meridian, a few mm &plusmn;15 degrees from the central meridian and a few cm &plusmn;20
 * degrees from the central meridian. The spherical equations are not approximations and should always give the correct
 * values.
 *
 * <p>There are a number of versions of the transverse mercator projection including the Universal (UTM) and Modified
 * (MTM) Transverses Mercator projections. In these cases the earth is divided into zones. For the UTM the zones are 6
 * degrees wide, numbered from 1 to 60 proceeding east from 180 degrees longitude, and between lats 84 degrees North and
 * 80 degrees South. The central meridian is taken as the center of the zone and the latitude of origin is the equator.
 * A scale factor of 0.9996 and false easting of 500000m is used for all zones and a false northing of 10000000m is used
 * for zones in the southern hemisphere.
 *
 * <p>NOTE: formulas used below are not those of Snyder, but rather those from the {@code proj4} package of the USGS
 * survey, which have been reproduced verbatim. USGS work is acknowledged here.
 *
 * <p><b>References:</b>
 *
 * <ul>
 *   <li>Proj-4.4.6 available at <A HREF="http://www.remotesensing.org/proj">www.remotesensing.org/proj</A><br>
 *       Relevent files are: {@code PJ_tmerc.c}, {@code pj_mlfn.c}, {@code pj_fwd.c} and {@code pj_inv.c}.
 ...
 * @see <A HREF="http://www.remotesensing.org/geotiff/proj_list/transverse_mercator.html">"Transverse_Mercator" on
 *     RemoteSensing.org</A>
 * @since 2.1
 * @version $Id$
...
public class TransverseMercator extends MapProjection {
    ...
    /**
     * A derived quantity of excentricity, computed by <code>e'² = (a²-b²)/b² = es/(1-es)</code> where <var>a</var> is
     * the semi-major axis length and <var>b</bar> is the semi-minor axis length.
     */
    ...
    private static final class Spherical extends TransverseMercator {
        ...
        /**
         * Maximal error tolerated for assertions in the spherical case. When assertions are enabled, every projection
         * using spherical formulas is followed by a projection using the ellipsical formulas, and the results are
         * compared. If a distance greater than the tolerance level is found, then an {@link AssertionError} will be
         * thrown. Subclasses may override this method if they need to relax the tolerance level.
         *
         * <p>The default implementation allows a larger tolerance when comparing spherical to elliptical calculations
         * when we are far from the central meridian (elliptical calculations are not valid here).
         *
         * @param longitude The longitude standardized (in radians) with {@linkplain #centralMeridian} already removed
         *     from it.
    ...
    /**
     * Convenience method computing the zone code from the central meridian. Information about zones convention must be
     * specified in argument. Two widely set of arguments are of Universal Transverse Mercator (UTM) and Modified
     * Transverse Mercator (MTM) projections:<br>
     *
     ...
     *
     * @param centralLongitudeZone1 Longitude in the middle of zone 1, in decimal degrees relative to Greenwich.
     *     Positive longitudes are toward east, and negative longitudes toward west.
     * @param zoneWidth Number of degrees of longitudes in one zone. A positive value means that zones are numbered from
     *     west to east (i.e. in the direction of positive longitudes). A negative value means that zones are numbered
     *     from east to west.
    ...
    /**
     * Convenience method returning the meridian in the middle of current zone. This meridian is typically the central
     * meridian. This method may be invoked to make sure that the central meridian is correctly set.
     *
     * @param centralLongitudeZone1 Longitude in the middle of zone 1, in decimal degrees relative to Greenwich.
     *     Positive longitudes are toward east, and negative longitudes toward west.
     * @param zoneWidth Number of degrees of longitudes in one zone. A positive value means that zones are numbered from
     ...
     *
     * @return The zone number, using the scalefactor and false easting to decide if this is a UTM or MTM case. Returns
     *     0 if the case of the projection cannot be determined.
     */
    public int getZone() {
        // UTM
     ...
     * meridian. This method may be invoked to make sure that the central meridian is correctly set.
     *
     * @return The central meridian, using the scalefactor and false easting to decide if this is a UTM or MTM case.
     *     Returns {@link Double#NaN} if the case of the projection cannot be determined.
     */
     ...
     * {@linkplain TransverseMercator Transverse Mercator} projection (EPSG code 9807).
     *
     * @since 2.1
     * @version $Id$
     * @author Martin Desruisseaux (PMO, IRD)
    ...
    /**
     * The {@linkplain org.geotools.referencing.operation.MathTransformProvider math transform provider} for a South
     * Orientated {@linkplain TransverseMercator Transverse Mercator} projection (EPSG code 9808). Note that at the
     * contrary of what this class name suggest, the projected coordinates are still increasing toward North. This is
     * because all {@linkplain MapProjection map projections} in Geotools must complies with standard axis orientations.
     * The real axis flip is performed by the CRS framework outside this package. See "<cite>Axis units and
     * orientation</cite>" in {@linkplain org.geotools.referencing.operation.projection package description} for
     * details.
     *
     ...
     *
     * <ul>
     *   <li><var>easting</var> = <var>{@linkplain #falseEasting false easting}</var> + <var>px</var>
     *   <li><var>northing</var> = <var>{@linkplain #falseNorthing false northing}</var> + <var>py</var>
     * </ul>
     *
     ...
     *
     * <ul>
     *   <li><var>westing</var> = <var>{@linkplain #falseEasting false easting}</var> - <var>px</var>
     *   <li><var>southing</var> = <var>{@linkplain #falseNorthing false northing}</var> - <var>py</var>
     * </ul>
     *
     * <p>Where the <var>px</var> and <var>py</var> terms are the same in both cases. Transforms created by this
     * provider actually computes (<var>easting</var>,<var>northing</var>) = (-<var>westing</var>,-<var>southing</var>).
     * This is equivalent to a {@link TransverseMercator} projection with {@link #falseEasting falseEasting} and
     * {@link #falseNorthing falseNorthing} sign reverted. This operation is implemented as a concatenation of a
     * North-orientated transverse mercator projection with an affine transform for (<var>false easting</var>,<var>false
     * northing</var>) correction.
     *
     * @since 2.2
     * @version $Id$
     * @author Martin Desruisseaux (PMO, IRD)
