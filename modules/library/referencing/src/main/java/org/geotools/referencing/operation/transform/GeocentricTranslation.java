 ...
 *
 * <ul>
 *   <li>If {@code "src_semi_major"} and {@code "src_semi_minor"} parameters are provided, then a
 *       {@code "Ellipsoid_To_Geocentric"} transform is concatenated before this transform.
 *   <li>If {@code "tgt_semi_major"} and {@code "tgt_semi_minor"} parameters are provided, then a
 *       {@code "Geocentric_To_Ellipsoid"} transform is concatenated after this transform.
 * </ul>
 *
 * @since 2.2
 * @version $Id$
...
public class GeocentricTranslation extends ProjectiveTransform {
    ...
    /** Creates a new geocentric affine transform using the specified parameter descriptor. */
    private GeocentricTranslation(final Matrix matrix, final ParameterDescriptorGroup descriptor) {
        super(matrix);
        this.descriptor = descriptor;
    }
    ...
    public static class Provider extends MathTransformProvider {
        ...
        protected MathTransform createMathTransform(final ParameterValueGroup values)
            ...
            return concatenate(
                    concatenate(
                            new GeocentricTranslation(parameters, getParameters()),
                            values,
                            SRC_SEMI_MAJOR,
                            SRC_SEMI_MINOR,
                            SRC_DIM),
