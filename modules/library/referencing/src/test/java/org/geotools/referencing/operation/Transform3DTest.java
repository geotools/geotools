...
/**
 * Tests of the {@code createProjectedCRS(...)} setting up the CRS with a 3D cartesian output for one test, and using a
 * 2D + vertical CRS compound for the second test. This test constructs most objects using GeoAPI only (except for a few
 * helper classes).
 *
 * @version $Id$
 * @author Justin Couch
...
public final class Transform3DTest {
    ...
    public void testProjectedToGeocentric() throws FactoryException, TransformException {
        ...
        // ----------------------------------------------------------
        final CoordinateSystemAxis x_axis =
                csFactory.createCoordinateSystemAxis(name("X"), "X", AxisDirection.OTHER, SI.METRE);
        final CoordinateSystemAxis y_axis =
                csFactory.createCoordinateSystemAxis(name("Y"), "Y", AxisDirection.WEST, SI.METRE);
        final CoordinateSystemAxis z_axis =
                csFactory.createCoordinateSystemAxis(name("Z"), "Z", AxisDirection.NORTH, SI.METRE);
        ...
        final GeographicCRS geographic_3d_crs =
                crsFactory.createGeographicCRS(name("3D geographic CRS"), wgs84, ellipsoidal_3d_cs);
    ...
    /**
     * Tries to transforms some points using the specified operation. This method transforms two points from the
     * projected CRS to the geocentric CRS. The first point is on the ellipsoid, and the second point is 10 km above the
     * ellipsoid. After transformation to geocentric CRS using a cartesian CS, the two points should still ten
     * kilometers apart each others.
     *
