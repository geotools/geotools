 ...
 * is performed against the same input, then it returns the cached result, if not the same, a new
 * evaluation is performed.
 *
 * <p>Implementation note: has been implemented using proxies after checking they do not hold a
 * significant overhead, the cost of filter/expression evaluation in GeoTools is much more expensive
 * in comparison
 *
 * @param <T>
 */
class FilterMemoizer {
    ...
    /**
     * Checks that a expression is not using unstable/volatile functions that might give back
     * different results in the same invocation thread
     */
