...
/**
 * Searches for all available {@link ExternalGraphicFactory} and {@link MarkFactory} implementations.
 *
 * <p>In addition to implementing this interface dynamic symbol handlers should have a services file:
 *
 * <ul>
 *   <li><code>META-INF/services/org.geotools.renderer.style.MarkFactory</code> if the are {@link MarkFactory} instances
 *   <li><code>META-INF/services/org.geotools.renderer.style.ExternalGraphicFactory</code> if the are
 *       {@link ExternalGraphicFactory} instances
 * </ul>
 *
...
public final class DynamicSymbolFactoryFinder {
    ...
    /**
     * Rendering hint key to store a {@link Comparator}<{@link MarkFactory}> instance to set the {@link MarkFactory}
     * execution order.
     */
    public static final Hints.Key MARK_FACTORY_ORDER = new Hints.Key(Comparator.class);
    /**
     * Rendering hint key to store a {@link Predicate}<{@link MarkFactory}> instance to set the allowed
     * {@link MarkFactory} instances to be evaluated.
     */
    ...
    /**
     * Finds all implementations of {@link MarkFactory} which have registered using the services mechanism.
     *
    ...
    /**
     * Finds all implementations of {@link MarkFactory} which have registered using the services mechanism. Returns a
     * filtered and ordered iterator based on the provided Hints input parameter.
     *
     * @param hints An optional map of hints for factory configuration, or {@code null} if none. Allowed Hints are:
     *     {@link DynamicSymbolFactoryFinder.MARK_FACTORY_ORDER}, {@link DynamicSymbolFactoryFinder.MARK_FACTORY_FILTER}
     * @return A filtered and ordered iterator over all discovered datastores which have registered factories, and whose
    ...
    /**
     * Finds all implementations of {@link ExternalGraphicFactory} which have registered using the services mechanism.
     *
