...
public class UserLayerImplTest {
    ...
    public void testUserLayerWithInlineFeatures() throws Exception {
        ...
        final UserLayerImpl layer = sf.createUserLayer();
        layer.setName(LAYER_NAME);
        layer.setInlineFeatureType(schema);
        layer.setInlineFeatureDatastore(ds);

        // create a user style and add it to that layer -----------------------
        final StyleImpl style = sf.createStyle();

        final StyleBuilder sb = new StyleBuilder(sf);
        final ExternalGraphicImpl overlay =
                sb.createExternalGraphic(IMAGE_URL, "image/" + IMAGE_EXT);
        final GraphicImpl g = sb.createGraphic(overlay, null, null, OPACITY, Double.NaN, 0.0);
        final PointSymbolizerImpl ps = sb.createPointSymbolizer(g);
        final FeatureTypeStyleImpl fts = sb.createFeatureTypeStyle(ps);
