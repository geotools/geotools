/**
 * This package contains relevant classes to support configuring a GeoTools ArcSDE <code>
 * ISessionPool</code> (a pool of connections to an ArcSDE server) inside a Java Naming and
 * Directory Interface (JNDI) context.
 *
 * <p>Since ArcSDE uses a legacy protocol to communicate between the ArcSDE service instance and the
 ...
 *
 * <p>The JNDI specification acts only at the API level, so how to configure a given resource to be
 * accessible through JNDI is J2EE Container specific. Some of them provide a means to configure an
 ...
 *
 * <p>Whether an ArcSDE connection pool is going to be shared among different web applications on
 ...
 * container's shared libraries folder instead of inside {@code <geoserver war>/WEB-INF/lib}. For
 ...
 *   <li>{@code commons-pool-1.3.jar} (or whatever commons-pool version the gt-arcsde-common depends
 *       on)
 * </ul>
 *
 ...
 *
 * <p>The following are configuration examples to set up <a href="http://geosever.org">GeoServer</a>
 ...
 *
 * <p>First off, you need to add an entry on the web application's WEB-INF/web.xml file to indicate
 ...
 *
 * <ul>
 *   <li>Copy the {@code jsde_sdk.jar, jpe_sdk.jar, icu4j.jar, commons-pool-1.3.jar and
 *       gt-arcsde-common-<version>.jar} files to the Tomcat shared libs folder (either <catalina
 *       home>/lib or <catalina home>/common/lib folder, depending on your Tomcat version).
 ...
 * </ul>
 *
 * Finally, configure the {@link ArcSDEConnectionFactory ArcSDE Connection Factory} as explained in
 * the <i>Adding Custom Resource Factories</i> section of the Tomcat's <a
 * href="http://tomcat.apache.org/tomcat-6.0-doc/jndi-resources-howto.html">JNDI Resources
 * HOW-TO</a>. That is, add an entry as the following to the file {@code <catalina
 * home>/conf/context.xml} as a child of the root {@code <Context>} element:
 *
 * <pre>
 * <code>
 ...
 * </pre>
 *
 * And that's it. Now you're able to select the <i>ArcSDE JNDI</i> DataStore factory and use the
 * {@code "geotools/arcsde"} (or whatever entry name of your choice) as the DataStore's JNDI
 * resource path name.
 *
 * <h4>Configuration in JBoss Application Server (4.0.3+)</h4>
 *
 * <p>JBoss provides an extensible mechanism to configure any kind of object as a JNDI resource
 ...
 * Disregard what the JBoss documentation says on its <a
 * href="http://www.jboss.org/file-access/default/members/jbossweb/freezone/docs/latest/jndi-resources-howto.html"
 * >JNDI Resources HOW-TO</a> , that seems to be a verbatim copy of the Tomcat documentation but is
 * just not supported.
 *
 * <p>To configure a JNDI ArcSDE session pool on JBoss, you need to create an xml file containing
 * the managed bean definition that configures the ArcSDE connection pool. You can call this file as
 * you want, for example {@code geoserver-service.xml}, and place it on the deploy folder for the
 * JBoss configuration that you're going to run. For example, {@code <jboss installation
 * dir>/server/default/deploy/geoserver-service.xml}
 *
 * <p>The contents of the {@code geoserver-service.xml} file shall be as following. For more
 * information about this kind of configuration consult the JBoss' <a
 * href="http://www.jboss.org/community/wiki/JNDIBindingServiceMgr">JNDIBindingServiceMgr</a>
 * documentation.
 *
 * <pre>
 * <code>
 ...
 * </pre>
 *
 * <p>What we're doing here is to create a {@code java.util.Properties} object containing the
 * connection parameters, and storing it on the JNDI container at the {@code geotools/arcsdeGlobal}
 * path. This is going to be a globally accessible resource that the {@link ArcSDEConnectionFactory}
 * will use to create the appropriate {@link org.geotools.arcsde.session.ISessionPool connection
 * pool}.
 *
 * <p>But to get this resource visible to your web application, there's one more step missing. In
 * addition to the resource-ref entry in {@code WEB-INF/web.xml} mentioned above, JBoss requires an
 * extra config file to manage the indirection between your web application and the globally
 * configured JNDI resource. You'll need to create a file called {@code jboss-web.xml} as a sibling
 * of {@code WEB-INF/web.xml} with the following content:
 *
 * <pre>
 * <code>
 * <?xml version="1.0" encoding="UTF-8"?>
 ...
 * </pre>
 *
 * Where {@code geotools/arcsdeGlobal} is the name of the globally configured resource, and {@code
 * geotools/arcsde} is the JNDI name by which the resource will actually be accessible to your web
 * application, and shall match the name used for the resource-ref entry in {@code WEB-INF/web.xml}.
 *
 * <p>Now you're ready to select the <i>ArcSDE JNDI</i> DataStore factory and use the {@code
 * "geotools/arcsde"} (or whatever entry name of your choice you have configured) as the DataStore's
