...
public class AutoCommitVersionHandler implements ArcSdeVersionHandler {
    ...
    private static final Logger LOGGER =
            Loggers.getLogger(AutoCommitVersionHandler.class.getName());
        ...
        session.issue(
                new Command<Void>() {
                    ...
                    public Void execute(ISession session, SeConnection connection)
                        ...
                        if (!(streamOperation instanceof SeQuery)) {
                            ...
                            if (LOGGER.isLoggable(Level.FINER)) {
                                LOGGER.finer(
                                        versionName
                                                + " version state: "
                                                + currStateId.longValue()
                                                + ", parent: "
                                                + currentState.getParentId().longValue()
                                                + ", open: "
                                                + currentState.isOpen()
                                                + ", owner: "
                            ...
                            if (!(currentState.isOpen() && currUser.equals(stateOwner))) {
                                ...
                                SeState newState =
                                        session.createChildState(currStateId.longValue());
                                if (LOGGER.isLoggable(Level.FINER)) {
                                    LOGGER.finer(
                                            "Setting version "
                                                    + versionName
                        ...
                        streamOperation.setState(
                                currentStateId, differencesId, SeState.SE_STATE_DIFF_NOCHECK);
                        return null;
                    }
                });
    }
    ...
    public void commitEditState() throws IOException {
        throw new UnsupportedOperationException(
                "commit shouldn't be called for" + " an autocommit versioning handler ");
    }
    ...
    public void rollbackEditState() throws IOException {
        throw new UnsupportedOperationException(
                "rollback shouldn't be called for" + " an autocommit versioning handler ");
    }
}
