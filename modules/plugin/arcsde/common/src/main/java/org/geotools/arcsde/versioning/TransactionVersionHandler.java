...
public class TransactionVersionHandler implements ArcSdeVersionHandler {
    ...
    private static final Logger LOGGER =
            Loggers.getLogger(TransactionVersionHandler.class.getName());
                ...
                new Command<Void>() {
                    ...
                    public Void execute(ISession session, SeConnection connection)
                        ...
                        if (transactionState == null) {
                            LOGGER.finer(
                                    "no transaction state created yet, about "
                                            + "to create a new state for the transaction");
                            try {
                                if (LOGGER.isLoggable(Level.FINEST)) {
                                    LOGGER.finest(
                                            "Refreshing '" + version.getName() + "' version info");
                                }
                                version.getInfo();

                                LOGGER.finest("Getting version state");
                                final SeState currentState =
                                        new SeState(connection, version.getStateId());
                                ...
                                LOGGER.finer(
                                        "New transaction state: "
                                                ...
                                                + ", parent: "
                                                + transactionState.getParentId().longValue());
                ...
                new Command<Void>() {
                    ...
                    public Void execute(ISession session, SeConnection connection)
                        ...
                        LOGGER.finer(
                                "Chaning version '"
                                        + version.getName()
                                        + "' state to point to transaction state "
                                        + transactionStateId);
