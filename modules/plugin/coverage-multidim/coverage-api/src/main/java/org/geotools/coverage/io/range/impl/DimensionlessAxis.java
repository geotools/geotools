 ...
 *
 * <p>This implementation of Axis can be seen as a stub implementation since in this case we do not
 * really have an {@link Axis} for this kind of data, or rather we have an axis that just represents
 * an ordinal or a certain set of .
 *
 * @author Simone Giannecchini, GeoSolutions
 * @todo add convenience constructor based on {@link SampleDimension} and or {@link SampleModel}
 * @source $URL$
 */
public class DimensionlessAxis implements Axis<String, Dimensionless> {
    ...
    /**
     * Helper classes for creating {@link DimensionlessAxis} for the most common color models'
     * bands.
     *
     ...
     *
     * @param raster a {@link RenderedImage} implementation from which to extract needed info,
     *     usually {@link ColorModel} and {@link SampleModel}.
    ...
    /** */
    public DimensionlessAxis(
            final int bandsNumber, final Name name, final InternationalString description) {
        String[] bandsKeys = new String[bandsNumber];
        for (int i = 0; i < bandsNumber; i++) bandsKeys[i] = Integer.toString(i);
        init(bandsKeys, name, description);
    }

    /** */
    public DimensionlessAxis(
            final String[] bands, final Name name, final InternationalString description) {
        init(bands, name, description);
    }

    /** */
    public DimensionlessAxis(
            final List<String> bandsKeys, final Name name, final InternationalString description) {
    ...
    public List<BandIndexMeasure> getKeys() {
        ...
        int i = 0;
        for (String band : this.bandsKeys) list.add(new BandIndexMeasure(i++, band));
        return list;
    }
