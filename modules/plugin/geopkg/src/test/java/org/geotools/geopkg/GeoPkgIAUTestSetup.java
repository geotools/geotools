...
public class GeoPkgIAUTestSetup extends JDBCIAUTestSetup {
    ...
    protected void createMarsPoiTable() throws Exception {
        ...
        run(
                "CREATE TABLE mars_poi (id integer primary key autoincrement, geom blob, name varchar, diameter double)");
        run(
                "INSERT INTO mars_poi (geom, name, diameter) VALUES (X'"
    ...
    protected void removeIAU49901() throws Exception {
        run(
                "DELETE FROM gpkg_spatial_ref_sys WHERE organization_coordsys_id = 49901 and organization = 'IAU'");
    }
}
