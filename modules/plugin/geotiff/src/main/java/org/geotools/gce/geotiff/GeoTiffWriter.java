...
public class GeoTiffWriter extends AbstractGridCoverageWriter implements
	...
	private static void setGeoReference(final CoordinateReferenceSystem crs,
			...
			final AffineTransform rasterToModel, GridEnvelope2D range, boolean retainAxesOrder)
			throws IOException {
		...
		//
		// Note that here we assume that in case of a Flip the flip is on the Y
		// axis.
		//
		...
		if (!(Double.isInfinite(rotation) || Double.isNaN(rotation) || Math
				.abs(rotation) > 1E-6)) {
			final double tiePointLongitude = (swapAxes) ? modifiedRasterToModel
					.getTranslateX() : modifiedRasterToModel.getTranslateY();
			final double tiePointLatitude = (swapAxes) ? modifiedRasterToModel
					.getTranslateY() : modifiedRasterToModel.getTranslateX();
			metadata.setModelTiePoint(0, 0, 0, tiePointLongitude,
					tiePointLatitude, 0);
			// scale
			final double scaleModelToRasterLongitude = (swapAxes) ? Math
					.abs(modifiedRasterToModel.getScaleX()) : Math.abs(modifiedRasterToModel
					.getShearY());
			final double scaleModelToRasterLatitude = (swapAxes) ? Math
