...
public class ROIExcessGranuleRemover {
    ...
    public boolean addGranule(GranuleLoadingResult result) {
        ...
        CoordinateReferenceSystem granuleCRS = result.getGranuleDescriptor().getGranuleEnvelope()
                .getCoordinateReferenceSystem();
        if (granuleCRS != null && targetCRS != null
        ...
        if (roi == null) {
            ...
            Rectangle bounds = new Rectangle(raster.getMinX(), raster.getMinY(), raster.getWidth(),
                    raster.getHeight());
        ...
        if (!roi.getBounds().intersects(rasterBounds)) {
            ...
            if (binarizator == null) {
                binarizator = new Binarizator(JTS.toPolygon(rasterBounds), rasterBounds.width,
                        rasterBounds.height, tileWidth, tileHeight);
            }
