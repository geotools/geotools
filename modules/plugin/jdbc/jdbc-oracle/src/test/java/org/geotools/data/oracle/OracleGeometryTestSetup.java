...
public class OracleGeometryTestSetup extends JDBCGeometryTestSetup {
    ...
    public void setUp() throws Exception {
        ...
        sql =
                "CREATE INDEX GTMETA_GEOMETRY_IDX ON GTMETA(GEOMETRY) INDEXTYPE IS MDSYS.SPATIAL_INDEX";
        run(sql);

        sql =
                "INSERT INTO GTMETA VALUES (0,"
    ...
    public void setupGeometryColumns(JDBCDataStore dataStore) throws Exception {
        ...
        String sql =
                "CREATE TABLE GEOMETRY_COLUMNS(F_TABLE_SCHEMA VARCHAR(30), F_TABLE_NAME VARCHAR(30), "
                        + "F_GEOMETRY_COLUMN VARCHAR(30), COORD_DIMENSION INTEGER, SRID INTEGER, TYPE VARCHAR(30))";
        run(sql);

        // register it in the override table with a different srs, so that we're sure it's getting
        // read
        sql =
                "INSERT INTO GEOMETRY_COLUMNS (F_TABLE_SCHEMA, F_TABLE_NAME, F_GEOMETRY_COLUMN, COORD_DIMENSION, SRID, TYPE) "
                        + "VALUES ('"
                        + schema
                        + "', 'GTMETA','GEOMETRY', 2, 4269, 'POINT')";
        run(sql);
