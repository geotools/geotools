...
public class PostGISDialect extends BasicSQLDialect {
    ...
    public void encodeGeometryEnvelope(String tableName, String geometryColumn, StringBuffer sql) {
        ...
        sql.append("ST_Extent(" + escapeIdentifier(geometryColumn) + "::geometry))))");
    }
    ...
    public String getSequenceForColumn(
        ...
        try {
            ...
            if (schemaName != null && !"".equals(schemaName))
                sql += escapeIdentifier(schemaName) + ".";
            sql += escapeIdentifier(tableName) + "', '" + columnName + "')";
                    ...
                    if (getVersion(cx).compareTo(V_2_0_0) >= 0) {
                        ...
                        sql =
                                "ALTER TABLE "
                                        ...
                                        + " "
                                        + "ALTER COLUMN "
                                        + escapeIdentifier(gd.getLocalName())
                                        + " "
                                        + "TYPE geometry ("
                                        + geomType
                                        + ", "
                        ...
                        if (srid > -1) {
                            sql =
                                    "ALTER TABLE " //
                                            ...
                                            + escapeIdentifier(tableName)
                                            + " ADD CONSTRAINT "
                                            + escapeIdentifier("enforce_srid_" + gd.getLocalName())
                                            + " CHECK (ST_SRID(" //
                                            + escapeIdentifier(gd.getLocalName())
                                            + ") = "
                                            + srid
                        ...
                        sql =
                                "ALTER TABLE " //
                                        ...
                                        + escapeIdentifier(tableName)
                                        + " ADD CONSTRAINT "
                                        + escapeIdentifier("enforce_dims_" + gd.getLocalName())
                                        + " CHECK (st_ndims("
                                        + escapeIdentifier(gd.getLocalName())
                                        + ")" //
                                        + " = "
                        ...
                        if (!geomType.equals("GEOMETRY")) {
                            sql =
                                    "ALTER TABLE " //
                                            ...
                                            + " ADD CONSTRAINT "
                                            + escapeIdentifier(
                                                    "enforce_geotype_" + gd.getLocalName())
                                            + " CHECK (geometrytype(" //
                                            + escapeIdentifier(gd.getLocalName())
                                            + ") = '"
                                            + geomType
