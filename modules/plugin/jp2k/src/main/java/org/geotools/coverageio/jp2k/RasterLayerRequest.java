...
class RasterLayerRequest {
    ...
    public RasterLayerRequest(
        ...
        // //
        //
        // Set specific imageIO parameters: type of read operation, imageReadParams
        //
        // //
        checkReadType();
        prepare();
    ...
    private void setDefaultParameterValues() {
        ...
        for (GeneralParameterDescriptor descriptor : parametersDescriptors) {
            ...
            // the syntax: "TileWidth,TileHeight" (without quotes where
            // TileWidth
    ...
    private void inspectCoordinateReferenceSystems() throws DataSourceException {
        ...
        if (destinationToSourceTransform != null && destinationToSourceTransform.isIdentity()) {
            ...
            // update the requested grid to world transformation by pre
            // concatenating the destination to source transform
     ...
     * {@code true} if a JAI imageRead operation need to be performed or {@code
     * false} if a simple read operation is needed.
     * 
    ...
    private void computeCropRasterArea() throws DataSourceException {
        ...
        try {
            requestedWorldToGrid = (MathTransform2D) PixelTranslation.translate(ProjectiveTransform.create(requestedGridToWorld),
        ...
        if (destinationToSourceTransform == null || destinationToSourceTransform.isIdentity()) {
            ...
            //
            // reproject the crop bbox back and then crop, notice that we are
            // imposing
            //
            try {
                ...
                destinationRasterArea = new GeneralGridEnvelope(CRS.transform(
                        requestedWorldToGrid, cropBBOXInRequestCRS), PixelInCell.CELL_CORNER, false).toRectangle();
                // intersect with the original requested raster space to be sure
                // that we stay within the requested raster area
     ...
     *            is the requested region where to load data of the specified
     *            envelope.
    ...
    private void computeRequestedResolution() throws DataSourceException {
        ...
        try {
            ...
            if (requestedGridToWorld instanceof LinearTransform) {
                ...
                //
                // the crs of the request and the one of the coverage are NOT
                // the
                // same and the conversion is not , we can get the resolution
                // from envelope + raster directly
                //
                if (destinationToSourceTransform != null && !destinationToSourceTransform.isIdentity()) {

                    //
                    // compute the approximated resolution in the request crs,
                    // notice that we are assuming a reprojection that keeps the raster area
                    // unchanged hence the effect is a degradation of quality, but we might take
                    // that into account empirically
                    //
                    ...
                    // the same, we can get the resolution from the grid to world
                    //					
                    requestedResolution = new double[] {
    ...
    private void computeCropBBOX() throws DataSourceException {
        ...
        try {
            ...
            if (destinationToSourceTransform != null && !destinationToSourceTransform.isIdentity()) {
                ...
                // aboid problems with the envelope checks
            ...
            //
            // STEP 2: intersect requested BBox in native CRS with coverage
            // native bbox to get the crop bbox
            //
            ...
            cropBBox = new ReferencedEnvelope(((ReferencedEnvelope) cropBBox).intersection(rasterManager.spatialDomainManager.coverageBBox),
                    rasterManager.spatialDomainManager.coverageCRS2D);
        ...
        try {
            ...
            // STEP 1 reproject the requested envelope to the coverage
            // geographic bbox
            if (!CRS.equalsIgnoreMetadata(rasterManager.spatialDomainManager.coverageGeographicCRS2D, requestCRS)) {
                ...
                requestCRSToCoverageGeographicCRS2D = CRS.findMathTransform(
                                requestCRS, rasterManager.spatialDomainManager.coverageGeographicCRS2D, true);
            ...
            // now go back to the coverage native CRS in order to compute an
            // approximate requested resolution
