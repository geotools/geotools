    ...
    class GranuleWorker {
        ...
        public void init(final ReferencedEnvelope aoi) {
            ...
            final GranuleLoader loader = new GranuleLoader(
                    baseReadParameters, imageChoice, bbox, finalWorldToGridCorner, granule, request.getTileDimensions());
        ...
        public void produce() {
            ...
            for (Future<RenderedImage> future : tasks) {
                ...
                try {
                    ...
                    if (loadedImage == null) {
                        if (LOGGER.isLoggable(Level.FINE)){
                            LOGGER.log(Level.FINE, new StringBuilder("Unable to load the raster for granule ")
                            .append(granuleIndex).append(" with request ").append(request.toString()).toString());
                        }
                        continue;
                    }
                    if (firstGranule) {
                        //
                        // We check here if the images have an alpha channel or
                        // some
                        // other sort of transparency. In case we have
                        // transparency
                    ...
                    if (LOGGER.isLoggable(Level.FINE)) {
                        LOGGER.fine(new StringBuilder("Loading image number ").append(granuleIndex)
                                .append(" failed, original request was ").append(request).toString());
                    }
                    continue;
    ...
    private RenderedImage prepareResponse() throws DataSourceException {
        ...
        try {
            ...
            final AffineTransform2D adjustments = new AffineTransform2D(
                    (level.resolutionX / baseLevel.resolutionX) * baseReadParameters.getSourceXSubsampling(), 0, 0,
                    (level.resolutionY / baseLevel.resolutionY) * baseReadParameters.getSourceYSubsampling(), 0, 0);
            ...
            rasterBounds = new GeneralGridEnvelope(
                    CRS.transform( finalWorldToGridCorner, bbox), PixelInCell.CELL_CORNER, false).toRectangle();
            ...
            if (worker.granulesNumber >= 1) {
                if (LOGGER.isLoggable(Level.FINE)) {
                    LOGGER.fine(new StringBuilder(
                            "Loaded bbox ").append(bbox.toString())
