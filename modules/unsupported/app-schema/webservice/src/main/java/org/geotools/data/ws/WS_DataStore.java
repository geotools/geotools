 ...
 *
 * <p>Unlike normal DataStores that return features, this returns xml.
 *
...
public class WS_DataStore implements XmlDataStore {
    ...
    public void dispose() {
        try {
            ...
            LOGGER.info(
                    "Failed closing capabilities stream for web service backend store: "
                            + e.getMessage());
        }
    }
    ...
    protected List<Integer> determineValidFeatures(
            Filter postFilter, Document doc, int maxFeatures) {
        ...
        if (Filter.INCLUDE.equals(postFilter)) {
            ...
            while (nodeIndex <= nodeCount && validFeatureIndex.size() <= maxFeatures) {
                XmlXpathFilterData peek =
                        new XmlXpathFilterData(namespaces, doc, nodeIndex, itemXpath);
    ...
    protected List<Integer> determineValidFeatures(
            String xpath, String value, Document doc, int maxFeatures) {
        int nodeCount = XmlXpathUtilites.countXPathNodes(namespaces, itemXpath, doc);
    ...
    public FeatureReader<SimpleFeatureType, SimpleFeature> getFeatureReader(
            Query query, final Transaction transaction) throws IOException {
        throw new UnsupportedOperationException("DS not supported!");
    }
