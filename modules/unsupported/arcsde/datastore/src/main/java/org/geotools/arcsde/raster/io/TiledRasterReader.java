 ...
 * </code>
 *
 * <p>So one has to call {@code nextRaster()} to get the id of the raster immediately available to
 * be read through {@link #read()}. This is so because there might be more than one raster on a
...
public interface TiledRasterReader {
     ...
     * @throws IOException for any problem occurred retrieving the next {@link SeRasterAttr} in the
     *     request
     */
     ...
     * @param matchingTiles the range of tiles to read at the given pyramid level. The boundaries of
     *     the tile range are inclusive and starts at {@code 0,0} for the upper left most tile.
     ...
     */
    RenderedImage read(
            final long rasterId, final int pyramidLevel, final GridEnvelope matchingTiles)
            throws IOException;
}
