...
/**
 * The root node of a grid, which has n GridNodes as children. As GridNodes do, it can store data
 * too.
 *
...
public class GridRootNode extends GridNode {
    ...
    /**
     * No-arg constructor for serialization purpose. Deserialized nodes must call setIdentifier
     * before using
     */
    GridRootNode() {
        super();
    ...
    public List<Integer> getChildren(Shape shape) {
        ...
        for (int i = 0; i < gridminindex.length; i++) {
            if (childMBR.getLow(i) <= mbr.getLow(i)) {
                ...
                gridminindex[i] =
                        (int) Math.floor((childMBR.getLow(i) - mbr.getLow(i)) / tiles_size);
            }
        }
        ...
        for (int i = 0; i < gridminindex.length; i++) {
            if (childMBR.getHigh(i) >= mbr.getHigh(i)) {
                ...
                gridmaxindex[i] =
                        (int) Math.floor((childMBR.getHigh(i) - mbr.getLow(i)) / tiles_size);
            }
        }
    ...
    public String toString() {
        ...
        sb.append("GridRootNode: MBR: " + this.getShape() + ", capacity: " + getCapacity());
    ...
    //		return "POLYGON(( " + minx + " " + miny + ", " + minx  + " " + maxy  + "," + maxx + " " +
