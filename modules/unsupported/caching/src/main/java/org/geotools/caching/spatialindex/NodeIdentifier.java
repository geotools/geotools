...
/**
 * Instances of this class provide unique identifiers for nodes, and are used to store and retrieve
 * nodes from their storage. Implementors must take care that instances have to be immutable. Nodes
 * are basically identified by the region they represent. Kinds of nodes or kinds of storage may
 * require to use other elements to identify nodes. Implementors must take care to override
 * hashCode() and equals() accordingly. NodeIdentifier should not reference the node they identify,
...
public abstract class NodeIdentifier implements Serializable {
    ...
    // wait a maximum of 5 minutes for lock; this should never be necessary it's just a safe guard
    ...
    /**
     * Sets the node associated with the nodeid; this node is stored as a soft reference so
