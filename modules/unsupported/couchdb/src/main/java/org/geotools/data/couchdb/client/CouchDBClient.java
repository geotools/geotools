 ...
 *
 * <p>The goal of the client is to abstract much of the HTTP client interaction as possible to allow
 * higher level use and hide the details of the client.
 *
 * <p>The lower level HTTP public APIs should not be used except by internal code.
 *
 * <p>All 'path' parameters, unless otherwise specified, are relative to the root URL.
 *
 * <p>All 'IOException's thrown in the client are due to HTTP errors, CouchDB specific errors are
 * covered by CouchDBException.
 *
 * <p>The Client does not perform any checking of CouchDBResponses for errors. All methods that
 * return CouchDBResponses will never return null.
 *
 * <p>A Client instance should always be closed when done to release pooled connections.
 *
 * <p>Notes: * Using older httpclient 3.1, could upgrade to 4.x, but what impact on other libraries
 *
...
public class CouchDBClient implements Closeable {
    ...
    /**
     * Create a new client that will connect to a CouchDB instance at the specified root URL
     *
        ...
        if (content != null) {
            try {
                put.setRequestEntity(
                        new StringRequestEntity(content, MIME_TYPE_JSON, DEFAULT_CHARSET));
            } catch (UnsupportedEncodingException ex) {
                throw new RuntimeException(ex);
            }
        }
