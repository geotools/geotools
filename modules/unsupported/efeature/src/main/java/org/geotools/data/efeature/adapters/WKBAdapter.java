...
/**
 * An Adapter class to be used to extract from -adapt- the argument object to some (primitive) byte
 * array that would later be used in <code>Condition</code> evaluation.
 *
...
public abstract class WKBAdapter implements IDataTypeAdapter<byte[]> {
    ...
    public static final WKBWriter WRITER = new WKBWriter();

    /** Cached {@link WKBReader} instance for reading */
    public static final WKBReader READER = new WKBReader();

    /**
     * The simplest {@link WKBAdapter} implementation that represents the case when the argument
     * object to adapt is a byte array or a {@link Geometry} instance.
     */
    public static final WKBAdapter DEFAULT =
            new WKBAdapter() {
                @Override
