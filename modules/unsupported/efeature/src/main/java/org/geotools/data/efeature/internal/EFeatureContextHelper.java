 ...
 *
 * <p><b>The context startup problem</b>:
 *
 * <p>
 *
 * <ol>
 *   <li>Since multiple {@link EFeatureContextFactory} instances are allowed, a {@link
 *       EFeatureContext context} ID is ambiguous without knowing which factory created the actual
 *       context.
 *   <li>Since EMF {@link EPackage}s are completely unaware of the {@link EFeatureContext context}
 *       during the construction of {@link EOBject}s from {@link EClass}s, the actual {@link
 *       EFeatureInfo structure} (and context) remains unknown until it is {@link
 *       EFeature#setStructure(EFeatureInfo) explicitly set}.
 * </ol>
 *
 * <b>Solution</b>
 *
 * <p>The problem is solved by an internal {@link EFeatureContext#isPrototype() prototype context}
 * which {@link EFeatureInternal} objects belongs to during context-unaware construction. This
 * context {@link EFeatureContextInfo#eFeatureInfoCache() caches} all the {@link EFeatureInfo
 * structures} generated from {@link EObject} during the context-unaware construction. Only objects
 * that implements {@link EFeature} or contains {@link Geometry EFeature compatible data} are added
 * to the {@link EFeatureInfoCache cache}.
 *
 * <p>Each {@link EFeatureInternal} instance use the structure returned by the {@link
 * EFeatureContextHelper helper}, until the actual context is known.
 *
 * <p>When the actual structure is known, {@link EFeatureInternal#setStructure(EFeatureInfo)} is
 * called, mapping the {@link EObject} to given structure.
 *
 * <p>The {@link EFeatureReader} does this during {@link EObject} iteration. <b>NOTE</b>: This
 * solution adds some memory overhead since one additional {@link EFeatureInfo} instance is created
 * for each unique {@link EClass} added to a {@link EFeatureContext context}.
 *
...
public final class EFeatureContextHelper {
    ...
    static {
        ...
        eContext =
                eContextFactory.create(
                        INTERNAL_CONTEXT_ID, new EFeatureVoidIDFactory(), new EFeatureHints());
    }
     ...
     *
     * <p>If not already added, this method adds the {@link EPackage}, which given object belongs,
     * to the context.
     *
