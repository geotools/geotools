...
public class EFeatureQueryTest extends AbstractResourceTest {
    ...
    private static final LineString[] LINESTRINGS =
            new LineString[] {create(LineString.class, "LINESTRING (20 20, 40 20)")};

    private static final Polygon[] POLYGONS =
            new Polygon[] {
                //
    ...
    private static final Geometry[] GEOMETRIES =
            EFeatureUtils.concat(Geometry.class, POINTS, LINESTRINGS, POLYGONS);

    private int eFeatureCount = 200;
    private EFeatureTestData eData;
    private Object[][] eTypeData =
            new Object[][] {
    ...
    public void testFeatureGeometryMatch() {
        try {
            ...
            while (it.hasNext()) {
                EObject eObject = it.next();
                assertTrue(
                        "EObject " + eObject + " is not an " + "instance of EFeatureData",
                        eObject instanceof EFeatureData);
                EFeatureData<?, ?> eFeatureData = (EFeatureData<?, ?>) eObject;
                Object a = eFeatureData.getAttribute();
                Object g = eFeatureData.getGeometry();
                assertEquals(
                        "Unexpected EFeatureData geometry value found",
                        GEOMETRIES[(Integer) a].toString(),
                        g.toString());
                count++;
            }
        } catch (Exception e) {
    ...
    public void testFeatureGeometryCQL() {
        try {
            ...
            int count = 0;
            TreeIterator<EObject> eObjects = eResource.getAllContents();
            EFeatureQuery eQuery =
                    EFeatureUtils.toEFeatureQuery(eFeatureDataInfo, eObjects, filter);
            EFeatureIterator it = eQuery.iterator();
            while (it.hasNext()) {
                EObject eObject = it.next();
                assertTrue(
            ...
            assertEquals(
                    "Unexpected number of EFeatureData instances found", GEOMETRIES.length, count);
        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, e.getMessage(), e);
            fail(e);
        }
    ...
    protected void doSetUp() throws Exception {
        ...
        eFeatureCompatibleDataInfo =
                eContextHelper.eGetFeatureInfo("efeature.EFeatureCompatibleData");
        //
