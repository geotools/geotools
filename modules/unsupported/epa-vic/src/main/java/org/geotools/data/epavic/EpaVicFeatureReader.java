...
public class EpaVicFeatureReader implements FeatureReader<SimpleFeatureType, SimpleFeature> {
    ...
    public EpaVicFeatureReader(
            SimpleFeatureType featureTypeIn, InputStream stream, Sites sites, Monitors monitors)
            throws IOException {
    ...
    public EpaVicFeatureReader(
        ...
        if (sites != null) {
            this.sites =
                    sites.getSites()
                            .stream()
                            .collect(Collectors.toMap(Site::getSiteId, item -> item, (u, v) -> u));
        }
        if (monitors != null) {
            this.monitors =
                    monitors.getMonitors()
                            ...
                            .collect(
                                    Collectors.toMap(
                                            Monitor::getMonitorId, item -> item, (u, v) -> u));
        }
        this.featIndex = 0;
    ...
    public SimpleFeature next() throws NoSuchElementException, IOException {
        if (jParser.hasCurrentToken()
            ...
            if (site == null) {
                throw new IllegalStateException("Site " + val.getSiteId() + " is unavailable");
            }
