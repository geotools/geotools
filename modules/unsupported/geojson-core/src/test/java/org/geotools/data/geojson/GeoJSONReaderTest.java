...
public class GeoJSONReaderTest {
	...
	/**
	 * Test method for
	 * {@link org.geotools.data.geojson.GeoJSONReader#getFeatures()}.
	 */
	@Test
	...
	public void testReadFromInputStream() throws Exception {
		...
		try (GeoJSONReader reader = new GeoJSONReader(new ByteArrayInputStream(input.getBytes()))) {
			SimpleFeatureCollection features = reader.getFeatures();
			assertNotNull(features);
			assertEquals(Point.class, features.getSchema().getGeometryDescriptor().getType().getBinding());
			assertEquals("wrong number of features read", 9, features.size());
	...
	public void testFeatureCollectionParser() throws Exception {
		String geojson1 = "{" + "'type': 'FeatureCollection'," + "'features': " + "[{" + "  'type': 'Feature',"
				+ "  'id': 'feature.0'," + "  'properties': {" + "    'otherGeometry': {"
				+ "      'type': 'LineString'," + "      'coordinates': [[1.1, 1.2], [1.3, 1.4]]" + "    }" + "  },"
	...
	public void testFeatureWithRegularGeometryAttributeReadAndGeometryAfterProperties() throws Exception {
		String geojson1 = "{" + "  'type': 'Feature'," + "  'id': 'feature.0'," + "  'properties': {"
				+ "    'otherGeometry': {" + "      'type': 'LineString',"
				+ "      'coordinates': [[1.1, 1.2], [1.3, 1.4]]" + "    }" + "  }," + "  'geometry': {"
	...
	public void testReadBoolean() throws Exception {
		String geojson = "{" + "  'type': 'Feature'," + "  'id': 'feature.0'," + "  'properties': {"
				+ "    'boolTrue': true," + "    'boolFalse': false" + "   }," + "  'geometry': {"
				+ "    'type': 'Point'," + "    'coordinates': [0.1, 0.1]" + "  }" + "}";
	...
	public void testReadNestedObject() throws Exception {
		String geojson = "{" + "  'type': 'Feature'," + "  'id': 'feature.0'," + "  'properties': {" + "    'object': {"
				+ "       'a': 10," + "       'b': 'foo'" + "    }" + "   }," + "  'geometry': {"
				+ "    'type': 'Point'," + "    'coordinates': [0.1, 0.1]" + "  }" + "}";
		geojson = geojson.replace('\'', '"');
		SimpleFeature feature = GeoJSONReader.parseFeature(geojson);
	...
	public void testReadNestedArray() throws Exception {
		String geojson = "{" + "  'type': 'Feature'," + "  'id': 'feature.0'," + "  'properties': {"
				+ "    'array': [10, 'abc', null]" + "   }," + "  'geometry': {" + "    'type': 'Point',"
				+ "    'coordinates': [0.1, 0.1]" + "  }" + "}";
		geojson = geojson.replace('\'', '"');
		SimpleFeature feature = GeoJSONReader.parseFeature(geojson);
