...
/**
 * A Complex is a collection of geometrically disjoint, simple Primitives. If a Primitive (other
 * than a Point) is in a particular Complex, then there exists a set of primitives of lower
 * dimension in the same complex that form the boundary of this primitive.
 *
 * <p>NOTE A geometric complex can be thought of as a set in two distinct ways. First, it is a
 * finite set of objects (via delegation to its elements member) and, second, it is an infinite set
 * of point values as a subtype of geometric object. The dual use of delegation and subtyping is to
 * disambiguate the two types of set interface. To determine if a Primitive P is an element of a
 * Complex C, call: C.element().contains(P).
 *
 * <p>The "element" attribute allows Complex to inherit the behavior of Set<Primitive> without
 * confusing the same sort of behavior inherited from TransfiniteSet<DirectPosition2D> inherited
 * through Object. Complexes shall be used in application schemas where the sharing of geometry is
 * important, such as in the use of computational topology. In a complex, primitives may be
 * aggregated many-to-many into composites for use as attributes of features. Examples of this are
...
public abstract class ComplexImpl extends GeometryImpl implements Complex {
    ...
    /**
     * Contains association The association "Contains" instantiates the contains operation from
     * Set<Primitive> as an association. Set<Complex> of Complex [0..n]
     */
    ...
    /**
     * The association "Complex" is defined by the "contains" operation in Object that is inherited
     * from TransfiniteSet<DirectPosition2D>. Complex::element [1..n] : Primitive If a complex
     * contains a Primitive, then it must also contain the elements of its boundary. Complex: --
     * closed under the boundary operation self?forAll(self?includesAll(boundary()))
     *
     * <p>ArrayList of Primitive [1..n]
     */
    ...
    /**
     * The Boolean valued operation "isMaximal" shall return TRUE if and only if this Complex is
     * maximal. Complex::isMaximal() : Boolean
     *
    ...
    public Complex[] getSuperComplexes() {
        return (this.superComplex == null)
                ? null
                : this.superComplex.toArray(new Complex[this.superComplex.size()]);
    }
    ...
    public Complex[] getSubComplexes() {
        return (this.subComplex == null)
                ? null
                : this.subComplex.toArray(new Complex[this.subComplex.size()]);
    }
