...
/**
 * CurveSegment defines a homogeneous segment of a Curve. Each CurveSegment shall be in, at most,
 * one Curve.
 *
...
public abstract class CurveSegmentImpl implements CurveSegment, Serializable {
     ...
     *
     * <p>GenericCurve::startParam() : Distance
     *
     * <p>GenericCurve:
     *
     * <p>{parameterization(startParam()) = startPoint()};
     *
     * <p>{parameterization(endParam()) = endPoint()};
     *
     * <p>{length() = endParam() - startParam()}
     *
     * <p>The start and end parameter of a Curve are always 0 and the arc length of the curve
     * respectively. For CurveSegments within a Curve, the start and end parameters of the
     * CurveSegment are equal to those of the Curve where this segment begins and ends in the
     * Segmentation association, so that the startParam of any segment (except the first) is equal
     * to the endParam of the previous segment. If a GenericCurve is used for other purposes, there
     * shall be a restriction that the two parameters must differ by the arc length of the
     * GenericCurve.
     */
     ...
     *
     * <p>GenericCurve::endParam() : Distance
     *
     * <p>GenericCurve:
     *
     * <p>{parameterization(startParam()) = startPoint()};
     ...
     * position of this CurveSegment.
     *
     * <p>CurveSegment::interpolation : CurveInterpolation = "linear"
     */
    protected CurveInterpolation interpolation = null;

    /**
     * Segmentation association from specification of Curve The association "segmentation" lists the
     * components (CurveSegments) of Curve, each of which defines the direct position of points
     * along a portion of the curve. The order of the CurveSegments is the order in which they are
     * used to trace the Curve.
     *
     * <p>Curve::segment [1..n] : Sequence <CurveSegment>
     *
     * <p>CurveSegment::curve [0,1] : Reference <Curve>
     *
     * <p>For a particular parameter interval, the Curve and CurveSegment agree. CurveSegment:
     * {curve.startParam() <= self.startParam()}; {curve.endParam() >= self.endParam()};
     * {self.startParam() < self.endParam()}; {s : Distance (startParam() <= s <= endParam())
     * implies (curve.parameterization(s) = self.parameterization(s))};
     *
     * <p>NOTE In this standard, curve segments do not appear except in the context of a curve, and
    ...
    public CurveBoundaryImpl getBoundary() {
        return new CurveBoundaryImpl(
                this.getCurve().getCoordinateReferenceSystem(),
    ...
    /**
     * The operation "samplePoint" returns an ordered array of point values (PointArray) that lie on
     * the CurveSegment. In most cases, these will be related to control points used in the
     * construction of the segment.
     *
     * <p>CurveSegment::samplePoint() : PointArray
     *
     * <p>NOTE The controlPoints of a curve segment are use to control its shape, and are not always
     * on the curve segment itself. For example in a spline curve, the curve segment is given as a
     * weighted vector sum of the controlPoints. Each weight function will have a maximum within the
    ...
    /**
     * The operation "boundary" on CurveSegment operates with the same semantics as that on Curve
     * except that the end points of a CurveSegment are not necessarily existing Points and thus the
     * boundary may contain transient Points.
     *
     * <p>CurveSegment::boundary() : CurveBoundary
     *
     * <p>NOTE The above CurveBoundary will almost always be two distinct positions, but, like
     * Curves, CurveSegments can be cycles in themselves. The most likely scenario is that all of
    ...
    /**
     * The reverse of a CurveSegment simply reverses the orientation of the parameterizations of the
     * segment. CurveSegment::reverse() : CurveSegment
     *
    ...
    /**
     * GenericCurve: {parameterization(startParam()) = startPoint()}; {parameterization(endParam())
     * = endPoint()}; {length() = endParam() - startParam()} returns the length => {length() =
     * this.endParam() - this.startParam()} Returns the complete length of the CurveSegment
     *
     * @return double
     * @version Implementation OK
     */
    ...
    /**
     * The method "isSimple" returns TRUE if this Object has no interior point of self-intersection
     * or self-tangency. In mathematical formalisms, this means that every point in the interior of
     * the object must have a metric neighborhood whose intersection with the object is isomorphic
     * to an n-sphere, where n is the dimension of this Object. Since most coordinate geometries are
     * represented, either directly or indirectly by functions from regions in Euclidean space of
     * their topological dimension, the easiest test for simplicity to require that a function exist
     * that is one-to-one and bicontinuous (continuous in both directions). Such a function is a
     * topological isomorphism. This test does not work for "closed" objects (that is, objects for
     * which the isCycle method returns TRUE). While Complexes contain only simple Objects,
     * non-simple Objects are often used in "spaghetti" data sets.
     *
