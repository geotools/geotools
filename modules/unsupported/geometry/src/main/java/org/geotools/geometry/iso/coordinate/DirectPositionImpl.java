...
public class DirectPositionImpl implements DirectPosition, Cloneable, Serializable {
    ...
    /**
     * The attribute "coordinate" is a sequence of Numbers that hold the coordinate of this position
     * in the specified reference system. DirectPosition2D::coordinate : Sequence<Number>
     *
     * <p>In order that this system will be based on an euclidic 2D system, the coordinates will be
     * stored in explicit variables for each X- and Y-Coordinate
     */
    ...
    public DirectPositionImpl clone() {
        ...
        // Return new DirectPosition by cloning the Coordinate array of double which define the
        // position
    ...
    public void setCoordinate(double[] coord) {
        if (coord.length != this.getDimension()) {
            throw new IllegalArgumentException(
                    "Index "
                            + coord.length
                            + " out of coordinate range (expected "
                            + getDimension()
                            + ")"); // $NON-NLS-1$
        }
        this.coordinate = coord;
    ...
    /**
     * Compares coodinates of DirectPosition Implementation Note: Parameter has to be of Type
    ...
    public boolean equals(Object o) {
        if (o instanceof DirectPosition) return this.equals((DirectPosition) o, 0);
        else if (o instanceof Position) return ((Position) o).equals(this);
        else return false;
    }
     ...
     *
     * @param values Array of doubles. values[0] will be added to the X ordinate, values[1] to the Y
     *     value and an optional values[2] to the Z value.
     */
    public void add(double[] values) {
        if (this.coordinate.length != values.length) throw new MismatchedDimensionException();
    ...
    public void add(DirectPositionImpl otherDP) {
        ...
        for (int i = 0; i < this.coordinate.length; i++) {
            if (Double.compare(this.coordinate[i], Double.NaN) == 0) {
                ...
                this.coordinate[i] =
                        DoubleOperation.add(this.coordinate[i], otherDP.getOrdinate(i));
            }
        }
    }
    ...
    // Auskommentiert, da die Methode auf einer nicht-robusten Methode scale-Methode von AlgoPointND
    // basiert.
    ...
    // TODO JR: Wenn du nichts dagegen hast, den folgenden teil bitte rausloeschen. solche methoden
    // werden in robuster form in jts angeboten.
