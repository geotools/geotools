 ...
 *
 * <p>The returned DirectPositions are pure wrappers over top of the array. The number of ordinates
 * used per each DirectPosition is based on the CRS. We start counting from the start position, in
...
public class DoublePointArray extends AbstractList<Position> implements PointArray, Serializable {
    ...
    */
    ...
    public DirectPosition getPosition(int index, DirectPosition position)
    ...
    public void setPosition(int index, DirectPosition position)
        ...
        // note: tempting to use System arraycopy on position.getCoordinates() but that would make
        // an temporary extra array
        int D = getDimension();
        for (int i = 0; i < D; i++) {
    ...
    public boolean equals(Object obj) {
        if (obj instanceof DoublePointArray) return this.equals((DoublePointArray) obj, 0);
        else return false;
    }
 ...
 *
 * <p>This class is private, althought not an inner class of DoublePointArray, this is done to allow
 * us to clone() in a safe manner.
 *
 * @author Jody Garnett
 */
class DoubleDirectPosition implements DirectPosition, Serializable {
    ...
    */
    ...
    public boolean equals(DirectPosition position, double tol) {
        ...
        for (int i = 0; i < D; ++i) {
            if (Double.isNaN(position.getOrdinate(i)) && Double.isNaN(array[index + i])) continue;
