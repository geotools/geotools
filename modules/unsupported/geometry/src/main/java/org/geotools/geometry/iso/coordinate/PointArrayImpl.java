...
/**
 * Many of the geometric constructs in this International Standard require the use of reference
 * points which are organized into sequences or grids (sequences of equal length sequences).
 * PointArray::column[1..n] : Position PointGrid::row[1..n] : PointArray
 *
 * <p>The class name follows the ISO19107. It is a confusing name, since it contains an array of
 * positions and not of points. The positions themselves contain either a direct position or a
 * point.
 *
...
public class PointArrayImpl extends ArrayList<Position> implements PointArray {
    ...
    public PointArrayImpl(List<Position> positions) {
        ...
        if (positions.size() == 0) {
            throw new IllegalArgumentException(
                    "Parameter positions is empty. Cannot create empty PointArray as we need the CRS");
        }
        crs = getDirectPosition(0).getDirectPosition().getCoordinateReferenceSystem();
    }
    ...
    /**
     * Returns the Feature Geometry Factory Instance based on the reference of the DirectPositions
     * of this PointArray
     *
     * @return Factory instance
     *     <p>private FeatGeomFactoryImpl getFeatGeomFactory() { if (this.isEmpty()) { return null;
     *     } DirectPositionImpl tDP = get(0).getDirectPosition(); return tDP.getGeometryFactory(); }
     */
    ...
    /**
     * Creates a LineSegment connecting the Positins p0 and p1, whereas p0 is the position at arg0
     * and p1 is the position at (arg0+1). If (arg0+1) == size() then returns the LineSegment
     * connecting the last position with the first one
     *
        ...
        public LineSegmentImpl get(int arg0) {
            ...
            for (int i = 1; i <= arg0; i++) {
                startParam =
                        DoubleOperation.add(
                                startParam,
                                AlgoPointND.getDistance(
                                        this.getStartCoordinate(i - 1),
                                        this.getEndCoordinate(i - 1)));
                // startParam += AlgoPointND.getDistance(this.getStartCoordinate(i-1),
                // this.getEndCoordinate(i-1));
            }
            LineSegmentImpl rSeg =
                    new LineSegmentImpl(
                            pointArray.getCoordinateReferenceSystem(), p0, p1, startParam);
    ...
    public DirectPosition getDirectPosition(int index, DirectPosition dest)
            throws IndexOutOfBoundsException {
        if (dest == null) {
