...
public class GeometryToWKTString {
    ...
    public String getString(Geometry geom) {
        ...
        if (geom instanceof Curve) rString += curveToString((Curve) geom);
        else if (geom instanceof Point) rString += pointToString((Point) geom);
        else if (geom instanceof Ring) rString += ringToString((Ring) geom);
        else if (geom instanceof SurfaceBoundary)
            rString += surfaceBoundaryToString((SurfaceBoundary) geom);
        else if (geom instanceof Surface) rString += surfaceToString((Surface) geom);
    ...
    private String multiPrimitiveToString(MultiPrimitive mp) {
        if (mp instanceof MultiPoint) return multiPointToString((MultiPoint) mp);
        else if (mp instanceof MultiCurve) return multiCurveToString((MultiCurve) mp);
        else if (mp instanceof MultiSurface) return multiSurfaceToString((MultiSurface) mp);
        else return "MultiPrimitive(" + this.multiPrimitiveCoordToString(mp) + ")";
    }
    ...
    private String ringCoordToString(Ring r) {
        ...
        String rString =
                directPositionToString(
                        ((Curve) orientableCurves.iterator().next()).getStartPoint());
    ...
    private String multiSurfaceCoordToString(MultiSurface mc) {
        ...
        String rString =
                "("
                        + this.surfaceBoundaryCoordToString(
                                (SurfaceBoundary) ((Surface) mpIter.next()).getBoundary())
                        + ")";
        while (mpIter.hasNext()) {
            if (this.lineBreak) {
                rString += "\n\t";
    ...
    private String multiPrimitiveCoordToString(MultiPrimitive mp) {
        ...
        while (primitives.hasNext()) {
            ...
            if (p instanceof Point) rString += "\n\t" + pointToString((Point) p);
            else if (p instanceof Curve) rString += "\n\t" + curveToString((Curve) p);
            else if (p instanceof Surface) rString += "\n\t" + surfaceToString((Surface) p);
