...
/**
 * Primitive is the abstract root class of the geometric primitives. Its main purpose is to define
 * the basic "boundary" operation that ties the primitives in each dimension together. A geometric
 * primitive is a geometric object that is not decomposed further into other primitives in the
 * system. This includes curves and surfaces, even though they are composed of curve segments and
 * surface patches, respectively. This composition is a strong aggregation: curve segments and
 * surface patches cannot exist outside the context of a primitive.
 *
 * <p>NOTE Most geometric primitives are decomposable infinitely many times. Adding a centre point
 * to a line may split that line into two separate lines. A new curve drawn across a surface may
 * divide that surface into two parts, each of which is a surface. This is the reason that the
 * normal definition of primitive as "non-decomposable" is not plausible in a geometry model - the
 * only non-decomposable object in geometry is a point. Any geometric object that is used to
 * describe a feature is a collection of geometric primitives. A collection of geometric primitives
 * may or may not be a geometric complex. Geometric complexes have additional properties such as
 * closure by boundary operations and mutually exclusive component parts.
 *
 * <p>Primitive and Complex share most semantics, in the meaning of operations, attributes and
 * associations. There is an exception in that a Primitive shall not contain its boundary (except in
 * the trivial case of Point where the boundary is empty), while a Complex shall contain its
 * boundary in all cases. This means that if an instantiated object implements Object operations
 * both as Primitive and as a Complex, the semantics of each set theoretic operation is determined
 * by the its name resolution. Specifically, for a particular object such as CompositeCurve,
 * Primitive::contains (returns FALSE for end points) is different from Complex::contains (returns
...
public abstract class PrimitiveImpl extends GeometryImpl implements Primitive {
    ...
    /**
     * The "Interior to" association associates Primitives which are by definition coincident with
     * one another. This allows applications to override the Set<DirectPosition> interpretation and
     * its associated computational geometry, and declare one Primitive to be "interior to" another.
     * This association should normally be empty when the Primitives are within a Complex, since in
     * that case the boundary information is sufficient for most cases.
     *
     * <p>Primitive::coincidentSubelement [0..n] : Reference<Primitive> Primitive::superElement
     * [0..n] : Reference<Primitive>
     *
     * <p>This association is constrained by the set theory operators and dimension operators
     * defined at Object.
     *
     * <p>Primitive: superElement=>includes(p: Primitive) = Object::contains (p) dimension() >=
     * coincidentSubelement. dimension()
     *
     * <p>NOTE This association should not be used when the two Primitives are not close to one
     ...
     *
     * <p>The fields containedPrimitive and containingPrimitive are unset per default
     */
    ...
    /**
     * A Primitive may be in several Complexes. This association may not be navigable in this
     * direction (from primitive to complex), depending on the application schema.
     *
     * <p>Primitive::complex [0..n] : Reference<Complex>
     *
     * <p>The field complex is unset per default
     */
    ...
    public Set<Complex> getMaximalComplex() {
        if (this.complex == null) return null;
