...
/**
 * A lightweight class used to store coordinates on the 2-dimensional Cartesian plane. It is
 * distinct from <code>Point</code>, which is a subclass of <code>Geometry</code> . Unlike objects
 * of type <code>Point</code> (which contain additional information such as an envelope, a precision
 * model, and spatial reference system information), a <code>Coordinate</code> only contains
 * ordinate values and accessor methods.
 *
 * <p><code>Coordinate</code>s are two-dimensional points, with an additional z-ordinate. JTS does
 * not support any operations on the z-ordinate except the basic accessor functions. Constructed
...
public class Coordinate implements Comparable<Coordinate>, Cloneable, Serializable {
    ...
    /**
     * SJ Creates a Coordinate of coordinate dimension 2 or 3 defined by an array of <code>double
     * </code>
     *
     ...
     * @return <code>true</code> if the x- and y-coordinates are equal; the z-coordinates do not
     *     have to be equal.
     */
    ...
    /**
     * Returns <code>true</code> if <code>other</code> has the same values for the x and y
     * ordinates. Since Coordinates are 2.5D, this routine ignores the z value when making the
     * comparison.
     *
    ...
    /**
     * Returns <code>true</code> if <code>other</code> has the same values for x, y and z.
     *
    ...
    /**
     * Returns a hash code for a double value, using the algorithm from Joshua Bloch's book
     * <i>Effective Java"</i>
     */
