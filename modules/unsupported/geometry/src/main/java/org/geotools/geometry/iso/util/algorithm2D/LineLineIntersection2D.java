...
public class LineLineIntersection2D {
    ...
    public void setValues(Line2D line0, Line2D line1) {
        ...
        if (n == 4) {
            /**
             * the segments are colinear and their interiors may overlap. The segments are equal if
             * the four itscs are real numbers, and in this case the four itscs must be 0.0 or 1.0.
             * The second possibility are two real numbers and two Double.NaN. In this case, the
             * interiors of the segments intersect
             */
            int count = 0;
    ...
    public boolean isCoincident() {
        /**
         * the segments are coincident if they overlap and the intersection itscs are 0.0 or 1.0.
         * This implies no NaN in the itscs
         */
        return (itsc != null)
                && (itsc.length == 4)
                && !isNaN(itsc[0])
    ...
    public boolean overlapsInterior() {
        /**
         * the interiors of the segments overlap if at least one of the four itsc is less then 1.0
         * and greater then 0.0
         */
        return ((itsc != null)
                && (itsc.length == 4)
    ...
    public boolean intersectInteriorInteriorPoint() {
        /**
         * the interiors of the segments intersect at a node if both itscs are real number not equal
         * 0.0 and not equal 1.0
         */
        if ((itsc == null) || (itsc.length != 2)) return false;
    ...
    public boolean isII() {
        /**
         * the interiors of the segments intersect if they intersect at one node or if their
         * interior overlap number and two itscs are Double.NaN
         */
    ...
    public boolean isIB() {
        /**
         * the interior of the line0 intersects the boundary of line1 if they have two itscs and the
         * first itsc is a real number between 0.0 and 1.0 and the second itsc is a real number
         * equal 0.0 or 1.0
         *
         * <p>(itsc[0]>0.0 and itsc[0]<1.0) and (itsc[1]==0.0 or itsc[1]==1.0)
         */
        return ((itsc != null)
                && (itsc.length == 2)
                && !isNaN(itsc[0])
    ...
    public boolean isBI() {
        /**
         * the boundary of the line0 intersects the interior of line1 if they have two itscs and the
         * first itsc is a real number between equal 0.0 or 1.0 and the second itsc is a real number
         * between 0.0 and 1.0
         *
         * <p>(itsc[0]==0.0 and itsc[0]==1.0) and (itsc[1]>0.0 or itsc[1]<1.0)
         */
    ...
    public boolean isBB() {
        /**
         * the boundary of both lines intersect if they have two or four itscs and the first itsc is
         * a real number equal 0.0 or 1.0 and the second itsc is a real itsc not equal 0.0 or 1.0
         *
         * <p>itsc[0]==0.0 or itsc[0]==1.0 itsc[1]!=0.0 or itsc[1]!=1.0
         */
    ...
    public Point2D[] intersectionPoint() {
        /**
         * returns one intersection node for a 0-d intersection or two intersection points for a 1-d
         * intersection (overlaping)
         */
        Point2D[] result = null;
        if (itsc == null) return null;
            ...
            if (count == 0) {
                ...
                // (q0,q1) *------------->*   *--------->*       *------------->*
                // *-------->*
                // result: (0.0,NaN,0.0,0.8)  (0.0,0.8,0.0,NaN)  (NaN,1.0,0.4,1.0)
                // (0.3,1.0,Nan,1.0)
    ...
    //        } else if ( itsc.length == 4 ) {
    //            if ( !java.lang.Double.isNaN(itsc[0]) )  itsc[0] = itsc[0] *
    // AlgoLine.length(line0);
    //            if ( !java.lang.Double.isNaN(itsc[1]) )  itsc[1] = itsc[1] *
    // AlgoLine.length(line0);
    //            if ( !java.lang.Double.isNaN(itsc[2]) )  itsc[2] = itsc[2] *
    // AlgoLine.length(line1);
    //            if ( !java.lang.Double.isNaN(itsc[3]) )  itsc[3] = itsc[3] *
    // AlgoLine.length(line1);
    //        }
    //        return null;
    //    }
