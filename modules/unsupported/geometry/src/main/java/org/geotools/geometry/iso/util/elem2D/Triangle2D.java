...
public class Triangle2D extends Simplex2D {
    ...
    public double getRadiusU() {
        ...
        Point2D pm =
                new Point2D.Double(
                        ((0.5 * p1.getX()) + (0.5 * p0.getX())),
                        ((0.5 * p1.getY()) + (0.5 * p0.getY())));
        double c =
                Math.sqrt(
                        (pm.getX() * pm.getX())
                                - (2 * (pm.getX() * p2.getX()))
        ...
        double angle =
                Math.acos(
                        ((p0_1.getX() * p0_2.getX())
                                / (Math.sqrt(Math.pow(p0_1.getX(), 2) + Math.pow(p0_1.getY(), 2))
                                        * Math.sqrt((p0_2.getX() * p0_2.getX())))));
        if (p1_1.getY() > 0) {
            angle = ((2 * Math.PI) - angle);
        }
        Point2D p2_2 =
                new Point2D.Double(
                        (Math.cos(angle) * (p2.getX() - pm.getX()))
    ...
    /**
     * return true if the interiors of both simplices intersect valid only for counterclockwise
     * simplices
     */
