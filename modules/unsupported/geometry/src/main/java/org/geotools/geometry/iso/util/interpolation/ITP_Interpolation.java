...
public class ITP_Interpolation {
    ...
    public static boolean shepardModified(
            ArrayList<Point3d> pIn, ArrayList<Point3d> pOut, double q, int nnbr) {
    ...
    private static void shepardModified(
            ArrayList<PointNeighboursGradients> pngColl, double q, Point3d p_eval) {
        ...
        for (int i = 0; i < pngColl.size(); ++i) {
            ...
            if (q == 2) {
                dist[i] =
                        (p_eval.x - png0.mP.x) * (p_eval.x - png0.mP.x)
        ...
        for (int i = 0; i < pngColl.size(); ++i) {
            ...
            sum0 +=
                    (png0.mP.z + AlgoPoint2D.scalar(AlgoPoint2D.subtract(p0, p1), png0.mGradXY))
                            * prod;
            sum1 += prod;
        }
        p_eval.z = (sum0 / sum1);
