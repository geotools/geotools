...
/**
 * This TestSuite picks up each JTS test and applies it to the provided Geometry*Factory.
 *
 * <p>If an accompanying .properties file is found, the following entries may apply: -
 * disabled=true: eliminates all tests in the xml file - description=skipped: skips the "case" entry
 * with that description - description=intersection|skipped: skips the intersection operation for
 * that description. - description=intersection|WKT TEXT: replaces the expectedResult value with the
 * WKT text. - description=intersection|boundary: for the case of multiprimitives, we need to
 * convert the resulting geometry to a boundary to compare with - description=intersection|point:
 * for the case of points, we need to convert the resulting geometry to a point instead of a
 * position to compare with - description=intersection|skipped|union|skipped: skips both the
...
public class GeometryConformanceTestSuite extends TestSuite {
    ...
    public static Test suite() {
        ...
        try {
            dir =
                    TestData.file(GeometryConformanceTestSuite.class, "LineTests.xml")
                            .getParentFile();
