...
public final class CRS2GeoTiffMetadataAdapter {
    ...
    private void parseLinearUnit(
        ...
        if (linearUnit != null && !SI.METER.isCompatible(linearUnit)) {
            throw new IllegalArgumentException(
                    Errors.format(ErrorKeys.NON_LINEAR_UNIT_$1, linearUnit));
        }
    ...
    private void parseGeoGCS(
        ...
        final Unit angularUnit =
                ((EllipsoidalCS) geographicCRS.getCoordinateSystem()).getAxis(0).getUnit();
    ...
    private void parseDatum(final DefaultGeodeticDatum datum, GeoTiffIIOMetadataEncoder metadata) {
        ...
        metadata.addGeoShortParam(GeoTiffGCSCodes.GeogGeodeticDatumGeoKey, 32767);
    ...
    private void parseUnit(Unit unit, int model, GeoTiffIIOMetadataEncoder metadata) {
        ...
        if (base != null) {
            metadata.addGeoDoubleParam(
                    ...
                    unit.getConverterTo(base).convert(1));
        } else
            metadata.addGeoDoubleParam(
                    model == 0
                            ? GeoTiffGCSCodes.GeogAngularUnitSizeGeoKey
