...
public final class GeoTiffMetadata2CRSAdapter {
     ...
     * {@link GeoTiffConstants#RasterPixelIsArea} pixel interpretation in order to convert the
     * transformation back to using the {@link GeoTiffConstants#RasterPixelIsPoint} convention which
     * is the one OGC requires for coverage.
     */
    ...
    public GeoTiffMetadata2CRSAdapter(Hints hints) {
        ...
        allAuthoritiesFactory =
                hints != null
                        ? new AllAuthoritiesFactory(this.hints)
    ...
    public CoordinateReferenceSystem createCoordinateSystem(
            final GeoTiffIIOMetadataDecoder metadata) throws IOException, FactoryException {
     ...
     * @throws IOException
     */
    private GeographicCRS createGeographicCoordinateSystem(final GeoTiffIIOMetadataDecoder metadata)
            throws IOException {
    ...
    /**
     * Create the grid to world (or raster to model) transformation for this sourceFile respecting
            ...
            if (pmCode != null) {
                if (pmCode.equals(GeoTiffConstants.GTUserDefinedGeoKey_String)) {
                    try {
                        ...
                        props.put(
                                "name",
                                (name != null) ? name : "User Defined GEOTIFF Prime Meridian");
    ...
    private GeodeticDatum createGeodeticDatum(
        ...
        if (datumCode.equals(GeoTiffConstants.GTUserDefinedGeoKey_String)) {
            ...
            final String datumName =
                    (metadata.getGeoKey(GeoTiffGCSCodes.GeogCitationGeoKey) != null
                            ? metadata.getGeoKey(GeoTiffGCSCodes.GeogCitationGeoKey)
                            : "unnamed");
            ...
            try {
                datum =
                        (GeodeticDatum)
                                (this.allAuthoritiesFactory.createDatum(
                                        new StringBuffer("EPSG:").append(datumCode).toString()));
            } catch (FactoryException fe) {
        ...
        try {
            ...
            if (name.equalsIgnoreCase("mercator_1SP")
                ...
                final double standard_parallel_1 =
                        this.getGeoKeyAsDouble(GeoTiffPCSCodes.ProjStdParallel1GeoKey, metadata);
    ...
    /**
     * This code creates an <code>javax.Units.Unit</code> object out of the <code>
    ...
    private Unit createUnit(
        ...
        if (unitCode.equals(GeoTiffConstants.GTUserDefinedGeoKey_String)) {
            ...
            try {
                ...
                return (Unit)
                        this.allAuthoritiesFactory.createUnit(
                                new StringBuffer("EPSG:").append(unitCode).toString());
            } catch (FactoryException fe) {
                final IOException io = new GeoTiffException(metadata, fe.getLocalizedMessage(), fe);
                throw io;
            }
