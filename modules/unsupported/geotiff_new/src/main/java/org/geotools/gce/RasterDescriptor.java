...
/**
 * A RasterDescriptor is an elementar piece of data image, with its own levels and everything.
 *
 ...
 * file with embedded levels, either explicit or intrinsic through wavelets like MrSID, ECW or
 * JPEG2000.
 *
...
public class RasterDescriptor {
    ...
    private static final Logger LOGGER =
            org.geotools.util.logging.Logging.getLogger(RasterDescriptor.class);
    ...
    public class RasterLevelDescriptor {
        ...
        public String toString() {
            return "RasterLevelDescriptor [source="
                    + source
                    + ", scaleX="
                    + scaleX
                    + ", scaleY="
    ...
    //        // get basic info
    //        this.rasterBBOX = new
    // ReferencedEnvelope(rasterManager.spatialDomainManager.coverageBBox);
    //        this.baseGridToWorld=new AffineTransform((AffineTransform)
    ...
    //                                    ImageReaderSource.wrapFile(i,granuleFile, streamSPI,
    // GeoTiffUtils.TIFFREADERFACTORY),
    //                                    1,
    //                                    1,
    //                                    new RasterLayout(0, 0, width,
    // height,reader.getTileGridXOffset(i),reader.getTileGridYOffset(i),reader.getTileWidth(i),reader.getTileHeight(i))
    //                                    )
    //                            );
    //                }
    //                else {
    ...
    //                // get a stream
    //                inStream =
    // rasterManager.parent.ovrInStreamSPI.createInputStreamInstance(rasterManager.parent.ovrSource,
    ...
    //                            new RasterLevelDescriptor(
    //
    // ImageReaderSource.wrapFile(k,rasterManager.parent.ovrSource,
    // rasterManager.parent.ovrInStreamSPI, GeoTiffUtils.TIFFREADERFACTORY),
    //                                    scaleX,
    //                                    scaleY,
    //                                    new RasterLayout(0, 0, width,
    // height,reader.getTileGridXOffset(k),reader.getTileGridYOffset(k),reader.getTileWidth(k),reader.getTileHeight(k))
    //                                    )
    //                            );
    //                }
    //
    ...
    public RasterDescriptor(final List<RasterSlice> slices) {
        ...
        this.levels.add(
                new RasterLevelDescriptor(fullRes.source, 1.0, 1.0, fullRes.rasterDimensions));

        final int baseLevelWidth = fullRes.rasterDimensions.getWidth(),
                baseLevelHeight = fullRes.rasterDimensions.getHeight();
        ...
        for (int i = 1; i < size; i++) {
            ...
            this.levels.add(
                    new RasterLevelDescriptor(
                            slice.source, scaleX, scaleY, slice.rasterDimensions));
        }
    }
    ...
    //    private RasterLevelDescriptor getLevel(ImageReader reader, RasterLayerRequest request,
    // int imageChoice, int overviewIndex) {
    //        synchronized (levels) {
    ...
    //                    //
    //                    // get selected level and base level dimensions
    //                    final Rectangle levelDimension =  new Rectangle(0, 0,
    // reader.getWidth(imageChoice), reader.getHeight(imageChoice));
    //
    ...
    //
    //                    // add the base level
    //                    final RasterLevelDescriptor newLevel = new RasterLevelDescriptor(scaleX,
    // scaleY, levelDimension.width,levelDimension.height);
    ...
    public String toString() {
        ...
        buffer.append("Description of a rasterGranuleLoader ")
                .append("\n")
                .append("BBOX:\t\t")
                .append(rasterBBOX.toString())
                .append("gridToWorld:\t\t")
                .append(baseGridToWorld);
        int i = 1;
        for (final RasterLevelDescriptor rasterLevelDescriptor : levels) {
            i++;
            buffer.append("Description of level ")
                    .append(i++)
                    .append("\n")
                    .append(rasterLevelDescriptor.toString())
                    .append("\n");
        }
