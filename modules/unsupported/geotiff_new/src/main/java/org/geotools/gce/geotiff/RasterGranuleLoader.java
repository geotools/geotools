 ...
 * 
 * <p>
 * Right now we are making the assumption that a single rasterGranuleLoader is made a by a
...
class RasterGranuleLoader {
	...
	class Level{
		...
		public String toString() {
			...
			buffer.append("levelToBaseTransform:\t\t").append(levelToBaseTransform.toString()).append("\n");
		...
		try {
			...
			if(reader==null)
			{
				if (LOGGER.isLoggable(java.util.logging.Level.WARNING))
					LOGGER.warning("Unable to get reader for rasterGranuleLoader "+this.toString()+ " with request "+request.toString());
				return null;
			}
			...
			final Rectangle sourceAreaWithCollar= (Rectangle) sourceArea.clone();
			sourceAreaWithCollar.grow((int)(sourceArea.width*0.05),(int)( sourceArea.height*0.05));
