...
public class TeradataGISDialect extends BasicSQLDialect {
    ...
    private WKBAttributeIO getWkbReader(GeometryFactory factory) {
        ...
        if (reader == null) {
            GeometryFactory geometryFactory = factory == null ? dataStore.getGeometryFactory()
                    : factory;
    ...
    public void encodeGeometryColumn(GeometryDescriptor gatt, int srid, StringBuffer sql) {
        ...
        if (mLastGeomColumns != null) {
            mLastGeomColumns.add(gatt.getLocalName());
        }
    }
        ...
        try {
            ...
            for (AttributeDescriptor att : featureType.getAttributeDescriptors()) {
                if (att instanceof GeometryDescriptor) {
                    ...
                    try {
                        Integer result = CRS
                                .lookupEpsgCode(gd.getCoordinateReferenceSystem(), true);
                        if (result != null) {
                    ...
                    try {
                        sql = MessageFormat.format("DROP INDEX \"{0}_{1}_mi\"", tableName, gd
                                .getLocalName());
                        LOGGER.fine(sql);
                        st.execute(sql);
                    } catch (Exception e) {
                    ...
                    try {
                        sql = MessageFormat.format("DROP INDEX \"{0}_{1}_mu\"", tableName, gd
                                .getLocalName());
                        LOGGER.fine(sql);
                        st.execute(sql);
                    } catch (Exception e) {
                    ...
                    try {
                        sql = MessageFormat.format("DROP INDEX \"{0}_{1}_md\"", tableName, gd
                                .getLocalName());
                        LOGGER.fine(sql);
                        st.execute(sql);
                    } catch (Exception e) {
                    ...
                    // create spatial index if we have one key
                    PrimaryKey keys = dataStore.getPrimaryKeyFinder().getPrimaryKey(dataStore,
                            schemaName, tableName, cx);
                    List<PrimaryKeyColumn> colsKey = keys.getColumns();
                    if (colsKey.size() == 1) {
                        PrimaryKeyColumn colKey = colsKey.get(0);
                        String key = colKey.getName();

                        // add the spatial index table
                        ...
                        // only if on not autogenerate key !
                        if (!(colKey instanceof AutoGeneratedPrimaryKeyColumn)) {
                            sql = MessageFormat
                                    .format(
                                            "CREATE TRIGGER \"{0}_{1}_mi\" AFTER INSERT ON {12}"
                                                    ...
                                                    // +
                                                    // "      WHEN (\"{1}\" IS NULL) THEN 0 ELSE sysspatial.tessellate_index("
                                                    // +
                                                    // "      \"{1}\" IS NOT NULL AND sysspatial.tessellate_index("
                            ...
                            // +
                            // "      {7,number,0}, {8,number,0}, {9,number,0}, {10,number,0.0#}, {11,number,0})"
                            // + "  ); ", tableName, gd.getLocalName(), key,
                            // u_xmin, u_ymin, u_xmax, u_ymax, g_nx, g_ny,
                            // levels, scale, shift,
    ...
    public void encodeValue(Object value, Class type, StringBuffer sql) {
        if (byte[].class.equals(type)) {
            // escape the into byte array representation
            StringBuffer sb = new StringBuffer();
    ...
    public void encodeTableName(String raw, StringBuffer sql) {
        ...
        mLastTableName = raw;
        mLastGeomColumns = new HashSet<String>();
    }
    ...
    public void initializeConnection(Connection cx) throws SQLException {
        ...
        if (mIndexTables == null) {
            ...
            ResultSet tables = cx.getMetaData()
                    .getTables("", "", "%_idx", new String[] { "TABLE" });
            while (tables.next()) {
                mIndexTables.add(tables.getString(3));
            }
        }
        mCurrentConnection = cx;
    }

    protected String getPrimaryKeyName(String schemaName, String tableName) throws SQLException {
        try {
            PrimaryKey keys = dataStore.getPrimaryKeyFinder().getPrimaryKey(dataStore, schemaName,
                    tableName, mCurrentConnection);
            if (keys != null) {
                List<PrimaryKeyColumn> colsKey = keys.getColumns();
                if (colsKey.size() == 1) {
                    PrimaryKeyColumn colKey = colsKey.get(0);
    ...
    protected void createIndex(int id, Statement stmt) throws SQLException {
            ...
            if (mIndexTables.contains(indexTableName)) {
                try {
                    String sql = MessageFormat.format(
                            "DELETE FROM {0} WHERE \"{1}\" = {2,number,0};", encodedIdxTableName,
                            key, id);
                    ...
                    sql = MessageFormat
                            .format(
                                    "INSERT INTO {13} (id, cellid) VALUES ({14,number,0}, SELECT "
                                            ...
                                            + "  ) " + "END", mLastTableName, geomColumn, key,
                                    u_xmin, u_ymin, u_xmax, u_ymax, g_nx, g_ny, levels, scale,
                                    shift, encodedTableName, encodedIdxTableName, id);
