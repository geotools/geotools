		...
		try {
			...
			for (AttributeDescriptor att : featureType
				...
				if (att instanceof GeometryDescriptor) {
					...
					try {
						sql = MessageFormat.format("DROP INDEX \"{0}_{1}_mi\"", tableName, gd.getLocalName());
						LOGGER.fine(sql);
						st.execute(sql);
					}
					catch (Exception e) {
					...
					try {
						sql = MessageFormat.format("DROP INDEX \"{0}_{1}_mu\"", tableName, gd.getLocalName());
						LOGGER.fine(sql);
						st.execute(sql);
					}
					catch (Exception e) {
					...
					try {
						sql = MessageFormat.format("DROP INDEX \"{0}_{1}_md\"", tableName, gd.getLocalName());
						LOGGER.fine(sql);
						st.execute(sql);
					}
					catch (Exception e) {
					...
					PrimaryKey keys = dataStore.getPrimaryKeyFinder().getPrimaryKey(dataStore, schemaName, tableName, cx);
...
//									+ "      {3,number,0.0#}, {4,number,0.0#}, {5,number,0.0#}, {6,number,0.0#}, "
//									+ "      {7,number,0}, {8,number,0}, {9,number,0}, {10,number,0.0#}, {11,number,0})"
//									+ "  ); ", tableName, gd.getLocalName(), key, u_xmin, u_ymin, u_xmax, u_ymax, g_nx, g_ny, levels, scale, shift,
//									encodedTableName, encodedIdxTableName);
		//					sql = MessageFormat.format("CREATE TRIGGER \"{0}_{1}_mi\" AFTER INSERT ON {12}"
		//							+ "  REFERENCING NEW AS nt"
		//							+ "  FOR EACH ROW"
...
//			sql.append("SYSSPATIAL.ST_GeomFromText('" + value.toText() + "', " + srid + ")");
    ...
    protected String getPrimaryKeyName(String schemaName, String tableName) throws SQLException {
    	try {
			PrimaryKey keys = dataStore.getPrimaryKeyFinder().getPrimaryKey(dataStore, schemaName, tableName, mCurrentConnection);
			if (keys != null) {
				List<PrimaryKeyColumn> colsKey = keys.getColumns(); 
				if (colsKey.size() == 1) {
					PrimaryKeyColumn colKey = colsKey.get(0);
