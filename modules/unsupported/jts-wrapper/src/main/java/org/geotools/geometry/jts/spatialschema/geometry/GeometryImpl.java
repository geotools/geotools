...
public abstract class GeometryImpl implements Geometry, Serializable, Cloneable, JTSGeometry {
    ...
    /**
     * The JTS equivalent of this geometry. This gets set to null whenever we make changes to the
     * geometry so that we can recompute it.
     */
    private org.locationtech.jts.geom.Geometry jtsPeer;

    /**
     * If this object is part of a composite, this this member should hold a pointer to that
     * composite so that when our JTS geometry is invalidated, we can also invalidate that of our
     * parent.
     */
     ...
     */
    public GeometryImpl(
            final CoordinateReferenceSystem coordinateReferenceSystem, boolean mutable) {
        ...
        if (d == 0) {
            ...
            Ring exteriorRing =
                    JTSUtils.linearRingToRing(
                            (org.locationtech.jts.geom.LineString) mls.getGeometryN(0), crs);
            Ring[] interiorRings = new Ring[n - 1];
            for (int i = 1; i < n; i++) {
                interiorRings[n - 1] =
                        JTSUtils.linearRingToRing(
                                (org.locationtech.jts.geom.LineString) mls.getGeometryN(i), crs);
            }
    ...
    /**
     * Returns the distance between the given geometry and this geometry. Note that this distance is
     * in units the same as the units of the coordinate reference system, and thus may not have any
     * physical meaning (such as when the coordinate system is a latitude/longitude system).
     */
    ...
    public final int getDimension(final DirectPosition point) {
        org.locationtech.jts.geom.Geometry jtsGeom = getJTSGeometry();
        if (jtsGeom instanceof org.locationtech.jts.geom.GeometryCollection) {
            org.locationtech.jts.geom.Point p = JTSUtils.directPositionToPoint(point);
    ...
    /**
     * Returns the dimension of the coordinates in this geometry. This delegates to the coordinate
     * reference system, so it may throw a null pointer exception if this geometry has no coordinate
     * reference system.
     */
    ...
    /**
     * This impementation of geometry does not support traversing this association in this direction
     * as it would require every geometry to know about all of the larger geometries of which it is
     * a part. This would add some memory usage and bookkeeping headaches for functionality that
    ...
    /**
     * Attempts to find a transform from the current CRS to the new CRS and creates a new geometry
     * by invoking that transform on each control point of this geometry.
     */
        ...
        Envelope result =
                new EnvelopeImpl(
                        new DirectPositionImpl(
                                crs, new double[] {jtsEnv.getMinX(), jtsEnv.getMinY()}),
                        new DirectPositionImpl(
                                crs, new double[] {jtsEnv.getMaxX(), jtsEnv.getMaxY()}));
        return result;
    }
    ...
    public final DirectPosition getCentroid() {
        ...
        return JTSUtils.pointToDirectPosition(jtsCentroid, getCoordinateReferenceSystem());
    }
    ...
    /**
     * Creates an immutable copy of this object or just returns this object if it's already
     * immutable.
     */
    ...
    /**
     * Returns a deep copy of this geometric object. Subclasses must override to make deep copies of
     * members that are themselves mutable objects. Note that all of the (private) members of
     * GeometryImpl are already immutable so this method simply delegates to the superclass (Object)
     * clone.
     */
    ...
    /**
     * Returns true if the given position lies in this geometry within the tolerance of the floating
     * point representation.
     */
    public boolean contains(DirectPosition point) {
        org.locationtech.jts.geom.Geometry jtsGeom1 = getJTSGeometry();
        org.locationtech.jts.geom.Geometry jtsGeom2 = JTSUtils.directPositionToPoint(point);
    ...
    public TransfiniteSet difference(TransfiniteSet pointSet) {
        org.locationtech.jts.geom.Geometry jtsGeom1 = getJTSGeometry();
        org.locationtech.jts.geom.Geometry jtsGeom2 = ((JTSGeometry) pointSet).getJTSGeometry();
        return JTSUtils.jtsToGo1(
                JTSUtils.difference(jtsGeom1, jtsGeom2), getCoordinateReferenceSystem());
    }
    ...
    public TransfiniteSet intersection(TransfiniteSet pointSet) {
        ...
        org.locationtech.jts.geom.Geometry jtsGeom2 = ((JTSGeometry) pointSet).getJTSGeometry();
        return JTSUtils.jtsToGo1(
                JTSUtils.intersection(jtsGeom1, jtsGeom2), getCoordinateReferenceSystem());
    }
    ...
    public TransfiniteSet symmetricDifference(TransfiniteSet pointSet) {
        ...
        org.locationtech.jts.geom.Geometry jtsGeom2 = ((JTSGeometry) pointSet).getJTSGeometry();
        return JTSUtils.jtsToGo1(
                JTSUtils.symmetricDifference(jtsGeom1, jtsGeom2), getCoordinateReferenceSystem());
    }
    ...
    public TransfiniteSet union(TransfiniteSet pointSet) {
        ...
        org.locationtech.jts.geom.Geometry jtsGeom2 = ((JTSGeometry) pointSet).getJTSGeometry();
        return JTSUtils.jtsToGo1(
                JTSUtils.union(jtsGeom1, jtsGeom2), getCoordinateReferenceSystem());
    }
