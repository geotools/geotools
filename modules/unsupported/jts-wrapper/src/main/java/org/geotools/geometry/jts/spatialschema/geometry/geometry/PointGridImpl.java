...
/**
 * A grid of points. The grid may be see as a sequences of equal length sequences. @UML datatype
 * GM_PointGrid
 *
...
public class PointGridImpl implements PointGrid {
    ...
    /**
     * Returns the length of this array. This is equivalent to {@code getRows().size()}.
     *
    ...
    /**
     * Returns the point at the given index. This is equivalent to {@code getRow(row).get(column)}.
     *
     ...
     * @throws IndexOutOfBoundsException if an index is out of bounds.
     */
    public DirectPosition get(int row, int column, DirectPosition dest)
            throws IndexOutOfBoundsException {
        DirectPosition target = new DirectPositionImpl(getInternal(row, column));

        if (dest == null
                || !dest.getCoordinateReferenceSystem()
                        .equals(target.getCoordinateReferenceSystem())) {
            return target;
        }
     ...
     */
    public void set(int row, int column, DirectPosition position)
     ...
     *
     * @return The rows in this array. @UML mandatory row
