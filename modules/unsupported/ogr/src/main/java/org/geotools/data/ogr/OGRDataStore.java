...
public class OGRDataStore extends ContentDataStore {
     ...
     * This call is specifically made available for the OGC store since for some data source types, such
     * as GML or KML, it is not possible to call createSchema() independently from a write, as
     * the result will not contain the schema definition without having data too.
     * 
     * @param schema the geotools schema
     * @param approximateFields if true, OGR will try to create fields that are approximations of
     *        the required ones when an exact match cannt be provided
