...
public class TPKFile {
    ...
    /**
     * Using the highest zoom level contained in the TPK file use the tile coverage to calculate the geographical bounds
     * of the map
     *
     ...
     *
     * <p>Populates imageFormat, creates and poulates zoomLevelMapping and zoomLevelResolutionMap hashmaps
     *
    ...
    /**
     * parse LODInfo/LevelId and LODInfo/Resolution in order to map the nominal LevelID value to an actual zoom level
     *
    ...
    private void parseLodInfo(Node lodInfo) {
        if (lodInfo.getNodeType() == Node.ELEMENT_NODE) {
            ...
            if (lid.getNodeType() == Node.ELEMENT_NODE && res.getNodeType() == Node.ELEMENT_NODE) {
                ...
                long zoom_level = Math.round(log2(WORLD_CIRCUMFERENCE / (resolution * TILE_PIXEL_SIZE)));
    ...
    /**
     * Iterate over the Zoom Levels contained in the TPK archive and build a TPKZoomLevel object for each
     *
     * <p>The TPKZoomLevel object caches control information about the zoom level and each bundle that comprises the
     * zoom level. Access to individual tile data is done via this object.
     */
    private void loadZoomLevels() {
        ...
        for (Long levelId : zoomLevelMapping.keySet()) {
            ...
            List<String> bundles = zipEntryMap.keySet().stream()
            ...
            if (cacheType == CacheType.V1) { // V2 caches don't have independent indexes
                indexes = zipEntryMap.keySet().stream()
                        .filter(s -> s.contains(levelFolder))
    ...
    /**
     * Using the supplied TPKZoomLevel object calculate the geographical bounds of the map using the zoom level's
     * minColumn, maxColumn, minRow, maxRow and resolution value
     *
