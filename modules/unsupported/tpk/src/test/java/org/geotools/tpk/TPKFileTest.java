...
public class TPKFileTest {
    ...
    @Test
    public void testReadCompactCacheV3() {
        Map<Long, TPKZoomLevel> zoomLevelMap = new HashMap<>();
        File theTpk = URLs.urlToFile(getClass().getResource("sample_v3.tpk"));
        TPKFile theTPKFile = new TPKFile(theTpk, zoomLevelMap, Charset.forName("GBK"));
        assertEquals(7, zoomLevelMap.size());
        assertEquals(0, theTPKFile.getMinZoomLevel());
        assertEquals(6, theTPKFile.getMaxZoomLevel());

        // get the bounds computed from highest zoom level coverage
        Bounds bounds = theTPKFile.getBounds();

        // minimum longitude
        assertEquals(106.874999, bounds.getMinimum(0), 0.000001);
        // maximum longitude
        assertEquals(123.749999, bounds.getMaximum(0), 0.000001);

        // min latitude
        assertEquals(27.059125, bounds.getMinimum(1), 0.00001);
        // max latitude
        assertEquals(36.597889, bounds.getMaximum(1), 0.00001);
        ...
        // zoom level 1
        assertEquals(1, theTPKFile.getMinColumn(1));
        assertEquals(1, theTPKFile.getMaxColumn(1));
        assertEquals(1, theTPKFile.getMinRow(1));
        assertEquals(1, theTPKFile.getMaxRow(1));

        // zoom level 2
        assertEquals(3, theTPKFile.getMinColumn(2));
        assertEquals(3, theTPKFile.getMaxColumn(2));
        assertEquals(2, theTPKFile.getMinRow(2));
        assertEquals(2, theTPKFile.getMaxRow(2));
        ...
        // try our "restart" constructor
        TPKFile newTPKFile = new TPKFile(theTpk, zoomLevelMap, bounds, imageFormat, Charset.forName("GBK"));
        assertNotNull(newTPKFile);
