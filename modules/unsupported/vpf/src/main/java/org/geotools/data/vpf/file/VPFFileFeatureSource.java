 ...
 */
public class VPFFileFeatureSource extends ContentFeatureSource {

    public VPFFileFeatureSource(ContentEntry entry, Query query) {
        super(entry, query);
    }

    /** Access parent VPFFileStore. */
    public VPFFileStore getDataStore() {
        return (VPFFileStore) super.getDataStore();
    }

    protected FeatureReader<SimpleFeatureType, SimpleFeature> getReaderInternal(Query query)
            throws IOException {
        return new VPFFileFeatureReader(getState(), query);
    }
    ...
    protected SimpleFeatureType buildFeatureType() throws IOException {
        VPFFileStore vpf = this.getDataStore();
