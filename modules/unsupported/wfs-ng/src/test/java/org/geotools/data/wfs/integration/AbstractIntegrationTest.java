...
public abstract class AbstractIntegrationTest {
    ...
    private void completeTestDataType(TestDataType test) throws IOException {
        ...
        test.feat12Filter = ff.id(ff.featureId(test.features[0].getID()),
    ...
    public void testGetFeatureReaderFilterAutoCommit() throws Exception {
        ...
        reader = data.getFeatureReader(new Query(first.typeName, Filter.EXCLUDE),
                Transaction.AUTO_COMMIT);

        assertEquals(type, reader.getFeatureType());
        assertEquals(0, count(reader));

        reader = data.getFeatureReader(new Query(first.typeName, first.feat1Filter),
                Transaction.AUTO_COMMIT);
    ...
    public void testGetFeatureWriter() throws Exception {
        FeatureWriter<SimpleFeatureType, SimpleFeature> writer = data.getFeatureWriter(
                first.typeName, Transaction.AUTO_COMMIT);
    ...
    public void testGetFeaturesWriterModify() throws Exception {
        ...
        FeatureReader<SimpleFeatureType, SimpleFeature> reader = data.getFeatureReader(new Query(
                first.typeName, first.feat1Filter), Transaction.AUTO_COMMIT);
    ...
    public void testGetFeatureSource(TestDataType test) throws Exception {
        ...
        Query query = new Query(test.typeName, test.feat12Filter, new String[] { test.stringAttribute, 
    ...
    public void testGetFeatureStoreAddFeatures() throws IOException {
        FeatureReader<SimpleFeatureType, SimpleFeature> reader = DataUtilities
                .reader(new SimpleFeature[] { first.newFeature, });
        SimpleFeatureStore road = (SimpleFeatureStore) data.getFeatureSource(first.typeName);
