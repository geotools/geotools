...
public class WFSDataStoreReadTest extends TestCase {
    ...
    public static void doFeatureType(URL url, boolean get, boolean post, int i) throws IOException,
            SAXException {
        if (url == null)
            return;
        try {
            ...
            if (get) {
                ...
                assertTrue("GET " + typeName
                        + " must have 1 geom and atleast 1 other attribute -- fair assumption",
                        ft.getGeometryDescriptor() != null && ft.getAttributeDescriptors() != null
                                && ft.getAttributeCount() > 0);
            }
            if (post) {
                // post
    ...
    public static void doFeatureReader(URL url, boolean get, boolean post, int i)
        ...
        try {
            ...
            if (post) {
                // post
                FeatureReader<SimpleFeatureType, SimpleFeature> ft = wfs.getFeatureReaderPost(
                        query, Transaction.AUTO_COMMIT);
                assertNotNull("FeatureType was null", ft);
                assertTrue("must have 1 feature -- fair assumption",
                        ft.hasNext() && ft.getFeatureType() != null && ft.next() != null);
                // disable for now
            ...
            if (get) {
                // get
                FeatureReader<SimpleFeatureType, SimpleFeature> ft = wfs.getFeatureReaderGet(query,
    ...
    public static void doFeatureReaderWithQuery(URL url, boolean get, boolean post, int i)
